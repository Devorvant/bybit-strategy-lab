...
def make_chart_html(
    ...
    *,
    ...
    # Right-side explanations for the currently displayed params.
    def _param_explanations_for_strategy() -> dict:
        if strategy == "my_strategy3.py":
            return {
                "position_usd": "Размер позиции в USD на сделку.",
                "use_no_trade": "Включить фильтр NO TRADE (пропуск входов в слабом тренде/флете).",
                "adx_len": "Период ADX (кол-во свечей для расчёта).",
                "adx_smooth": "Сглаживание ADX.",
                "adx_no_trade_below": "Порог ADX: если ADX ниже — включается NO TRADE.",
                "st_atr_len": "Период ATR внутри Supertrend.",
                "st_factor": "Коэффициент Supertrend (множитель ATR). Больше — шире канал, реже flip.",
                "use_rev_cooldown": "Включить паузу (cooldown) после разворота/flip.",
                "rev_cooldown_hrs": "Длительность cooldown в часах.",
                "use_flip_limit": "Ограничивать количество flip в сутки.",
                "max_flips_per_day": "Максимум flip в сутки (после — ограничение входов).",
                "use_emergency_sl": "Включить аварийный стоп-лосс (страховочный выход).",
                "atr_len": "Период ATR для аварийного SL.",
                "atr_mult": "Множитель ATR для аварийного SL (дальность стопа).",
                "close_at_end": "Закрывать позицию в конце окна (иначе может остаться OPEN).",
            }
        if strategy == "my_strategy3_tv_like.py":
            return {
                "initial_capital": "Стартовый депозит для TV-like симуляции.",
                "percent_of_equity": "Размер позиции как % от equity.",
                "commission_percent": "Комиссия в процентах (TV-like).",
                "slippage_ticks": "Проскальзывание в тиках (TV-like).",
                "tick_size": "Размер тика (syminfo.mintick).",
                "use_no_trade": "Включить фильтр NO TRADE.",
                "adx_len": "Период ADX.",
                "adx_smooth": "Сглаживание ADX.",
                "adx_no_trade_below": "Порог ADX: если ниже — NO TRADE.",
                "st_atr_len": "Период ATR внутри Supertrend.",
                "st_factor": "Коэффициент Supertrend (множитель ATR).",
                "use_rev_cooldown": "Пауза после разворота/flip.",
                "rev_cooldown_hrs": "Длительность cooldown (часы).",
                "use_flip_limit": "Ограничивать flip в сутки.",
                "max_flips_per_day": "Макс. flip в сутки.",
                "use_emergency_sl": "Аварийный стоп-лосс.",
                "atr_len": "ATR для аварийного SL.",
                "atr_mult": "Множитель ATR для SL.",
                "close_at_end": "Закрывать позицию в конце окна.",
            }
        if strategy == "my_strategy2.py":
            return {
                "position_usd": "Размер позиции в USD на сделку.",
                "fast_n": "Период быстрой SMA.",
                "slow_n": "Период медленной SMA.",
                "adx_n": "Период ADX.",
                "adx_enter": "Порог ADX для входа.",
                "adx_exit": "Порог ADX для выхода/фильтра.",
                "close_at_end": "Закрывать позицию в конце окна.",
            }
        if strategy == "my_strategy_tv_like.py":
            return {
                "position_usd": "Размер позиции в USD на сделку.",
                "fast_n": "Период быстрой SMA.",
                "slow_n": "Период медленной SMA.",
                "fee_rate": "Комиссия (доля, например 0.001 = 0.1%).",
                "slippage_bps": "Проскальзывание в bps (0.01% = 1 bp).",
                "close_at_end": "Закрывать позицию в конце окна.",
            }
        # my_strategy.py (SMA cross)
        return {
            "position_usd": "Размер позиции в USD на сделку.",
            "fast_n": "Период быстрой SMA.",
            "slow_n": "Период медленной SMA.",
    ...
    expl = _param_explanations_for_strategy()
    explain_items = "".join(
        f"<div class='e-k'>{html.escape(str(k))}</div><div class='e-v'>{html.escape(str(expl.get(k, '—')))}</div>"
    ...
    <div class="params-row">
      ...
      <div class="explain-card">
        <div class="params-head"><b>Пояснения</b><span class="muted">(что означает параметр)</span></div>
        <div class="explain-grid">{explain_items}</div>
      </div>
    </div>
    """
...
.params-row {{ display: flex; gap: 12px; align-items: flex-start; }}
.params-row .params-card {{ flex: 0 0 520px; max-width: 520px; margin: 10px 0 12px 0; }}
.params-row .explain-card {{ flex: 1 1 auto; margin: 10px 0 12px 0; padding: 10px 12px; background: #0e1830; border: 1px solid #1b2940; border-radius: 12px; }}
.explain-grid {{ margin-top: 8px; display: grid; grid-template-columns: 220px minmax(0, 1fr); gap: 6px 12px; font-size: 12px; }}
.e-k {{ opacity: 0.9; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; }}
.e-v {{ opacity: 0.95; overflow-wrap: anywhere; }}
    ...
    @media (max-width: 760px) {{
      ...
      .params-row .params-card {{ max-width: none; flex: 1 1 auto; }}
