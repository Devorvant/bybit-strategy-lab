...
def _make_auto_order_link_id(action: str, symbol: str, tf: str, bar_ts: int | None, opt_id: Any = None) -> str:
    ...
    # Entry orders must carry the exact strategy trade key.
    if a in {'OPEN_LONG', 'OPEN_SHORT'}:
        return make_strategy_trade_key(symbol, tf, opt_id, ts)

    # Close orders stay unique and readable, but do not participate in entry matching.
