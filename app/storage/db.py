...
def delete_bars(conn, symbol: str, tf: str) -> int:
    symbol = symbol.strip().upper()
    tf = str(tf).strip()
    if _is_postgres():
        with conn.cursor() as cur:
            cur.execute("DELETE FROM bars WHERE symbol=%s AND tf=%s", (symbol, tf))
            deleted = cur.rowcount or 0
        conn.commit()
        return int(deleted)
    cur = conn.execute("DELETE FROM bars WHERE symbol=? AND tf=?", (symbol, tf))
    conn.commit()
