...
class TradeChartService:
    ...
    def _build_real_trades(self, *, live_events: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ordered = sorted([e for e in live_events if str(e.get("group") or "") == "live"], key=lambda x: x.get("ts") or 0)
        if not ordered:
            return []

        explicit: dict[str, list[dict[str, Any]]] = {}
        fallback_groups: list[list[dict[str, Any]]] = []
        open_stack: list[list[dict[str, Any]]] = []

        for event in ordered:
            trade_id = str(event.get("trade_id") or "").strip()
            if trade_id:
                explicit.setdefault(trade_id, []).append(event)
                continue
            ...
            if _is_open_kind(kind):
                ...
                open_stack.append(group)
            elif _is_close_kind(kind):
                if open_stack:
                    open_stack[-1].append(event)
                    open_stack.pop()
                else:
                ...
                if open_stack:
                    ...
                    fallback_groups[-1].append(event)
                else:
                    fallback_groups.append([event])

        rows: list[dict[str, Any]] = []
        for trade_id, events in explicit.items():
            row = _build_real_trade_row(events, trade_id=trade_id)
            if row:
                rows.append(row)
        for idx, events in enumerate(fallback_groups, start=1):
            row = _build_real_trade_row(events, trade_id=f"fallback_{idx}")
            if row:
                rows.append(row)
...
def _trade_source(events: list[dict[str, Any]], exit_ev: dict[str, Any] | None) -> str | None:
    ...
    for e in events:
        ...
        if kind.startswith("manual_"):
            ...
            groups.add("auto")
    if exit_ev and str(exit_ev.get("kind") or "").startswith("exchange_") and not groups:
        return "external"
    if len(groups) > 1:
        return "mixed"
    if groups:
        ...
        if exit_ev and str(exit_ev.get("kind") or "").startswith("exchange_"):
            return "external" if base == "auto" else base
        return base
