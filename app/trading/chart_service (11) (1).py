...
def _sync_strategy_trade_journal_with_strategy(
    *,
    ...
    Rules:
    - build a map of CLOSED strategy trades by match_id/trade_key
    - determine the current OPEN strategy trade (latest trade without exit_ts), if any
    - among journal rows with status='open':
        * keep only the current open strategy trade open
        * if a journal key exists in CLOSED strategy trades and is not the current open key,
          close it using strategy exit fields
    """
    if not conn or not journal_rows:
        return 0

    closed_strategy_map: dict[str, dict[str, Any]] = {}
    ...
    for jr in journal_rows or []:
        ...
        st = closed_strategy_map.get(key)
        if not st:
            continue
