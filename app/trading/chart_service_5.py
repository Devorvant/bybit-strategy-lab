...
from .journal import fetch_strategy_trade_journal, update_strategy_trade_close
...
def _sync_strategy_trade_journal_with_strategy(
    *,
    ...
    """
    Lazy sync of strategy_trade_journal with current strategy state.

    Rules:
    - if a journal row is open and the same trade_key exists among CLOSED strategy trades,
      close the journal row using strategy exit fields.
    - keep open only the trade_key that is still the current OPEN strategy trade.
    - if a journal row is open but is no longer the current open strategy trade,
      and there is no explicit closed row for it, close it using the best available
      strategy information from its own trade row if present.
    """
    ...
    for jr in journal_rows or []:
        ...
        # keep the currently open strategy trade as open
        if current_open_key and key == current_open_key:
            continue

        st = strategy_map.get(key)
        if not st:
            # no matching strategy row found in current payload; leave as-is
            # to avoid accidental closure on unrelated filters/windows
            continue

        exit_ts = st.get("exit_ts")
        exit_price = st.get("exit_price")
        close_reason = st.get("reason") or st.get("close_reason")
        ...
        # close only if strategy row is already closed
        if not exit_ts_i:
            continue
