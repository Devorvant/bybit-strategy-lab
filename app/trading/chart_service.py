...
def _pair_real_trades(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if current:
        ...
        if row:
            rows.append(row)
    rows = _recompute_real_trade_metrics(rows)
    return rows
...
def _recompute_real_trade_metrics(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    # cumulative must be calculated from oldest to newest, regardless of display order
    chron = sorted(rows, key=lambda r: (
        _float_or_none(r.get("exit_ts")) if r.get("exit_ts") is not None else (_float_or_none(r.get("entry_ts")) or 0)
    ))
    running = 0.0
    for row in chron:
        ...
        pnl = row.get("pnl_usd")
        pnl_val = _float_or_none(pnl)
        if pnl_val is None and side and ep is not None and xp is not None and qty is not None:
            if side == "LONG":
                pnl_val = (xp - ep) * qty
            elif side == "SHORT":
                pnl_val = (ep - xp) * qty
        row["pnl_usd"] = pnl_val
        if pnl_val is None:
            row["cum_pnl_usd"] = running if chron.index(row) > 0 else None
        else:
            running += pnl_val
            row["cum_pnl_usd"] = running

    # keep display order newest first
