...
try:
    from .journal import (
        ...
        make_strategy_trade_key,
        upsert_strategy_trade_open,
        get_open_strategy_trade,
        update_strategy_trade_close,
        upsert_strategy_trade_from_strategy_row,
    )
except Exception:  # pragma: no cover
    def log_execution_event(conn, row):  # type: ignore
        return None
    def log_strategy_decision(conn, row):  # type: ignore
        return None
    def log_exchange_snapshot(conn, row):  # type: ignore
        return None
    def snapshot_from_exchange(symbol, position_snapshot, account_snapshot, **kwargs):  # type: ignore
        return {}
    def make_run_id(symbol, tf, opt_id, started_at):  # type: ignore
        return f"{symbol}:{tf}:{opt_id}:{int(started_at or 0)}"
    def log_execution_fills(conn, symbol, tf, fills):  # type: ignore
        return None
    def upsert_execution_fill_summary(conn, symbol, tf, order_link_id):  # type: ignore
        return None
    def make_strategy_trade_key(symbol, tf, opt_id, entry_ts):  # type: ignore
        return f"{symbol}:{tf}:{opt_id}:{int(entry_ts or 0)}"
    def upsert_strategy_trade_open(conn, row):  # type: ignore
        return None
    def get_open_strategy_trade(conn, **kwargs):  # type: ignore
        return None
    def update_strategy_trade_close(conn, **kwargs):  # type: ignore
        return None
    def upsert_strategy_trade_from_strategy_row(conn, row):  # type: ignore
        return None
...
AUTO_SNAPSHOT_REFRESH_SEC = float(os.getenv('AUTO_SNAPSHOT_REFRESH_SEC', '15'))
...
_LOOP_STATE: Dict[str, Any] = {
    ...
    'armed_bar_ts': None,
    'blocked_reason': None,
    'last_refresh_snapshot_ts': None,
}


def _current_run_id(symbol: str, tf: str, opt_id: Any) -> str:
    started = None
    try:
        with _LOOP_LOCK:
            if _LOOP_STATE.get('running') and str(_LOOP_STATE.get('symbol') or '') == str(symbol) and str(_LOOP_STATE.get('tf') or '') == str(tf):
                started = _LOOP_STATE.get('started_at')
    except Exception:
        started = None
    if started is None:
        started = time.time()
    return make_run_id(symbol, tf, opt_id, started)


def _journal_strategy(payload: Dict[str, Any], symbol: str, tf: str, opt_id: Any) -> None:
    try:
        conn = _get_conn()
        log_strategy_decision(conn, {
            'run_id': _current_run_id(symbol, tf, opt_id),
            'symbol': symbol,
            'tf': tf,
            ...
            'signal_note': payload.get('signal_note'),
            'exchange_side': payload.get('exchange_side'),
            'exchange_size': payload.get('exchange_size'),
...
def _make_auto_order_link_id(action: str, symbol: str, tf: str, bar_ts: int | None, opt_id: Any = None) -> str:
    ts = int(bar_ts or time.time() * 1000)
    a = str(action or '').strip().upper() or 'UNKNOWN'

    # For strategy-vs-real comparison use the exact same key format on AUTO entry orders
    # as we expect on the strategy side: SYMBOL:TF:OPT_ID:ENTRY_TS
    ...
    # Keep close orders unique and readable, but they do not participate in entry matching.
    return f"auto-{str(symbol or '').lower()}-{str(tf or '').strip()}-{a.lower()}-{ts}"


def _strategy_entry_meta(
    *,
    symbol: str,
    tf: str,
    opt_id: Any,
    ...
    if action not in {'OPEN_LONG', 'OPEN_SHORT'} or not signal_bars:
        return {}
    entry_bar = signal_bars[-1]
    entry_ts = int(entry_bar[0])
    entry_price = float(entry_bar[4])
    side = 'LONG' if action == 'OPEN_LONG' else 'SHORT'
    trade_key = make_strategy_trade_key(symbol, tf, opt_id, entry_ts)

    sl_pct = float(trade_params.get('sl_percent') or 0.0)
    tp_pct = float(trade_params.get('tp_percent') or 0.0)
...
def _journal_strategy_trade_open(
    *,
    symbol: str,
    tf: str,
    opt_id: Any,
    ...
    try:
        ...
        upsert_strategy_trade_open(conn, {
            ...
            'status': 'open',
            'bybit_order_link_id': order_link_id or meta['trade_key'],
            'created_ts': now_ms,
            'updated_ts': now_ms,
        })
    except Exception:
...
def _journal_strategy_trade_close(
    *,
    symbol: str,
    tf: str,
    opt_id: Any,
    ...
    try:
        conn = _get_conn()
        opened = get_open_strategy_trade(conn, symbol=symbol, tf=tf, strategy='my_strategy3', opt_id=opt_id)
        if not opened:
            return
        exit_ts = int(signal_bars[-1][0]) if signal_bars else int(time.time() * 1000)
        exit_price = float(signal_bars[-1][4]) if signal_bars else None
        update_strategy_trade_close(
            conn,
            trade_key=str(opened.get('trade_key') or ''),
            exit_ts=exit_ts,
...
def _sync_strategy_trade_journal_from_bt(
    *,
    symbol: str,
    tf: str,
    opt_id: Any,
    ...
    """
    Sync recent strategy trades into strategy_trade_journal.
    New rows are inserted.
    Existing rows keep entry fields and get close fields backfilled from strategy
    when strategy already has exit_ts/exit_price/close_reason.
    """
    try:
        conn = _get_conn()
    except Exception:
    ...
    for tr in recent:
        ...
        trade_key = make_strategy_trade_key(symbol, tf, opt_id, entry_ts)
        exit_ts = int(getattr(tr, 'exit_ts', 0) or 0) or None
        ...
        row = {
            ...
            'side': side,
            'entry_ts': entry_ts,
            'entry_price': _safe_float(getattr(tr, 'entry_price', None)),
            'tp_price': _safe_float(getattr(tr, 'trade_tp', None)),
            'sl_price': _safe_float(getattr(tr, 'trade_sl', None)),
            'exit_ts': exit_ts,
            'exit_price': _safe_float(getattr(tr, 'exit_price', None)),
            'close_reason': getattr(tr, 'exit_reason', None),
            'status': 'closed' if exit_ts else 'open',
            'bybit_order_link_id': trade_key,
            'created_ts': now_ms,
            'updated_ts': now_ms,
        }
        try:
            upsert_strategy_trade_from_strategy_row(conn, row)
            synced += 1
        except Exception:
            pass
    ...
    try:
        ...
        if op:
            ...
            if entry_ts:
                side = str(getattr(op, 'side', '') or '').upper()
                trade_key = make_strategy_trade_key(symbol, tf, opt_id, entry_ts)
                row = {
                    'trade_key': trade_key,
                    'symbol': symbol,
                    ...
                    'side': side,
                    'entry_ts': entry_ts,
                    'entry_price': _safe_float(getattr(op, 'entry_price', None)),
                    'tp_price': _safe_float(getattr(op, 'trade_tp', None)),
                    'sl_price': _safe_float(getattr(op, 'trade_sl', None)),
                    'exit_ts': None,
                    'exit_price': None,
                    'close_reason': None,
                    'status': 'open',
...
def _journal_execution(symbol: str, tf: str, opt_id: Any, **row: Any) -> None:
    try:
        conn = _get_conn()
        payload = {'run_id': _current_run_id(symbol, tf, opt_id), 'symbol': symbol, 'tf': tf}
        payload.update(row)
        log_execution_event(conn, payload)
    except Exception:
        pass


def _journal_snapshot(symbol: str, tf: str, opt_id: Any, *, source: str, position_snapshot: Dict[str, Any] | None, account_snapshot: Dict[str, Any] | None, bar_ts: int | None = None) -> None:
    try:
        conn = _get_conn()
        row = snapshot_from_exchange(
            symbol,
            position_snapshot,
            account_snapshot,
            source=source,
        ...
        log_exchange_snapshot(conn, row)
    except Exception:
        pass


def _persist_fills_for_order(symbol: str, tf: str, order_link_id: str | None, *, bybit: BybitClient) -> int:
    if not order_link_id:
        logger.info("auto fills persist skipped: empty order_link_id symbol=%s tf=%s", symbol, tf)
        return 0
    try:
        logger.info("auto fills persist start: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        conn = _get_conn()
        fills = bybit.find_fills_by_order_link_id(symbol=symbol, order_link_id=order_link_id, limit=100)
        logger.info("auto fills persist fetched: symbol=%s tf=%s order_link_id=%s fills_count=%s", symbol, tf, order_link_id, len(fills or []))
        if not fills:
            return 0
        ...
        for f in fills:
            ...
            try:
                ts = int(exec_time) if exec_time is not None else now_ms
            except Exception:
                ts = now_ms
            rows.append({
                "ts": ts,
                "symbol": symbol,
                "tf": tf,
                "order_link_id": f.get("orderLinkId"),
                "order_id": f.get("orderId"),
                "exec_id": f.get("execId"),
                "side": f.get("side"),
                "exec_price": f.get("execPrice"),
                "exec_qty": f.get("execQty"),
                "exec_fee": f.get("execFee"),
                "fee_currency": f.get("feeCurrency"),
                "is_maker": f.get("isMaker"),
                "reduce_only": None,
                "exec_time": exec_time,
                "payload": f.get("raw") or f,
            })

        try:
            logger.info("auto fills persist first row: order_link_id=%s row=%s", order_link_id, rows[0] if rows else None)
        except Exception:
            pass

        log_execution_fills(conn, rows)
        logger.info("auto fills persist inserted: symbol=%s tf=%s order_link_id=%s rows_count=%s", symbol, tf, order_link_id, len(rows))
        upsert_execution_fill_summary(conn, symbol=symbol, tf=tf, order_link_id=order_link_id)
        logger.info("auto fills persist summary updated: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        return len(rows)
    except Exception:
        logger.exception("auto fills persist failed: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        return 0

def _maybe_refresh_snapshot(symbol: str, tf: str, opt_id: Any, *, bybit: BybitClient, bar_ts: int | None = None, force: bool = False) -> bool:
    now = time.time()
    with _LOOP_LOCK:
        last_ts = float(_LOOP_STATE.get('last_refresh_snapshot_ts') or 0.0)
    if not force and (now - last_ts) < max(1.0, AUTO_SNAPSHOT_REFRESH_SEC):
        return False

    try:
        position_snapshot = bybit.get_position_snapshot(symbol)
        account_snapshot = bybit.get_account_snapshot()
        _journal_snapshot(
            symbol,
            tf,
            opt_id,
        ...
        with _LOOP_LOCK:
            _LOOP_STATE['last_refresh_snapshot_ts'] = now
        return True
...
def _reset_runtime_state(symbol: str, tf: str) -> Dict[str, Any]:
    st = _get_runtime_state(symbol, tf)
    st.update({
        'last_bar_ts': None,
        'last_signal': None,
        'last_action': None,
...
def process_symbol(
    ...
    *,
    ...
    # Keep the most recent bar timestamp for loop new-bar detection,
    # but if confirm_on_close=True, derive the trading signal only from
    # fully closed candles. This prevents the preview/live decision from
    # flipping to FLAT inside the currently-forming bar before the loop
    # has a chance to enter on the new candle.
    bar_ts = int(bars[-1][0])
    signal_bars = bars
    if bool(strategy_kwargs.get('confirm_on_close')) and len(bars) >= 2:
        signal_bars = bars[:-1]
    ...
    if len(signal_bars) < 50:
        return DecisionView(
            ...
            tf=tf,
            strategy='my_strategy3',
            error=f'not enough closed bars for strategy3: {len(signal_bars)}',
        ).to_dict()
    ...
    try:
        sig_out = generate_signal(signal_bars, **strategy_kwargs)
    except Exception as e:
        return DecisionView(
    ...
    try:
        bt = backtest_strategy3(
            signal_bars,
            position_usd=1000.0,
            use_no_trade=bool(strategy_kwargs.get('use_no_trade', True)),
            adx_len=int(strategy_kwargs.get('adx_len', 14)),
            adx_smooth=int(strategy_kwargs.get('adx_smooth', 14)),
            adx_no_trade_below=float(strategy_kwargs.get('adx_no_trade_below', 14.0)),
            st_atr_len=int(strategy_kwargs.get('st_atr_len', 14)),
            st_factor=float(strategy_kwargs.get('st_factor', 4.0)),
            use_rev_cooldown=bool(strategy_kwargs.get('use_rev_cooldown', True)),
            rev_cooldown_hrs=int(strategy_kwargs.get('rev_cooldown_hrs', 8)),
            use_flip_limit=bool(strategy_kwargs.get('use_flip_limit', False)),
            max_flips_per_day=int(strategy_kwargs.get('max_flips_per_day', 6)),
            use_emergency_sl=bool(strategy_kwargs.get('use_emergency_sl', True)),
            atr_len=int(strategy_kwargs.get('atr_len', 14)),
            atr_mult=float(strategy_kwargs.get('atr_mult', 3.0)),
            min_hold_bars=int(strategy_kwargs.get('min_hold_bars', 0)),
            close_at_end=False,
            confirm_on_close=bool(strategy_kwargs.get('confirm_on_close', False)),
        )
        _sync_strategy_trade_journal_from_bt(symbol=symbol, tf=tf, opt_id=opt_id, trade_params=trade_params, bt=bt, limit=30)
    except Exception:
        bt = None
    ...
    if symbol not in AUTO_TRADE_ALLOWED_SYMBOLS:
        ...
        ex_side_now = str(exchange_side or '').upper()
        expected_side = None
        if action == 'OPEN_LONG':
            expected_side = 'LONG'
        ...
        if expected_side and ex_side_now == expected_side:
    ...
    if execute and not blocked and action in {'OPEN_LONG', 'OPEN_SHORT', 'CLOSE_LONG', 'CLOSE_SHORT'}:
        ...
        entry_meta = _strategy_entry_meta(
            symbol=symbol,
            tf=tf,
            opt_id=opt_id,
            strategy_name=strategy_name or 'my_strategy3',
            action=action,
            signal_bars=signal_bars,
            trade_params=trade_params,
        )
        order_link_id = entry_meta.get('trade_key') if action in {'OPEN_LONG', 'OPEN_SHORT'} else _make_auto_order_link_id(action, symbol, tf, bar_ts, opt_id)
        if action in {'OPEN_LONG', 'OPEN_SHORT'}:
            _journal_strategy_trade_open(
                symbol=symbol,
                tf=tf,
                opt_id=opt_id,
                strategy_name=strategy_name or 'my_strategy3',
        ...
        _journal_execution(
            ...
            side='Buy' if action in {'OPEN_LONG', 'CLOSE_SHORT'} else 'Sell' if action in {'OPEN_SHORT', 'CLOSE_LONG'} else None,
            qty=None,
            price=None,
        ...
        try:
            post_pos = bybit.get_position_snapshot(symbol)
            post_acc = bybit.get_account_snapshot()
            _journal_snapshot(
                symbol,
                tf,
                opt_id,
                source='auto_after',
                position_snapshot=post_pos,
                account_snapshot=post_acc,
                bar_ts=bar_ts,
            )
        except Exception:
            pass
        try:
            _persist_fills_for_order(
                symbol,
                tf,
                (result_order_link_id or order_link_id),
                bybit=bybit,
            )
        except Exception:
            pass
        if executed and action in {'CLOSE_LONG', 'CLOSE_SHORT'}:
            _journal_strategy_trade_close(
                symbol=symbol,
                tf=tf,
                opt_id=opt_id,
                signal_bars=signal_bars,
                close_reason=(execution_result or {}).get('note') or reason,
            )
    ...
    _log_event(payload)
    _journal_strategy(payload, symbol, tf, opt_id)
    return payload
...
def _loop_runner() -> None:
    while not _LOOP_STOP.is_set():
        ...
        try:
            ...
            # Always use a fresh DB connection snapshot for each loop tick so long-lived
            # readers do not get stuck on stale bars after external writes/websocket updates.
            ...
            try:
                _maybe_refresh_snapshot(symbol, tf, opt_id, bybit=BybitClient(), bar_ts=cur_bar_ts, force=False)
            except Exception:
                pass

            # If a position that the loop opened on this signal was manually/externaly closed,
            # do not re-enter on the same bar. Re-arm until the next fresh bar.
            preview_state = dict(preview.get('state') or {}) if isinstance(preview, dict) else {}
            preview_action = str(preview.get('action') or '').upper() if isinstance(preview, dict) else ''
            preview_side = str(preview.get('exchange_side') or '').upper() if isinstance(preview, dict) else ''
            last_exec_action = str(preview_state.get('last_execute_action') or '').upper()
            last_exec_bar_ts = preview_state.get('last_execute_bar_ts')
            external_flat_after_entry = (
                cur_bar_ts is not None
                and preview_side == 'FLAT'
                and preview_action in {'OPEN_LONG', 'OPEN_SHORT'}
                and last_exec_action == preview_action
                and last_exec_bar_ts is not None
                and int(last_exec_bar_ts) == int(cur_bar_ts)
            )
            if external_flat_after_entry:
                st = _get_runtime_state(symbol, tf)
                st['last_execute_bar_ts'] = None
                st['last_execute_action'] = None
                st['last_execution_ok'] = None
                with _LOOP_LOCK:
                    _LOOP_STATE['state'] = 'ARMED_WAIT_NEXT_BAR'
                    _LOOP_STATE['armed_bar_ts'] = int(cur_bar_ts)
                loop_state = 'ARMED_WAIT_NEXT_BAR'
                armed_bar_ts = cur_bar_ts
                result = dict(preview)
                result['reason'] = 'manual/external close detected; wait next bar before re-entry'
                result['execute_requested'] = False
                result['executed'] = False
                result['blocked'] = False
                _log_event({'ok': True, 'mode': 'loop', 'event': 'external_close_rearm', 'bar_ts': cur_bar_ts, 'symbol': symbol, 'tf': tf})

            if loop_state == 'ARMED_WAIT_NEXT_BAR':
                # stale armed state: if the current bar is older than the loop start context
                # re-arm from the freshest seen bar.
                ...
                # If we should already be on the next bar by clock, force one more fresh read
                # before deciding that we are still waiting. This helps after manual close /
                # stop-start when a long-lived DB reader lags one snapshot behind.
                if (
                    cur_bar_ts is not None and armed_bar_ts is not None and tf_ms > 0
                    and int(cur_bar_ts) <= int(armed_bar_ts)
                    and now_ms >= int(armed_bar_ts) + tf_ms
                ):
                    _reset_db_connection()
                    refreshed = process_symbol(symbol=symbol, tf=tf, execute=False, opt_id=opt_id, trade_params=tp)
                    if refreshed.get('ok'):
                        result = refreshed
                        cur_bar_ts = refreshed.get('bar_ts')
...
def start_auto_loop(symbol: str, tf: str, *, opt_id: int, poll_sec: float | None = None, trade_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Start the auto loop in ARMED_WAIT_NEXT_BAR mode.

    Rationale: keep behavior consistent with manual CLOSE handling and with
    confirm_on_close-style strategies: the loop will enter only on a fresh bar.
    If you want an immediate entry, use the UI's 'Execute once'.
    """
    global _LOOP_THREAD
    symbol = (symbol or '').strip().upper()
    tf = str(tf or '').strip()
    ...
    # Reset stale DB/runtime state BEFORE probing current bars so Start Auto arms from
    # a fresh bar snapshot rather than an old cached connection state.
    _reset_db_connection()
    _reset_runtime_state(symbol, tf)
    _reset_loop_state()

    probe = process_symbol(
        symbol=symbol,
        tf=tf,
        execute=False,
        opt_id=int(opt_id),
...
def stop_auto_loop() -> Dict[str, Any]:
    ...
    with _LOOP_LOCK:
        ...
        symbol = str(_LOOP_STATE.get('symbol') or '')
        tf = str(_LOOP_STATE.get('tf') or '')
