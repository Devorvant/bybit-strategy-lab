...
def _persist_fills_for_order(symbol: str, tf: str, order_link_id: str | None, *, bybit: BybitClient) -> int:
    ...
    try:
        ...
        if fills:
            try:
                logger.info("auto fills persist first fill: order_link_id=%s fill=%s", order_link_id, fills[0])
            except Exception:
                pass
        if not fills:
            return 0
        log_execution_fills(conn, symbol=symbol, tf=tf, fills=fills)
        logger.info("auto fills persist inserted: symbol=%s tf=%s order_link_id=%s fills_count=%s", symbol, tf, order_link_id, len(fills or []))
