...
from app.strategy.my_strategy3 import generate_signal as strat3_generate_signal, generate_signal_via_backtest as strat3_generate_signal_via_backtest
...
PARAM_HELP = {
    ...
    "min_hold_bars": "Мин. число баров удержания перед переворотом",
...
def _build_fig_and_bt(
    ...
    *,
    ...
    # Styling
    if strategy == "my_strategy3.py":
        fig.add_annotation(text=f"Signal mode: {html.escape(str(signal_mode))}", xref="paper", yref="paper", x=0.99, y=1.12, showarrow=False, font=dict(size=11, color="#94a3b8"), align="right")
    fig.update_layout(
        height=760,
...
def make_chart_html(
    ...
    *,
    ...
    selected_opt_entry: Optional[dict] = None,
    ...
    plot_html = _build_plot_html(
        ...
        capital_usd=float(capital_usd),
        signal_mode=str(signal_mode or "backtest"),
        div_id="chartPlot",
    )
    ...
    signal_mode = str(signal_mode or "backtest").strip().lower()
    if signal_mode not in ("legacy", "backtest"):
        signal_mode = "backtest"
    ...
    if strategy == "my_strategy3.py":
        try:
            bars_list = list(bars)
            sig_params = {
                "use_no_trade": bool(cur_params.get("use_no_trade", True)),
                "adx_len": int(cur_params.get("adx_len", 14)),
                "adx_smooth": int(cur_params.get("adx_smooth", 14)),
                "adx_no_trade_below": float(cur_params.get("adx_no_trade_below", 14.0)),
                "st_atr_len": int(cur_params.get("st_atr_len", 14)),
                "st_factor": float(cur_params.get("st_factor", 4.0)),
                "use_rev_cooldown": bool(cur_params.get("use_rev_cooldown", True)),
                "rev_cooldown_hrs": int(cur_params.get("rev_cooldown_hrs", 8)),
                "use_flip_limit": bool(cur_params.get("use_flip_limit", False)),
                "max_flips_per_day": int(cur_params.get("max_flips_per_day", 6)),
                "use_emergency_sl": bool(cur_params.get("use_emergency_sl", True)),
                "atr_len": int(cur_params.get("atr_len", 14)),
                "atr_mult": float(cur_params.get("atr_mult", 3.0)),
                "min_hold_bars": int(cur_params.get("min_hold_bars", 0)),
                "confirm_on_close": bool(cur_params.get("confirm_on_close", False)),
            }
            sig_legacy = strat3_generate_signal(bars_list, **{k:v for k,v in sig_params.items() if k in {"use_no_trade","adx_len","adx_smooth","adx_no_trade_below","st_atr_len","st_factor","use_rev_cooldown","rev_cooldown_hrs"}})
            sig_bt = strat3_generate_signal_via_backtest(bars_list, **sig_params)
            def _sig_badge(sigobj, selected=False):
                if sigobj is None:
                    s='N/A'; note=''
                else:
                    s=str(getattr(sigobj,'signal','N/A'))
                    note=str(getattr(sigobj,'note',''))
                bg = '#16a34a' if s == 'LONG' else ('#dc2626' if s == 'SHORT' else '#475569')
                ring = 'box-shadow:0 0 0 2px #93c5fd inset;' if selected else ''
                return f"<div style='padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;{ring}'><div style='font-size:11px;color:#94a3b8;'>{{html.escape(note or 'signal')}}</div><div style='display:inline-block;margin-top:4px;padding:3px 8px;border-radius:999px;background:{bg};color:white;font-weight:700;'>{{html.escape(s)}}</div></div>"
            legacy_signal = str(getattr(sig_legacy, "signal", "-") if sig_legacy is not None else "-")
            legacy_note = str(getattr(sig_legacy, "note", "") if sig_legacy is not None else "")
            bt_signal = str(getattr(sig_bt, "signal", "-") if sig_bt is not None else "-")
            bt_note = str(getattr(sig_bt, "note", "") if sig_bt is not None else "")
        except Exception as _e:
            signal_compare_html = f"<div class='params-card'><div class='params-head'><b>Signal compare</b></div><div class='muted'>error: {html.escape(str(_e))}</div></div>"
    ...
    slider_html = ""
    if strategy == "my_strategy3.py":
        MAIN_PLAY_KEYS = ["st_factor", "atr_mult", "adx_no_trade_below", "rev_cooldown_hrs", "max_flips_per_day", "min_hold_bars"]
        ...
        PLAY_SPECS = {
            ...
            "min_hold_bars": {"min": 0, "max": 24, "step": 1, "default": 0, "kind": "int", "digits": 0},
    ...
    selected_opt_auto = False
    if isinstance(selected_opt_entry, dict):
        try:
            selected_opt_id = int(selected_opt_entry.get("id")) if selected_opt_entry.get("id") is not None else None
        except Exception:
            selected_opt_id = None
        selected_opt_auto = bool(selected_opt_entry.get("auto_trade_enabled"))
    ...
    <input id="mt_tf" type="text" value="2h" placeholder="tf"
        ...
        <input id="mt_leverage" type="number" step="1" min="1" placeholder="Lev"
               style="width:70px;padding:6px 8px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;" />
        <input id="mt_qty_usd" type="number" step="0.01" min="0" placeholder="Margin USDT"
               style="width:70px;padding:6px 8px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;" />
    ...
    auto_trade_html = f"""
    <div class="manual-trade-box" style="margin:12px 0;padding:10px;border:1px solid #2a3b5f;border-radius:10px;background:#0b1220;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
        <div style="font-weight:700;color:#dbeafe;">Auto Wrapper (strategy 3)</div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          <label style="display:flex;align-items:center;gap:6px;color:#cbd5e1;font-size:12px;padding:4px 8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;">
            <input id="mt_auto_enabled_for_strategy" type="checkbox" {'checked' if selected_opt_auto else ''} {'disabled' if not selected_opt_id else ''}/>
            Участвует в автоторговле
          </label>
          <span id="mt_auto_opt_label" data-selected-opt-id="{selected_opt_id if selected_opt_id is not None else ''}" style="font-size:12px;color:#cbd5e1;">Preset: {('#' + str(selected_opt_id)) if selected_opt_id is not None else 'не выбран'}</span>
          <label style="display:flex;align-items:center;gap:6px;color:#cbd5e1;font-size:12px;">
            <span>Risk profile</span>
            <select id="mt_risk_profile_select" style="min-width:180px;padding:4px 8px;border-radius:8px;border:1px solid #23324f;background:#0f172a;color:#e2e8f0;">
              <option value="">выбери профиль</option>
            </select>
          </label>
          <button id="mt_auto_preview" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #2563eb;background:#0b1220;color:#dbeafe;cursor:pointer;">Preview</button>
          <button id="mt_auto_process" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #166534;background:#123320;color:#bbf7d0;cursor:pointer;">Process once</button>
          <button id="mt_auto_execute" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #9a3412;background:#3b1d0a;color:#fed7aa;cursor:pointer;">Execute once</button>
          <button id="mt_auto_loop_start" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #14532d;background:#052e16;color:#bbf7d0;cursor:pointer;">Start Auto</button>
          <button id="mt_auto_loop_stop" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #7f1d1d;background:#3f0d0d;color:#fecaca;cursor:pointer;">Stop Auto</button>
          <button id="mt_bars_clear" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #7c2d12;background:#431407;color:#fed7aa;cursor:pointer;">Clear bars</button>
          <button id="mt_bars_rebuild" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #1d4ed8;background:#172554;color:#bfdbfe;cursor:pointer;">Rebuild bars</button>
          <span id="mt_auto_status" style="font-size:12px;color:#93c5fd;">Idle</span>
        </div>
      </div>
      <div style="margin-top:8px;display:grid;grid-template-columns:repeat(8,minmax(120px,1fr));gap:8px;">
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Signal</div><div id="mt_auto_signal" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Action</div><div id="mt_auto_action" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Exchange side</div><div id="mt_auto_side" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Preset</div><div id="mt_auto_preset" style="font-weight:700;color:#e2e8f0;">{('#' + str(selected_opt_id)) if selected_opt_id is not None else '-'}</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Mode</div><div id="mt_auto_mode" style="font-weight:700;color:#e2e8f0;">preview</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Loop</div><div id="mt_auto_loop" style="font-weight:700;color:#e2e8f0;">stopped</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Risk profile</div><div id="mt_auto_risk_profile" style="font-weight:700;color:#e2e8f0;">-</div><div id="mt_auto_risk_meta" style="font-size:11px;color:#94a3b8;margin-top:2px;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Executed</div><div id="mt_auto_exec" style="font-weight:700;color:#e2e8f0;">-</div></div>
      </div>
      <pre id="mt_auto_result" style="margin-top:8px;white-space:pre-wrap;font-size:12px;color:#cbd5e1;max-height:260px;overflow:auto;"></pre>
    </div>
    """
    ...
    <div class="manual-trade-box" style="margin:12px 0;padding:10px;border:1px solid #2a3b5f;border-radius:10px;background:#0b1220;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
        <div style="font-weight:700;color:#dbeafe;">Live Position (Bybit)</div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          <button id="mt_live_refresh" type="button" style="padding:5px 10px;border-radius:8px;border:1px solid #2563eb;background:#0b1220;color:#dbeafe;cursor:pointer;">Refresh</button>
          <label style="display:flex;align-items:center;gap:6px;color:#cbd5e1;font-size:12px;">
            <input id="mt_live_auto" type="checkbox" checked />
            Auto
          </label>
          <span id="mt_live_status" style="font-size:12px;color:#93c5fd;">Idle</span>
        </div>
      </div>

      <div style="margin-top:8px;display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:8px;">
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Side</div><div id="mt_lp_side" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Size</div><div id="mt_lp_size" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Entry</div><div id="mt_lp_entry" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Mark</div><div id="mt_lp_mark" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">PnL</div><div id="mt_lp_pnl" style="font-weight:700;color:#e2e8f0;">-</div></div>

        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Leverage</div><div id="mt_lp_lev" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Liq</div><div id="mt_lp_liq" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">TP</div><div id="mt_lp_tp" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">SL</div><div id="mt_lp_sl" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Symbol</div><div id="mt_lp_symbol" style="font-weight:700;color:#e2e8f0;">(selected)</div></div>
      </div>
    </div>

    <div style="margin-top:10px;padding:10px;border:1px solid #23324f;border-radius:10px;background:#0b1220;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
        <div style="font-size:14px;font-weight:700;color:#e2e8f0;">Account (Bybit)</div>
        <div style="display:flex;align-items:center;gap:10px;">
          <button id="mt_acc_refresh" type="button" onclick="try{window.mtRefreshAccountInfo ? window.mtRefreshAccountInfo(true) : (window.mtRefreshAccount && window.mtRefreshAccount(true));}catch(e){console.error(e)}" style="padding:4px 10px;border-radius:8px;border:1px solid #2563eb;background:#0b1220;color:#dbeafe;font-weight:600;cursor:pointer;">Refresh</button>
          <span id="mt_acc_status" style="font-size:12px;color:#93c5fd;">Idle</span>
        </div>
      </div>
      <div style="margin-top:8px;display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:8px;">
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Equity</div><div id="mt_ac_equity" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Wallet Balance</div><div id="mt_ac_wallet" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Available Balance</div><div id="mt_ac_available" style="font-weight:700;color:#e2e8f0;">-</div></div>
        <div style="padding:8px;border:1px solid #23324f;border-radius:8px;background:#0f172a;"><div style="font-size:11px;color:#94a3b8;">Unrealized PnL total</div><div id="mt_ac_upnl" style="font-weight:700;color:#e2e8f0;">-</div></div>
      </div>
    </div>
    """
...
function mtSetLiveFieldColored(id, value, color) {{
...
function mtColorizePnl(v) {{
  const n = Number(v);
  if (!isFinite(n)) {{
    mtSetLiveFieldColored('mt_lp_pnl', '-', '#e2e8f0');
    return;
  }}
  const txt = (n >= 0 ? '+' : '') + mtFmtNum(n) + ' USDT';
  const color = n > 0 ? '#22c55e' : (n < 0 ? '#ef4444' : '#e2e8f0');
  mtSetLiveFieldColored('mt_lp_pnl', txt, color);
}}

async function mtLiveClosePosition() {{
  const btn = document.getElementById('mt_live_close');
  const statusEl = document.getElementById('mt_live_status');
  const symbol = mtResolveSymbol();
  if (!confirm('Закрыть позицию MARKET 100% по ' + symbol + '?')) return;
  if (btn) btn.disabled = true;
  try {{
    if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Closing ' + symbol + '...';
    const payload = {{
      symbol: symbol,
      action: 'CLOSE',
      tf: (document.getElementById('mt_tf')?.value || '2h').trim() || '2h',
      note: (document.getElementById('mt_note')?.value || 'manual from terminal').trim()
    }};
    ...
    if (data && data.ok) {{
      if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Close sent: ' + symbol;
      try {{ await mtRefreshLivePosition(); }} catch(_e) {{}}
      try {{ await mtRefreshAccountInfo(); }} catch(_e) {{}}
    }} else {{
      if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Close error';
    }}
  }} catch (e) {{
    if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Close error: ' + (e?.message || e);
  }} finally {{
    if (btn) btn.disabled = false;
  }}
}}

function mtSetLiveField(id, value) {{
        const el = document.getElementById(id);
      ...
      function mtColorizeField(id, v) {{
        const el = document.getElementById(id);
        if (!el) return;
        const n = Number(v);
        if (!isFinite(n)) {{ el.style.color = '#e2e8f0'; return; }}
        el.style.color = n > 0 ? '#22c55e' : (n < 0 ? '#ef4444' : '#e2e8f0');
      }}
      function mtRenderAccountInfo(payload) {{
        const st = document.getElementById('mt_acc_status');
        if (!payload) {{
          if (st) st.textContent = 'No data';
          ['mt_ac_equity','mt_ac_wallet','mt_ac_available','mt_ac_upnl'].forEach(id => mtSetLiveField(id,'-'));
          return;
        }}
        const wrap = (payload.account ? payload : {{ account: payload }});
        const a = wrap.account || {{}};
        if (wrap.ready === false) {{
          if (st) st.textContent = 'Bybit not ready';
          ['mt_ac_equity','mt_ac_wallet','mt_ac_available','mt_ac_upnl'].forEach(id => mtSetLiveField(id,'-'));
          return;
        }}
        mtSetLiveField('mt_ac_equity', mtFmtUsd(a.equity));
        mtSetLiveField('mt_ac_wallet', mtFmtUsd(a.walletBalance));
        mtSetLiveField('mt_ac_available', mtFmtUsd(a.availableBalance));
        mtSetLiveField('mt_ac_upnl', mtFmtUsd(a.unrealizedPnlTotal));
        mtColorizeField('mt_ac_upnl', a.unrealizedPnlTotal);
        if (st) st.textContent = '[' + new Date().toLocaleTimeString() + '] Updated';
      }}
      async function mtRefreshAccountInfo(_force) {{
        const st = document.getElementById('mt_acc_status');
        if (st) st.textContent = '[' + new Date().toLocaleTimeString() + '] Refreshing...';
        try {{
          const r = await fetch('/trade/account');
          const t = await r.text();
          let j;
          try {{ j = JSON.parse(t); }} catch (_) {{ j = {{ ok:false, raw:t }}; }}
          if (!r.ok) {{
            if (st) st.textContent = '[' + new Date().toLocaleTimeString() + '] ERROR loading account';
            return;
          }}
          mtRenderAccountInfo(j);
        }} catch(e) {{
          if (st) st.textContent = '[' + new Date().toLocaleTimeString() + '] ERROR: ' + (e && e.message ? e.message : String(e));
        }}
      }}
      // expose for inline onclick + bind click (same pattern as live-position refresh)
      try {{ window.mtRefreshAccountInfo = mtRefreshAccountInfo; }} catch (e) {{}}
      const mtAccRefreshBtn = document.getElementById('mt_acc_refresh');
      if (mtAccRefreshBtn && !mtAccRefreshBtn.dataset.bound) {{
        mtAccRefreshBtn.dataset.bound = '1';
        mtAccRefreshBtn.addEventListener('click', function(ev) {{
          try {{ ev.preventDefault(); ev.stopPropagation(); }} catch (e) {{}}
          mtRefreshAccountInfo(true);
        }});
      }}

      function mtSetupAccountAuto() {{
        const btn = document.getElementById('mt_acc_refresh');
        if (btn && !btn.dataset.bound) {{
          btn.dataset.bound = '1';
          btn.addEventListener('click', function() {{ mtRefreshAccountInfo(); }});
        }}
        mtRefreshAccountInfo();
      }}
      
function mtResolveSymbol() {{
  const manual = document.getElementById('mt_symbol')?.value?.trim();
  if (manual) return manual.toUpperCase().replace('.P', '');

  const manualLegacy = document.getElementById('manual_symbol')?.value?.trim();
  if (manualLegacy) return manualLegacy.toUpperCase().replace('.P', '');

  const selByName = document.querySelector('select[name="symbol"]')?.value?.trim();
  if (selByName) return selByName.toUpperCase().replace('.P', '');

  const selAny = document.querySelector('select[id*="symbol"], select[id^="symbol"]')?.value?.trim();
  if (selAny) return selAny.toUpperCase().replace('.P', '');

  const qp = new URLSearchParams(location.search).get('symbol');
  if (qp) return String(qp).trim().toUpperCase().replace('.P', '');

  return 'BTCUSDT';
}}

function mtRenderLivePosition(payload) {{
        const wrap = (payload && payload.position) ? payload.position : payload;
        const statusEl = document.getElementById('mt_live_status');
        mtSetLiveField('mt_lp_symbol', mtResolveSymbol());
        if (!wrap) {{
        ...
        if (!wrap.ready) {{
          if (statusEl) statusEl.textContent = 'Bybit not ready';
          ['mt_lp_side','mt_lp_size','mt_lp_entry','mt_lp_mark','mt_lp_pnl','mt_lp_lev','mt_lp_liq','mt_lp_tp','mt_lp_sl'].forEach(function(id) {{ mtSetLiveField(id, '-'); }});
          return;
        }}
        if (!wrap.has_position || !pos) {{
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] No open position';
          mtSetLiveField('mt_lp_side', 'No position');
          ['mt_lp_size','mt_lp_entry','mt_lp_mark','mt_lp_pnl','mt_lp_lev','mt_lp_liq','mt_lp_tp','mt_lp_sl'].forEach(function(id) {{ mtSetLiveField(id, '-'); }});
          return;
        }}
        if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Updated ' + (wrap.symbol || '');
        mtSetLiveField('mt_lp_side', pos.side || '-');
        mtSetLiveField('mt_lp_size', mtFmtNum(pos.size) + ' ' + ((wrap.symbol || mtResolveSymbol()).replace('USDT','').replace('USDC','')));
        mtSetLiveField('mt_lp_entry', mtFmtNum(pos.avgPrice));
        mtSetLiveField('mt_lp_mark', mtFmtNum(pos.markPrice));
        mtColorizePnl(pos.unrealisedPnl);
        mtSetLiveField('mt_lp_lev', (pos.leverage != null && pos.leverage !== '') ? (String(pos.leverage) + 'x') : '-');
        mtSetLiveField('mt_lp_liq', mtFmtNum(pos.liqPrice));
        mtSetLiveField('mt_lp_tp', mtFmtNum(pos.takeProfit));
        mtSetLiveField('mt_lp_sl', mtFmtNum(pos.stopLoss));
      }}
      async function mtRefreshLivePosition() {{
        const statusEl = document.getElementById('mt_live_status');
        const sym = mtGetCurrentSymbolBase ? mtGetCurrentSymbolBase() : 'APTUSDT';
        if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Refreshing ' + sym + '...';
        try {{
          const r = await fetch('/trade/position?symbol=' + encodeURIComponent(sym));
          const t = await r.text();
          let j;
          try {{ j = JSON.parse(t); }} catch (_) {{ j = {{ ok: false, raw: t }}; }}
          if (!r.ok) {{
            if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ERROR loading position';
            return;
          }}
          mtRenderLivePosition(j);
        }} catch (e) {{
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ERROR: ' + (e && e.message ? e.message : String(e));
        }}
      }}
      function mtSetupLiveAuto() {{
        const btn = document.getElementById('mt_live_refresh');
        const chk = document.getElementById('mt_live_auto');
        if (btn && !btn.dataset.bound) {{
          btn.dataset.bound = '1';
          btn.addEventListener('click', function() {{ mtRefreshLivePosition(); }});
        }}
        function resetTimer() {{
          if (mtLiveTimer) {{ clearInterval(mtLiveTimer); mtLiveTimer = null; }}
          if (chk && chk.checked) {{
            mtLiveTimer = setInterval(function() {{ mtRefreshLivePosition(); }}, 5000);
          }}
        }}
        if (chk && !chk.dataset.bound) {{
          chk.dataset.bound = '1';
          chk.addEventListener('change', resetTimer);
        }}
        resetTimer();
        mtRefreshLivePosition();
        try {{ mtRefreshAccountInfo(); }} catch(_e) {{}}
      }}
      ...
      async function mtSendTradeAction(action) {{
        ...
        const levEl = document.getElementById('mt_leverage');
        const qtyEl = document.getElementById('mt_qty_usd');
        const slEl = document.getElementById('mt_sl_pct');
        const tpEl = document.getElementById('mt_tp_pct');
        const tfv = ((tfEl && tfEl.value) || '2h').trim();
        const note = ((noteEl && noteEl.value) || 'manual from terminal').trim();
        const leverageRaw = ((levEl && levEl.value) || '').trim();
        const qtyUsdRaw = ((qtyEl && qtyEl.value) || '').trim();
        const slPctRaw = ((slEl && slEl.value) || '').trim();
        const tpPctRaw = ((tpEl && tpEl.value) || '').trim();
        const symbolBase = mtGetCurrentSymbolBase();
        const symbolBridge = symbolBase.replace(/\.P$/i, '');
        if (['LONG', 'SHORT', 'CLOSE'].indexOf(action) === -1) {{ if (out) out.textContent = 'Unsupported action: ' + action; return; }}
        if (!confirm('Отправить ' + action + ' на ' + symbolBridge + ' (tf=' + tfv + (leverageRaw ? ', lev=' + leverageRaw : '') + ')?')) return;
        const payload = {{ symbol: symbolBridge, tf: tfv, action: action, note: note }};
        if (leverageRaw) payload.leverage = Number(leverageRaw);
        if (qtyUsdRaw) payload.position_usd = Number(qtyUsdRaw);
        if (slPctRaw) payload.sl_percent = Number(slPctRaw);
        if (tpPctRaw) payload.tp_percent = Number(tpPctRaw);
        mtSending = true;
        mtSetButtonsDisabled(true);
        if (out) out.textContent = '[' + new Date().toLocaleTimeString() + '] Sending ' + action + '...\\n' + JSON.stringify(payload, null, 2);
        try {{
          const headers = {{ 'Content-Type': 'application/json' }};const resp = await fetch('/trade/execute', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify(payload)
          }});
          ...
          if (out) {{
            ...
            if (okFlag) {{
              ...
              try {{ mtRefreshLivePosition(); }} catch (_) {{}}
            }} else {{
      ...
      function mtSetAutoField(id, value) {{
        const el = document.getElementById(id);
        if (el) el.textContent = (value == null || value === '') ? '-' : String(value);
      }}
      function mtGetSelectedOptId() {{
        const optEl = document.querySelector("select[name='opt_id']");
        const raw = optEl && optEl.value ? String(optEl.value).trim() : '';
        return raw && /^\d+$/.test(raw) ? Number(raw) : null;
      }}
      function mtSetAutoExecuteEnabled(enabled, titleText) {{
        const btn = document.getElementById('mt_auto_execute');
        if (!btn) return;
      ...
      function mtGetRiskProfileSelect() {{
        return document.getElementById('mt_risk_profile_select');
      }}
      function mtSetRiskProfileSummary(profile) {{
        const nameEl = document.getElementById('mt_auto_risk_profile');
        const metaEl = document.getElementById('mt_auto_risk_meta');
        if (!profile) {{
          if (nameEl) nameEl.textContent = 'not selected';
          if (metaEl) metaEl.textContent = 'выбери профиль';
          return;
        }}
        if (nameEl) nameEl.textContent = profile.name || profile.id || '-';
        const usd = (profile.position_usd != null ? String(profile.position_usd) : '-');
        const lev = (profile.default_leverage != null ? String(profile.default_leverage) : '-');
        const sl = (profile.sl_percent != null ? String(profile.sl_percent) : '-');
        if (metaEl) metaEl.textContent = 'USD ' + usd + ' · Lev ' + lev + 'x · SL ' + sl + '%';
      }}
      function mtFillRiskProfileSelect(profiles, selectedId) {{
        const sel = mtGetRiskProfileSelect();
        if (!sel) return;
        const arr = Array.isArray(profiles) ? profiles : [];
        sel.innerHTML = '';
        const empty = document.createElement('option');
        empty.value = '';
        empty.textContent = 'выбери профиль';
      ...
      async function mtLoadRiskProfiles() {{
        const sel = mtGetRiskProfileSelect();
        const statusEl = document.getElementById('mt_auto_status');
        if (sel) {{
          sel.disabled = true;
        ...
        try {{
          const resp = await fetch('/trade/risk-profiles');
          const data = await resp.json();
          profiles = (data && Array.isArray(data.profiles)) ? data.profiles : [];
        }} catch (e) {{
          profiles = [];
        }}
        const optId = mtGetSelectedOptId();
        if (optId) {{
          try {{
            const resp = await fetch('/trade/strategy3/risk-profile?opt_id=' + encodeURIComponent(String(optId)));
            const data = await resp.json();
            selectedId = (data && data.risk_profile_id) ? String(data.risk_profile_id) : '';
            selectedProfile = (data && data.risk_profile) ? data.risk_profile : null;
          }} catch (e) {{
            selectedId = '';
            selectedProfile = null;
          }}
        }}
        mtFillRiskProfileSelect(profiles, selectedId);
        if (!selectedProfile && selectedId) {{
          selectedProfile = (profiles || []).find(function(p) {{ return String(p.id) === String(selectedId); }}) || null;
        }}
        mtSetRiskProfileSummary(selectedProfile);
        if (sel) sel.disabled = !optId;
        if (statusEl && optId && !selectedId) statusEl.textContent = 'Выбери risk profile для preset';
      }}
      async function mtSaveRiskProfileSelection() {{
        const optId = mtGetSelectedOptId();
        const sel = mtGetRiskProfileSelect();
        const statusEl = document.getElementById('mt_auto_status');
        if (!sel) return;
        const profileId = String(sel.value || '').trim();
        if (!optId) {{
          if (statusEl) statusEl.textContent = 'Сначала выбери preset';
          return;
        }}
        if (!profileId) {{
          mtSetRiskProfileSummary(null);
          if (statusEl) statusEl.textContent = 'Выбери risk profile';
          return;
        }}
        sel.disabled = true;
        if (statusEl) statusEl.textContent = 'Сохраняю risk profile...';
        try {{
          const resp = await fetch('/trade/strategy3/risk-profile', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ opt_id: optId, risk_profile_id: profileId }})
          }});
          const data = await resp.json();
          const profile = (data && data.risk_profile) ? data.risk_profile : null;
          mtSetRiskProfileSummary(profile);
          if (statusEl) statusEl.textContent = 'Risk profile сохранён';
        }} catch (e) {{
          if (statusEl) statusEl.textContent = 'Ошибка сохранения risk profile';
        }} finally {{
          sel.disabled = false;
        }}
      }}
      async function mtRefreshAutoPresetUi() {{
        const optId = mtGetSelectedOptId();
        const statusEl = document.getElementById('mt_auto_status');
        const labelEl = document.getElementById('mt_auto_opt_label');
        const cb = document.getElementById('mt_auto_enabled_for_strategy');
        mtSetAutoField('mt_auto_preset', optId ? ('#' + String(optId)) : '-');
        if (labelEl) labelEl.textContent = 'Preset: ' + (optId ? ('#' + String(optId)) : 'не выбран');
        if (!cb) return;
        cb.disabled = !optId;
        if (!optId) {{
          cb.checked = false;
          mtSetAutoExecuteEnabled(false, 'Сначала выбери сохранённый номер стратегии');
          if (statusEl) statusEl.textContent = 'Выбери saved preset';
          mtSetAutoField('mt_auto_loop', 'STOPPED');
          mtFillRiskProfileSelect([], '');
          mtSetRiskProfileSummary(null);
          return;
        }}
        try {{
          const resp = await fetch('/trade/strategy3/auto-status?opt_id=' + encodeURIComponent(String(optId)));
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
          const st = parsed && parsed.strategy ? parsed.strategy : null;
          cb.checked = !!(st && st.auto_trade_enabled);
          mtSetAutoExecuteEnabled(!!(st && st.auto_trade_enabled), st && st.auto_trade_enabled ? '' : 'Для execute включи флаг автоторговли у выбранного номера');
          if (statusEl) statusEl.textContent = (st && st.auto_trade_enabled) ? 'Preset разрешён для auto' : 'Preset manual only';
        }} catch (e) {{
          cb.checked = false;
          mtSetAutoExecuteEnabled(false, 'Не удалось получить статус preset');
          if (statusEl) statusEl.textContent = 'Ошибка чтения статуса preset';
        }}
      }}
      async function mtToggleAutoPreset(enabled) {{
        const optId = mtGetSelectedOptId();
        const statusEl = document.getElementById('mt_auto_status');
        const cb = document.getElementById('mt_auto_enabled_for_strategy');
        if (!optId) {{
          if (cb) cb.checked = false;
          mtSetAutoExecuteEnabled(false, 'Сначала выбери saved preset');
          if (statusEl) statusEl.textContent = 'Нельзя включить auto без выбранного номера';
          return;
        }}
        if (statusEl) statusEl.textContent = 'Сохраняю флаг auto...';
        try {{
          const resp = await fetch('/trade/strategy3/auto-status', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ opt_id: optId, auto_trade_enabled: !!enabled }})
          }});
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
          const st = parsed && parsed.strategy ? parsed.strategy : null;
          const isOn = !!(st && st.auto_trade_enabled);
          if (cb) cb.checked = isOn;
          mtSetAutoExecuteEnabled(isOn, isOn ? '' : 'Для execute включи флаг автоторговли у выбранного номера');
          if (statusEl) statusEl.textContent = isOn ? 'Auto разрешён для preset #' + String(optId) : 'Auto выключен для preset #' + String(optId);
        }} catch (e) {{
          if (cb) cb.checked = !enabled;
          if (statusEl) statusEl.textContent = 'Ошибка сохранения флага auto';
        }}
      }}
      function mtRenderAutoResult(payload, mode) {{
        const res = (payload && payload.result) ? payload.result : payload;
        const out = document.getElementById('mt_auto_result');
        const st = document.getElementById('mt_auto_status');
        if (!res) {{
          if (st) st.textContent = 'No data';
          return;
        }}
        mtSetAutoField('mt_auto_signal', res.signal || '-');
        mtSetAutoField('mt_auto_action', res.action || '-');
        mtSetAutoField('mt_auto_side', res.exchange_side || '-');
        mtSetAutoField('mt_auto_bar', res.bar_ts || '-');
        mtSetAutoField('mt_auto_mode', mode || (res.execute_requested ? 'process' : 'preview'));
        mtSetAutoField('mt_auto_preset', res.opt_id ? ('#' + String(res.opt_id)) : '-');
        mtSetAutoField('mt_auto_exec', res.executed ? 'YES' : (res.execute_requested ? 'NO' : '-'));
        mtSetRiskProfileSummary((res.risk_profile || null));
        if (st) st.textContent = '[' + new Date().toLocaleTimeString() + '] ' + (res.reason || (res.ok ? 'OK' : 'ERROR'));
        if (out) out.textContent = JSON.stringify(res, null, 2);
      }}
      function mtSetLoopButtons(running) {{
        const startBtn = document.getElementById('mt_auto_loop_start');
        const stopBtn = document.getElementById('mt_auto_loop_stop');
        if (startBtn) {{
      ...
      async function mtRefreshLoopStatus() {{
        try {{
          const resp = await fetch('/trade/auto/loop/status');
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
          const loop = parsed && parsed.loop ? parsed.loop : {{}};
          const running = !!loop.running;
          const loopState = String(loop.state || (running ? 'RUNNING' : 'STOPPED'));
          mtSetAutoField('mt_auto_loop', loopState);
          mtSetLoopButtons(running);
          if (loop.symbol || loop.tf || loop.opt_id) {{
            const st = document.getElementById('mt_auto_status');
            if (st && running) st.textContent = '[' + new Date().toLocaleTimeString() + '] loop running ' + (loop.symbol || '') + ' tf=' + (loop.tf || '') + (loop.opt_id ? (' preset #' + String(loop.opt_id)) : '');
          }}
        }} catch (e) {{
          mtSetAutoField('mt_auto_loop', 'error');
          mtSetLoopButtons(false);
        }}
      }}
      async function mtStartAutoLoop() {{
        const statusEl = document.getElementById('mt_auto_status');
        const sym = mtGetCurrentSymbolBase ? mtGetCurrentSymbolBase() : 'APTUSDT';
        const tfEl = document.getElementById('tf') || document.getElementById('mt_tf');
        const tfv = ((tfEl && tfEl.value) || '120').trim();
        const optId = mtGetSelectedOptId();
        const levEl = document.getElementById('mt_leverage');
        const qtyEl = document.getElementById('mt_qty_usd');
        const slEl = document.getElementById('mt_sl_pct');
        const tpEl = document.getElementById('mt_tp_pct');
        const trade_params = {{}};
        if (levEl && String(levEl.value || '').trim()) trade_params.leverage = Number(levEl.value);
        if (qtyEl && String(qtyEl.value || '').trim()) trade_params.position_usd = Number(qtyEl.value);
        if (slEl && String(slEl.value || '').trim()) trade_params.sl_percent = Number(slEl.value);
        if (tpEl && String(tpEl.value || '').trim()) trade_params.tp_percent = Number(tpEl.value);
        if (!optId) {{
          if (statusEl) statusEl.textContent = 'Выбери saved preset для auto loop';
          return;
        }}
        if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Starting auto loop...';
        try {{
          const resp = await fetch('/trade/auto/loop/start', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ symbol: sym.replace(/\.P$/i, ''), tf: tfv, opt_id: optId, risk_profile_id: (mtGetRiskProfileSelect() && mtGetRiskProfileSelect().value) || null, trade_params }})
          }});
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
          const out = document.getElementById('mt_auto_result');
          if (out) out.textContent = JSON.stringify(parsed, null, 2);
          const ok = !!(parsed && parsed.ok);
          const loop = parsed && parsed.loop ? parsed.loop : {{ running: false }};
          mtSetAutoField('mt_auto_loop', String(loop.state || (loop.running ? 'RUNNING' : 'STOPPED')));
          mtSetLoopButtons(!!loop.running);
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ' + (parsed.reason || (ok ? 'Auto loop started' : 'Auto loop start failed'));
        }} catch (e) {{
          mtSetAutoField('mt_auto_loop', 'error');
          mtSetLoopButtons(false);
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] loop start error: ' + (e && e.message ? e.message : String(e));
        }}
      }}
      async function mtStopAutoLoop() {{
        const statusEl = document.getElementById('mt_auto_status');
        if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] Stopping auto loop...';
        try {{
          const resp = await fetch('/trade/auto/loop/stop', {{ method: 'POST' }});
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
          const out = document.getElementById('mt_auto_result');
          ...
          mtSetAutoField('mt_auto_loop', 'STOPPED');
          mtSetLoopButtons(false);
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ' + (parsed.reason || 'Auto loop stopped');
        }} catch (e) {{
          mtSetAutoField('mt_auto_loop', 'error');
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] loop stop error: ' + (e && e.message ? e.message : String(e));
        }}
      }}
      async function mtBarsAdmin(action) {{
        const statusEl = document.getElementById('mt_auto_status');
        const sym = mtGetCurrentSymbolBase ? mtGetCurrentSymbolBase() : 'APTUSDT';
        const tfEl = document.getElementById('tf') || document.getElementById('mt_tf');
        const tfv = ((tfEl && tfEl.value) || '120').trim();
        const endpoint = action === 'rebuild' ? '/admin/bars/rebuild' : '/admin/bars/clear';
        const label = action === 'rebuild' ? 'Rebuild bars' : 'Clear bars';
        if (!confirm(label + ' for current symbol/tf?\n\n' + sym.replace(/\.P$/i, '') + ' / ' + tfv)) return;
        if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ' + label + '...';
        try {{
          const resp = await fetch(endpoint, {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ symbol: sym.replace(/\.P$/i, ''), tf: tfv }})
          }});
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
          const out = document.getElementById('mt_auto_result');
          if (out) out.textContent = JSON.stringify(parsed, null, 2);
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ' + (parsed && parsed.ok ? (label + ' done') : (label + ' failed'));
        }} catch (e) {{
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ' + label + ' error: ' + (e && e.message ? e.message : String(e));
        }}
      }}
      async function mtAutoCall(execute) {{
        const statusEl = document.getElementById('mt_auto_status');
        const sym = mtGetCurrentSymbolBase ? mtGetCurrentSymbolBase() : 'APTUSDT';
        const tfEl = document.getElementById('tf') || document.getElementById('mt_tf');
        const tfv = ((tfEl && tfEl.value) || '120').trim();
        ...
        if (tpEl && String(tpEl.value || '').trim()) trade_params.tp_percent = Number(tpEl.value);
        const optId = mtGetSelectedOptId();
        const payload = {{ symbol: sym.replace(/\.P$/i, ''), tf: tfv, execute: !!execute, trade_params, opt_id: optId, risk_profile_id: (mtGetRiskProfileSelect() && mtGetRiskProfileSelect().value) || null }};
        if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ' + (execute ? 'Processing/Executing...' : 'Previewing...');
        try {{
          let resp;
          if (execute) {{
            resp = await fetch('/trade/auto/process', {{
              method: 'POST',
              headers: {{ 'Content-Type': 'application/json' }},
              body: JSON.stringify(payload)
            }});
          }} else {{
            const q = '/trade/auto/preview?symbol=' + encodeURIComponent(payload.symbol) + '&tf=' + encodeURIComponent(payload.tf) + (payload.opt_id ? ('&opt_id=' + encodeURIComponent(String(payload.opt_id))) : '') + (payload.risk_profile_id ? ('&risk_profile_id=' + encodeURIComponent(String(payload.risk_profile_id))) : '');
            resp = await fetch(q);
          }}
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok: false, raw: txt }}; }}
          mtRenderAutoResult(parsed, execute ? 'process/execute' : 'preview');
          try {{ mtRefreshLivePosition(); }} catch (_) {{}}
        }} catch (e) {{
          if (statusEl) statusEl.textContent = '[' + new Date().toLocaleTimeString() + '] ERROR: ' + (e && e.message ? e.message : String(e));
        }}
      }}
      var mtAutoPreviewBtn = document.getElementById('mt_auto_preview');
      var mtAutoProcessBtn = document.getElementById('mt_auto_process');
      var mtAutoExecuteBtn = document.getElementById('mt_auto_execute');
      var mtAutoLoopStartBtn = document.getElementById('mt_auto_loop_start');
      var mtAutoLoopStopBtn = document.getElementById('mt_auto_loop_stop');
      var mtBarsClearBtn = document.getElementById('mt_bars_clear');
      var mtBarsRebuildBtn = document.getElementById('mt_bars_rebuild');
      var mtAutoEnabledCb = document.getElementById('mt_auto_enabled_for_strategy');
      var mtOptSelect = document.querySelector("select[name='opt_id']");
      var mtRiskProfileSelect = document.getElementById('mt_risk_profile_select');
      if (mtAutoEnabledCb) mtAutoEnabledCb.addEventListener('change', function() {{ mtToggleAutoPreset(!!mtAutoEnabledCb.checked); }});
      if (mtOptSelect) mtOptSelect.addEventListener('change', function() {{ mtRefreshAutoPresetUi();
      mtLoadRiskProfiles(); }});
      if (mtRiskProfileSelect) mtRiskProfileSelect.addEventListener('change', function() {{ mtSaveRiskProfileSelection(); }});
      mtRefreshAutoPresetUi();
      mtLoadRiskProfiles();
      if (mtAutoPreviewBtn) mtAutoPreviewBtn.addEventListener('click', function() {{ mtAutoCall(false); }});
      if (mtAutoProcessBtn) mtAutoProcessBtn.addEventListener('click', function() {{ mtAutoCall(false); }});
      if (mtAutoExecuteBtn) mtAutoExecuteBtn.addEventListener('click', function() {{
        if (!confirm('Выполнить один цикл auto wrapper для strategy3?')) return;
        mtAutoCall(true);
      }});
      if (mtAutoLoopStartBtn) mtAutoLoopStartBtn.addEventListener('click', function() {{ mtStartAutoLoop(); }});
      if (mtAutoLoopStopBtn) mtAutoLoopStopBtn.addEventListener('click', function() {{ mtStopAutoLoop(); }});
      if (mtBarsClearBtn) mtBarsClearBtn.addEventListener('click', function() {{ mtBarsAdmin('clear'); }});
      if (mtBarsRebuildBtn) mtBarsRebuildBtn.addEventListener('click', function() {{ mtBarsAdmin('rebuild'); }});
      ...
      mtRefreshLoopStatus();
      setInterval(mtRefreshLoopStatus, 10000);
      mtSetupLiveAuto();
...
<body>
  <form id="chartForm" class="topbar" method="get" action="/chart">
    <a class="navlink active" href="/chart?symbol={html.escape(symbol)}&tf={tf_e}&strategy={html.escape(strategy)}&signal_mode={html.escape(signal_mode)}&signal_mode={html.escape(signal_mode)}&limit={int(limit)}">📈 Chart</a>
  ...
  <div class="wrap">{params_html}{slider_html}{manual_trade_html}{live_trade_html}{auto_trade_html}{plot_html}{trades_table_html}</div>
</body>
</html>
"""
