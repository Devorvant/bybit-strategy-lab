...
def _persist_fills_for_order(symbol: str, tf: str, order_link_id: str | None, *, bybit: BybitClient) -> int:
    if not order_link_id:
        logger.warning("auto fills persist skipped: empty order_link_id symbol=%s tf=%s", symbol, tf)
        return 0
    try:
        logger.warning("auto fills persist start: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        conn = _get_conn()
        fills = []
        ...
        for attempt, delay_sec in enumerate(delays, start=1):
            ...
            try:
                fills = bybit.find_fills_by_order_link_id(symbol=symbol, order_link_id=order_link_id, limit=100) or []
            except Exception:
                fills = []
            logger.warning(
                "auto fills persist fetched: symbol=%s tf=%s order_link_id=%s attempt=%s delay=%ss fills_count=%s",
                symbol, tf, order_link_id, attempt, delay_sec, len(fills),
            )
            if fills:
                break

        if not fills:
            logger.warning("auto fills persist exhausted: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
            return 0
        ...
        try:
            logger.warning("auto fills persist first row: order_link_id=%s row=%s", order_link_id, rows[0] if rows else None)
        except Exception:
            pass

        log_execution_fills(conn, rows)
        logger.warning("auto fills persist inserted: symbol=%s tf=%s order_link_id=%s rows_count=%s", symbol, tf, order_link_id, len(rows))
        upsert_execution_fill_summary(conn, symbol=symbol, tf=tf, order_link_id=order_link_id)
        logger.warning("auto fills persist summary updated: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        return len(rows)
    except Exception:
