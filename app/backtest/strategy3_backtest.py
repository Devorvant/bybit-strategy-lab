...
def backtest_strategy3(
    ...
    for i in range(len(bars)):
        ...
        if use_emergency_sl and pos != 0 and atr_v[i] is not None:
            ...
            if pos == 1:
                ...
                if l[i] <= stop_px:
                    pnl = entry_size_usd * pos * (stop_px - entry_price) / entry_price
                    realized += pnl
                    cum += pnl
                    trades.append(
        ...
        if no_trade[i]:
            if pos != 0:
                ...
                if entry_price <= 0 or px <= 0:
                    ...
                    equity[-1] = realized
                    continue
                pnl = entry_size_usd * pos * (px - entry_price) / entry_price
                realized += pnl
                cum += pnl
                trades.append(
    ...
    if pos != 0:
        ...
        if close_at_end:
            ...
            pnl = entry_size_usd * pos * (cur_px - entry_price) / entry_price
            realized += pnl
            cum += pnl
            trades.append(
                Trade(
