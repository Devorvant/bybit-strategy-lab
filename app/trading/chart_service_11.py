...
from .journal import fetch_strategy_trade_journal, upsert_strategy_trade_from_strategy_row, replace_strategy_trade_journal_slice
    ...
    for jr in journal_rows or []:
        ...
        try:
            upsert_strategy_trade_from_strategy_row(
                ...
                opt_id=opt_id,
                strategy_row=st,
                bybit_order_link_id=str(jr.get("bybit_order_link_id") or key),
            )
            updated += 1
        except Exception:
            pass
...
def rebuild_strategy_trade_journal(
    *,
    ...
    payload_rows = []
    now_ms = _now_ms()
    for tr in list(strategy_trades or [])[:last_trades]:
        payload_rows.append({
            "trade_key": tr.get("match_id"),
            "side": tr.get("side"),
            "entry_ts": tr.get("entry_ts"),
            "entry_price": tr.get("entry_price"),
            "tp_price": tr.get("trade_tp"),
            "sl_price": tr.get("trade_sl"),
            "exit_ts": tr.get("exit_ts"),
            "exit_price": tr.get("exit_price"),
            "reason": tr.get("reason"),
            "close_reason": tr.get("reason"),
            "status": "closed" if tr.get("exit_ts") else "open",
            "created_ts": now_ms,
            "updated_ts": now_ms,
            "bybit_order_link_id": tr.get("match_id"),
        })

    written = replace_strategy_trade_journal_slice(
        svc.repo.conn,
        symbol=symbol,
        tf=tf,
        strategy="my_strategy3",
