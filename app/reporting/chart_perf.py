...
# Keyed by (strategy, bars_sig, params_hash, capital_usd_rounded).
# TTL keeps memory bounded and handles live updates.
_PLOT_CACHE = {}  # type: ignore[var-annotated]
...
def _params_hash(strategy_params: Optional[dict]) -> str:
    if not strategy_params:
        return "none"
    try:
        s = json.dumps(strategy_params, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    except Exception:
...
def build_plotly_payload(
    ...
    *,
    ...
    sig = _bars_sig(bars)
    ph = _params_hash(strategy_params)
    cap_key = round(float(capital_usd or 0.0), 6)
    key = (strategy, sig, ph, cap_key)

    now = time.monotonic()
    rec = _PLOT_CACHE.get(key)
    if rec is not None:
        ts0, payload = rec
        if now - ts0 <= _PLOT_CACHE_TTL_SEC:
            return payload
    ...
    if err_html is not None or fig is None:
        payload = {"ok": False, "error_html": err_html or "<div>error</div>"}
        _PLOT_CACHE[key] = (now, payload)
        return payload
    ...
    payload = {
        ...
        "trades_html": _build_trades_table_html(bt, bars=bars, params=strategy_params, max_rows=_TRADES_TABLE_MAX_ROWS),
    }
    _PLOT_CACHE[key] = (now, payload)
    return payload
...
def _build_plot_html(
    ...
    *,
    ...
    ph = _params_hash(strategy_params)
    cap_key = round(float(capital_usd or 0.0), 6)
    key = (f"html:{strategy}", sig, ph, cap_key, div_id)
    ...
    if err_html is not None or fig is None:
        html0 = err_html or '<div style="padding:12px;font-size:14px;">No bars yet for this symbol/tf.</div>'
        _PLOT_CACHE[key] = (now, html0)
        return html0
...
def _build_trades_table_html(bt, bars=None, params=None, max_rows: int = 200) -> str:
