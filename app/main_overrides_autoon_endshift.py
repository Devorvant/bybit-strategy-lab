...
def _load_bars_cached(symbol: str, tf: str, limit: int, end_shift_hours: float = 0.0) -> list:
    """Load last `limit` bars for (symbol, tf). If end_shift_hours>0, end the window earlier.

    IMPORTANT: cache key includes an hourly bucket of the computed end_ts so shifted windows don't all map to the same cache entry.
    """
    try:
        end_shift_hours_f = float(end_shift_hours or 0.0)
    except Exception:
        end_shift_hours_f = 0.0

    # Find latest timestamp to anchor the shift.
    # load_bars(limit=1) should return the most recent bar for this symbol/tf.
    conn = get_conn()
    latest = load_bars(conn, symbol, tf, limit=1)
    latest_ts = None
    if latest:
        # row can be dict-like; ts key is expected.
        latest_ts = int(latest[-1].get("ts"))
    if latest_ts is None:
        return []

    end_ts = latest_ts - int(end_shift_hours_f * 3600 * 1000)

    # Cache in 1-hour buckets to prevent unbounded keys, but still differentiate shifts.
    end_bucket = int(end_ts // (3600 * 1000))
    key = (symbol, tf, int(limit), end_bucket)
    now = time.monotonic()
    ...
    if end_shift_hours_f > 0:
        rows = load_bars_before(conn, symbol, tf, end_ts=end_ts, limit=limit)
    else:
...
def bars(symbol: str = Query("APTUSDT"), tf: str = Query(None), limit: int = Query(500, ge=10, le=50000)):
    ...
    rows = _load_bars_cached(symbol.upper(), tf, limit, end_shift_hours=0.0)
...
def chart(
    ...
    end_shift_hours: float = Query(0.0, ge=0.0, le=1e6),
