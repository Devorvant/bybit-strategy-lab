...
def _cmp_pct(real_v: Any, strategy_v: Any) -> float | None:
    rv = _float_or_none(real_v)
    sv = _float_or_none(strategy_v)
    if rv is None or sv is None or sv == 0:
        return None
    return (rv - sv) / sv * 100.0


def _build_auto_comparison(strategy_rows: list[dict[str, Any]], real_rows: list[dict[str, Any]]) -> dict[str, Any]:
    strategy_rows = strategy_rows or []
    real_auto_rows = [r for r in (real_rows or []) if str(r.get("source") or "").lower() == "auto"]

    strategy_map: dict[str, dict[str, Any]] = {}
    for s in strategy_rows:
        key = str(s.get("match_id") or "").strip()
        if key:
            strategy_map[key] = s

    real_map: dict[str, dict[str, Any]] = {}
    for r in real_auto_rows:
        key = str(r.get("entry_code") or "").strip()
        if key:
            real_map[key] = r

    all_keys = sorted(set(strategy_map.keys()) | set(real_map.keys()))
    rows: list[dict[str, Any]] = []

    matched_entry_pct = []
    matched_exit_pct = []
    sum_strategy_pnl = 0.0
    sum_real_pnl = 0.0
    sum_delta_pnl = 0.0
    ...
    for idx, key in enumerate(all_keys, start=1):
        ...
        side = (s or r or {}).get("side")

        strategy_entry_ts = (s or {}).get("entry_ts")
        real_entry_ts = (r or {}).get("entry_ts")
        strategy_exit_ts = (s or {}).get("exit_ts")
        real_exit_ts = (r or {}).get("exit_ts")

        strategy_entry_price = (s or {}).get("entry_price")
        real_entry_price = (r or {}).get("entry_price")
        strategy_exit_price = (s or {}).get("exit_price")
        real_exit_price = (r or {}).get("exit_price")

        strategy_pnl = _float_or_none((s or {}).get("pnl_usd") or (s or {}).get("pnl"))
        real_pnl = _float_or_none((r or {}).get("net_pnl_usd") or (r or {}).get("pnl_usd") or (r or {}).get("pnl"))

        delta_entry_sec = None
        if strategy_entry_ts is not None and real_entry_ts is not None:
            delta_entry_sec = (_float_or_none(real_entry_ts) - _float_or_none(strategy_entry_ts)) / 1000.0

        delta_exit_sec = None
        if strategy_exit_ts is not None and real_exit_ts is not None:
            delta_exit_sec = (_float_or_none(real_exit_ts) - _float_or_none(strategy_exit_ts)) / 1000.0

        delta_entry_price = None
        sev = _float_or_none(strategy_entry_price)
        rev = _float_or_none(real_entry_price)
        if sev is not None and rev is not None:
            delta_entry_price = rev - sev

        delta_exit_price = None
        sxv = _float_or_none(strategy_exit_price)
        rxv = _float_or_none(real_exit_price)
        if sxv is not None and rxv is not None:
            delta_exit_price = rxv - sxv

        delta_pnl = None
        if strategy_pnl is not None and real_pnl is not None:
            delta_pnl = real_pnl - strategy_pnl
        ...
        row = {
            ...
            "strategy_entry_ts": strategy_entry_ts,
            "real_entry_ts": real_entry_ts,
            "delta_entry_sec": delta_entry_sec,

            "strategy_entry_price": strategy_entry_price,
            "real_entry_price": real_entry_price,
            "delta_entry_price": delta_entry_price,
            "delta_entry_price_pct": _cmp_pct(real_entry_price, strategy_entry_price),

            "strategy_exit_ts": strategy_exit_ts,
            "real_exit_ts": real_exit_ts,
            "delta_exit_sec": delta_exit_sec,

            "strategy_exit_price": strategy_exit_price,
            "real_exit_price": real_exit_price,
            "delta_exit_price": delta_exit_price,
            "delta_exit_price_pct": _cmp_pct(real_exit_price, strategy_exit_price),

            "strategy_pnl_usd": strategy_pnl,
            "real_net_pnl_usd": real_pnl,
            "delta_pnl_usd": delta_pnl,

            "strategy_reason": (s or {}).get("exit_reason"),
            "real_reason": (r or {}).get("close_reason"),

            "real_entry_code": (r or {}).get("entry_code"),
            "real_exit_code": (r or {}).get("exit_code"),
            "comment": status,
        }
        rows.append(row)

        if status == "matched":
            dep = _float_or_none(row["delta_entry_price_pct"])
            dxp = _float_or_none(row["delta_exit_price_pct"])
            if dep is not None:
                matched_entry_pct.append(dep)
            if dxp is not None:
                matched_exit_pct.append(dxp)
            if strategy_pnl is not None:
                sum_strategy_pnl += strategy_pnl
            if real_pnl is not None:
                sum_real_pnl += real_pnl
            if delta_pnl is not None:
                sum_delta_pnl += delta_pnl

    summary = {
        "strategy_count": len(strategy_rows),
        "real_auto_count": len(real_auto_rows),
        "matched_count": sum(1 for r in rows if r["status"] == "matched"),
        "missing_real_count": sum(1 for r in rows if r["status"] == "missing_real"),
        "extra_real_count": sum(1 for r in rows if r["status"] == "extra_real"),
        "avg_entry_slippage_pct": (sum(matched_entry_pct) / len(matched_entry_pct)) if matched_entry_pct else None,
        "avg_exit_slippage_pct": (sum(matched_exit_pct) / len(matched_exit_pct)) if matched_exit_pct else None,
        "sum_strategy_pnl": sum_strategy_pnl if rows else None,
        "sum_real_net_pnl": sum_real_pnl if rows else None,
        "sum_delta_pnl": sum_delta_pnl if rows else None,
    }
...
def _attach_fill_summary(rows: list[dict[str, Any]], repo: TradeChartRepo) -> list[dict[str, Any]]:
    ...
    for row in ordered:
        ...
        row["exec_source"] = "db_fill" if (entry_summary or exit_summary) else "reconstructed"
