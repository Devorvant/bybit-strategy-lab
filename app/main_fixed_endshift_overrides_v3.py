...
def _get_row_ts(row) -> int:
    ...
    try:
        ...
        pass
    # tuple/list fallback: ts is first column in our queries
    try:
        return int(row[0])
    except Exception:
        return 0
...
def _load_bars_cached(symbol: str, tf: str, limit: int, end_shift_hours: float = 0.0) -> list:
    ...
    limit_i = int(limit)
    # Find latest ts (ms) quickly
    latest_ts = _get_latest_ts(symbol, tf)

    end_ts = latest_ts - int(float(end_shift_hours or 0.0) * 3600 * 1000)
    # Bucket by hour to keep cache useful but correct.
    ...
    if end_shift_hours and end_shift_hours != 0:
