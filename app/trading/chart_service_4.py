...
def _sync_strategy_trade_journal_with_strategy(
    *,
    ...
    """
    Lazy sync:
    if a journal row is still open, but current strategy backtest already has the same trade closed,
    then close the journal row using strategy exit fields.
    """
    if not conn or not strategy_trades or not journal_rows:
        return 0
