...
PARAM_HELP = {
    ...
    "leverage_mult": "Мультипликатор (плечо). Позиция = капитал × мультипликатор",
    "trade_from_current_capital": "Торговать от текущего капитала (компаунд)",
    "slippage_ticks": "Проскальзывание (тики) на вход/выход",
    "tick_size": "Размер тика цены (шаг цены)",
}
...
def _build_params_items() -> str:
    # Strategy3: show grouped params with colored headers.
    if strategy == "my_strategy3.py":
        groups = [
            ("Исходные данные", "g-base", ["capital_usd", "leverage_mult", "trade_from_current_capital"]),
            ("Комиссии и проскальзывание", "g-costs", ["slippage_ticks", "tick_size"]),
            ("Коэф. модели", "g-model", [
                "use_no_trade",
                "adx_no_trade_below",
                "st_factor",
        ...
        for title, cls, keys in groups:
            keys_present = [k for k in keys if k in cur_params and k not in used]
            if not keys_present:
                continue
            out.append(f"<div class='p-g {cls}'>{html.escape(title)}</div>")
            for k in keys_present:
                ...
                used.add(k)
        # Any leftovers (keep stable iteration order as in dict)
        leftovers = [(k, v) for k, v in cur_params.items() if k not in used]
        if leftovers:
            out.append(f"<div class='p-g'>Прочее</div>")
    ...
    params_items = _build_params_items()
    params_html = f"""
    ...
    try:
        if strategy == "my_strategy2.py":
            ...
            # UI-only key
            params_for_metrics = dict(params)
            bt = backtest_strategy3(bars, **params)
        elif strategy == "my_strategy_tv_like.py":
