  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", () => {{
      ...
      function mtGetCurrentSymbolBase() {{
        var manualEl = document.getElementById('mt_symbol');
        var manualVal = manualEl && manualEl.value ? String(manualEl.value).trim() : '';
        if (manualVal) return manualVal;

        var sel = document.querySelector("select[name='symbol']");
        if (sel && sel.value) return String(sel.value).trim();

        var qs = new URLSearchParams(location.search);
        var fromUrl = qs.get('symbol');
      ...
      async function mtSendTradeAction(action) {{
        ...
        const note = ((noteEl && noteEl.value) || 'manual from terminal').trim();
        const symbolBase = (typeof mtGetCurrentSymbolBase === 'function' ? mtGetCurrentSymbolBase() : ((document.getElementById('mt_symbol') && document.getElementById('mt_symbol').value) ? document.getElementById('mt_symbol').value.trim() : 'APTUSDT'));
        const symbolBridge = symbolBase.endsWith('.P') ? symbolBase : (symbolBase + '.P');
