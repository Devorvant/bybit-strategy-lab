...
from app.storage.db import init_db, load_bars, load_bars_before, last_signal, delete_bars
from app.data.bybit_ws import ws_collect
from app.data.backfill import backfill_on_startup, backfill_symbol_tf
...
from app.trading.strategy_registry import ensure_opt_auto_columns, get_opt_strategy_entry
app.include_router(trading_router)

conn = None

# Very small in-memory cache to avoid re-reading the same bars repeatedly when user tweaks params.
_BARS_CACHE: Dict[Tuple[str, str, int, int], Tuple[float, list]] = {}
_BARS_CACHE_TTL_SEC = 60.0

# Cache latest ts per (symbol, tf) for a short time to avoid extra DB hits on rapid UI updates.
_LATEST_TS_CACHE: Dict[Tuple[str, str], Tuple[float, int]] = {}
_LATEST_TS_TTL_SEC = 2.0

def _get_row_ts(row) -> int:
    """Support both dict-like and tuple rows."""
    if row is None:
        return 0
    try:
        return int(row.get('ts'))  # type: ignore[attr-defined]
    except Exception:
        pass
    try:
        return int(getattr(row, 'ts'))
    except Exception:
        pass
    try:
        return int(row[0])
...
def _get_latest_ts(symbol: str, tf: str) -> int:
    key = (symbol, tf)
    now = time.monotonic()
    rec = _LATEST_TS_CACHE.get(key)
    if rec is not None:
        t0, ts0 = rec
        if now - t0 <= _LATEST_TS_TTL_SEC:
            return ts0
    rows_last = load_bars(conn, symbol, tf, limit=1)
    latest_ts = _get_row_ts(rows_last[-1]) if rows_last else 0
    _LATEST_TS_CACHE[key] = (now, latest_ts)
    return latest_ts



def _load_bars_cached(symbol: str, tf: str, limit: int, end_shift_hours: float = 0.0) -> list:
    """Load bars with a small TTL cache. Cache key includes an end_ts hour bucket so different window shifts don't mix."""
    limit_i = int(limit)
    latest_ts = _get_latest_ts(symbol, tf)
    end_ts = latest_ts - int(float(end_shift_hours or 0.0) * 3600 * 1000)
    end_bucket = int(end_ts // (3600 * 1000)) if end_ts > 0 else 0

    key = (symbol, tf, limit_i, end_bucket)
    now = time.monotonic()
    ...
    if end_shift_hours and float(end_shift_hours) != 0.0:
        rows = load_bars_before(conn, symbol, tf, end_ts=end_ts, limit=limit_i)
    else:
        rows = load_bars(conn, symbol, tf, limit=limit_i)
...
def health():
    return {
        ...
        "trade_env": settings.TRADE_ENV,
        "bybit_category": settings.CATEGORY,
...
@app.post("/admin/bars/clear")
async def admin_clear_bars(payload: dict = Body(...)):
    symbol = str((payload or {}).get("symbol") or "").strip().upper()
    tf = str((payload or {}).get("tf") or "").strip()
    if not symbol or not tf:
        return {"ok": False, "error": "symbol and tf are required"}
    deleted = delete_bars(conn, symbol, tf)
    try:
        _BARS_CACHE.clear()
    except Exception:
        pass
    return {"ok": True, "symbol": symbol, "tf": tf, "deleted": int(deleted)}


@app.post("/admin/bars/rebuild")
async def admin_rebuild_bars(payload: dict = Body(...)):
    symbol = str((payload or {}).get("symbol") or "").strip().upper()
    tf = str((payload or {}).get("tf") or "").strip()
    days = int((payload or {}).get("days") or settings.BACKFILL_DAYS)
    if not symbol or not tf:
        return {"ok": False, "error": "symbol and tf are required"}
    deleted = delete_bars(conn, symbol, tf)
    try:
        ...
        pass
    await backfill_symbol_tf(conn, symbol=symbol, tf=tf, days=days, sleep_ms=settings.BACKFILL_SLEEP_MS)
    try:
        _BARS_CACHE.clear()
    except Exception:
        pass
    rows = _load_bars_cached(symbol, tf, 10)
    return {
        "ok": True,
        "symbol": symbol,
        "tf": tf,
...
def chart(
    ...
    end_shift_hours: float = Query(0.0),
    ...
    p_adx_len: int | None = Query(None, ge=1, le=500),
    p_adx_smooth: int | None = Query(None, ge=1, le=500),
    p_adx_no_trade_below: float | None = Query(None, ge=0.0, le=100.0),
    p_st_atr_len: int | None = Query(None, ge=1, le=500),
    p_st_factor: float | None = Query(None, ge=0.0, le=100.0),
    p_use_rev_cooldown: bool | None = Query(None),
    ...
    # Auto-enable overrides when any p_* parameter is provided in the URL.
    if not use_overrides:
        use_overrides = int(any(v is not None for v in [
            p_position_usd, p_leverage_mult, p_trade_from_current_capital,
            p_slippage_ticks, p_tick_size, p_fee_percent, p_spread_ticks, p_funding_8h_percent,
    ...
    tf = tf or settings.TF
    symbol = symbol.upper()
    rows = _load_bars_cached(symbol, tf, limit, end_shift_hours=end_shift_hours)
    ...
    if opt_id_int is not None:
        try:
            ensure_opt_auto_columns(conn)
            selected_opt_entry = get_opt_strategy_entry(opt_id_int, conn)
        except Exception:
            selected_opt_entry = None
    ...
    if strategy == "my_strategy3.py" and int(use_overrides or 0) == 1:
        ...
        if p_adx_len is not None:
            overrides["adx_len"] = int(p_adx_len)
        if p_adx_smooth is not None:
            overrides["adx_smooth"] = int(p_adx_smooth)
        if p_adx_no_trade_below is not None:
            overrides["adx_no_trade_below"] = float(p_adx_no_trade_below)
        if p_st_atr_len is not None:
            overrides["st_atr_len"] = int(p_st_atr_len)
        if p_st_factor is not None:
            overrides["st_factor"] = float(p_st_factor)
        if p_use_rev_cooldown is not None:
            overrides["use_rev_cooldown"] = bool(p_use_rev_cooldown)
        ...
        if p_use_emergency_sl is not None:
            overrides["use_emergency_sl"] = bool(p_use_emergency_sl)
        if p_atr_len is not None:
        ...
        if p_confirm_on_close is not None:
            overrides["confirm_on_close"] = bool(p_confirm_on_close)
    ...
    return make_chart_html(
        ...
        selected_opt_entry=selected_opt_entry,
