...
def _make_client() -> Optional['HTTP']:
    ...
    if mode == 'demo':
        key = getattr(settings, 'BYBIT_DEMO_KEY', '')
        secret = getattr(settings, 'BYBIT_DEMO_SECRET', '')
        if not (key and secret):
            return None
        # pybit uses `domain` without protocol for custom hosts
        return HTTP(testnet=False, api_key=key, api_secret=secret, domain='api-demo.bybit.com')
    ...
    if mode == 'mainnet':
        ...
        if not (key and secret):
            return None
        return HTTP(testnet=False, api_key=key, api_secret=secret)
...
class BybitClient:
    ...
    def ensure_ready(self):
        if self.client is None:
            raise HTTPException(status_code=500, detail='Bybit API not configured for current mode (set BYBIT_KEY/BYBIT_SECRET or BYBIT_DEMO_KEY/BYBIT_DEMO_SECRET and install pybit)')
