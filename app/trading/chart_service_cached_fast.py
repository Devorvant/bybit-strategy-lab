...
class TradeChartService:
    ...
    def build_trade_chart_payload(
        ...
        *,
        ...
        real_trades = self._build_real_trades(symbol=symbol, live_events=live_events)
    ...
    def _build_real_trades(self, *, symbol: str, live_events: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        rows = _attach_fill_metrics(symbol=symbol, rows=rows)
    ...
    def _build_real_trades_summary(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        wins = [r for r in closed if float((r.get("net_pnl_usd") if r.get("net_pnl_usd") is not None else r.get("pnl_usd")) or 0.0) > 0]
        return {
            ...
            "realized_pnl_usd": round(sum(float((r.get("net_pnl_usd") if r.get("net_pnl_usd") is not None else r.get("pnl_usd")) or 0.0) for r in closed), 8),
...
def _fee_to_usd(fee: float | None, fee_currency: str | None) -> float | None:
    if fee is None:
        return None
    cur = str(fee_currency or "").upper()
    # conservative: directly subtract only USD stable fees
    if cur in {"USDT", "USD", "USDC", ""}:
        return fee
    return fee


def _summarize_fill_set(summary: dict[str, Any] | None) -> tuple[float | None, float | None, float | None, str | None]:
    if not summary:
        return None, None, None, None
    qty = _float_or_none(summary.get("qty"))
    avg_price = _float_or_none(summary.get("avg_price"))
    fee = _float_or_none(summary.get("fee"))
    fee_currency = summary.get("fee_currency")
    return qty, avg_price, fee, fee_currency


def _attach_fill_metrics(symbol: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    try:
        bybit = BybitClient()
    except Exception:
        return rows

    if not getattr(bybit, "ready", False):
        return rows
    if not hasattr(bybit, "find_fills_by_order_link_id") or not hasattr(bybit, "summarize_fills"):
        return rows

    for row in rows:
        entry_code = row.get("entry_code")
        exit_code = row.get("exit_code")
        entry_ts = _millis_from_mixed(row.get("entry_ts"))
        exit_ts = _millis_from_mixed(row.get("exit_ts"))
        ...
        try:
            if entry_code:
                fills = bybit.find_fills_by_order_link_id(
                    symbol=symbol,
                    order_link_id=str(entry_code),
                    limit=100,
                    start_ms=(entry_ts - 6 * 60 * 60 * 1000) if entry_ts else None,
                    end_ms=(entry_ts + 6 * 60 * 60 * 1000) if entry_ts else None,
                )
                entry_summary = bybit.summarize_fills(fills)
        except Exception:
            entry_summary = None
        ...
        try:
            if exit_code:
                fills = bybit.find_fills_by_order_link_id(
                    symbol=symbol,
                    order_link_id=str(exit_code),
                    limit=100,
                    start_ms=(exit_ts - 6 * 60 * 60 * 1000) if exit_ts else None,
                    end_ms=(exit_ts + 6 * 60 * 60 * 1000) if exit_ts else None,
                )
                exit_summary = bybit.summarize_fills(fills)
        except Exception:
            exit_summary = None

        entry_qty, entry_price, entry_fee, entry_fee_ccy = _summarize_fill_set(entry_summary)
        exit_qty, exit_price, exit_fee, exit_fee_ccy = _summarize_fill_set(exit_summary)

        # prefer exchange fills where available
        if entry_price is not None:
            row["entry_price"] = entry_price
            row["entry_price_source"] = "bybit_fills"
        if exit_price is not None:
            row["exit_price"] = exit_price
            row["exit_price_source"] = "bybit_fills"

        qty_existing = _float_or_none(row.get("qty"))
        qty = qty_existing
        if entry_qty is not None and exit_qty is not None:
            qty = min(entry_qty, exit_qty)
        elif entry_qty is not None:
            qty = entry_qty
        elif exit_qty is not None:
            qty = exit_qty
        row["qty"] = qty

        row["entry_fee"] = entry_fee
        row["exit_fee"] = exit_fee
        row["fee_currency"] = entry_fee_ccy or exit_fee_ccy

        gross = _calc_trade_pnl(
        ...
        fee_usd = 0.0
        have_fee = False
        ef = _fee_to_usd(entry_fee, entry_fee_ccy)
        xf = _fee_to_usd(exit_fee, exit_fee_ccy)
        if ef is not None:
            fee_usd += ef
            have_fee = True
        if xf is not None:
        ...
        if gross is not None:
            row["net_pnl_usd"] = round(gross - fee_usd, 8) if have_fee else gross
            row["pnl_usd"] = row["net_pnl_usd"]
        else:
...
def _attach_fill_data_to_real_trades(symbol: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """
    Enrich only the freshest trades with real fill prices/fees from Bybit.
    Older rows keep reconstructed values to keep page fast.
    """
    try:
    ...
    target_rows = rows[:MAX_REAL_TRADES_WITH_FILLS]
    for row in target_rows:
        ...
        if entry_code:
            ck = _fill_cache_key(symbol, entry_code)
            entry_summary = _cache_get(_FILL_SUMMARY_CACHE, ck, FILL_SUMMARY_CACHE_TTL_SEC)
            if entry_summary is None:
                try:
                    fills = client.find_fills_by_order_link_id(symbol=symbol, order_link_id=entry_code, limit=100)
                    entry_summary = client.summarize_fills(fills)
                except Exception:
                    entry_summary = {}
                _cache_set(_FILL_SUMMARY_CACHE, ck, entry_summary)

        if exit_code:
            ck = _fill_cache_key(symbol, exit_code)
            exit_summary = _cache_get(_FILL_SUMMARY_CACHE, ck, FILL_SUMMARY_CACHE_TTL_SEC)
            if exit_summary is None:
                try:
                    fills = client.find_fills_by_order_link_id(symbol=symbol, order_link_id=exit_code, limit=100)
                    exit_summary = client.summarize_fills(fills)
                except Exception:
                    exit_summary = {}
                _cache_set(_FILL_SUMMARY_CACHE, ck, exit_summary)

        ep = _float_or_none((entry_summary or {}).get("avg_price"))
        xp = _float_or_none((exit_summary or {}).get("avg_price"))
        eq = _float_or_none((entry_summary or {}).get("qty"))
        xq = _float_or_none((exit_summary or {}).get("qty"))
        ef = _float_or_none((entry_summary or {}).get("fee"))
        xf = _float_or_none((exit_summary or {}).get("fee"))
        fee_ccy = (entry_summary or {}).get("fee_currency") or (exit_summary or {}).get("fee_currency")

        if ep is not None:
            row["entry_price"] = ep
        if xp is not None:
            row["exit_price"] = xp
        qty = eq if eq is not None else xq
        if qty is not None:
            row["qty"] = qty

        row["entry_fee"] = ef
        row["exit_fee"] = xf
        row["fee_currency"] = fee_ccy
        ...
        gross = None
        if side == "LONG" and entry_price is not None and exit_price is not None and qty is not None:
            gross = (exit_price - entry_price) * qty
        elif side == "SHORT" and entry_price is not None and exit_price is not None and qty is not None:
            gross = (entry_price - exit_price) * qty
        row["gross_pnl_usd"] = gross

        total_fee = (_float_or_none(ef) or 0.0) + (_float_or_none(xf) or 0.0)
        if gross is not None:
            row["net_pnl_usd"] = gross - total_fee
            row["pnl_usd"] = row["net_pnl_usd"]
    return rows


def _attach_deposit_delta(rows: list[dict[str, Any]], snapshots: list[dict[str, Any]]) -> list[dict[str, Any]]:
    snaps = sorted(snapshots or [], key=lambda s: _float_or_none(s.get("ts")) or 0)
    for row in rows:
