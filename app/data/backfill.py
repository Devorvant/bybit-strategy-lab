...
from app.config import settings, REAL_DATA_ENABLED, REAL_DATA_SYMBOLS, REAL_BYBIT_REST_URL, real_storage_symbol
...
async def backfill_symbol_tf(
    ...
    *,
    ...
    storage_sym = real_storage_symbol(symbol) if suffix else symbol
    tf = str(tf).strip()
    end_ms_target = _now_ms()
    start_ms_target = end_ms_target - int(days) * 24 * 60 * 60 * 1000

    min_ts, max_ts = bars_min_max_ts(conn, storage_sym, tf)
    if min_ts is not None and max_ts is not None:
        if min_ts <= start_ms_target and max_ts >= end_ms_target - 60_000:
            print(f"[{tag}] {storage_sym} tf={tf}: already has range, skip")
            return

    print(f"[{tag}] {storage_sym} tf={tf}: start")

    async with httpx.AsyncClient(
        base_url=rest_url or settings.BYBIT_REST_URL,
        timeout=20,
        ...
        while cursor_end > start_ms_target:
            ...
            upsert_bars(conn, storage_sym, tf, rows)
            pages += 1
            total += len(rows)
        ...
        print(f"[{tag}] {storage_sym} tf={tf}: done pages={pages} rows={total}")
...
async def backfill_on_startup(conn):
    ...
    if REAL_DATA_ENABLED and settings.BACKFILL_ON_START:
            ...
            for tf in settings.TFS:
                try:
                    await backfill_symbol_tf(
                        ...
                        days=settings.BACKFILL_DAYS,
                        sleep_ms=settings.BACKFILL_SLEEP_MS,
                        rest_url=REAL_BYBIT_REST_URL,
                        suffix='_R',
                        tag='backfill-real',
                    )
                except Exception as e:
                    print(f"[backfill-real] ERROR {sym} tf={tf}: {e!r}")
