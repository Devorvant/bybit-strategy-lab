...
def backtest_strategy3(
    ...
    def _slip(px: float, action: str) -> float:
        """Apply simple slippage in ticks to a fill price.
        action: 'BUY' or 'SELL'
        """
        try:
            st = int(slippage_ticks)
        except Exception:
            st = 0
        try:
            tsz = float(tick_size)
            ...
            tsz = 0.0
        if st <= 0 or tsz <= 0 or px <= 0:
            return px
        adj = st * tsz
        if action.upper() == "BUY":
            return px + adj
        if action.upper() == "SELL":
            return max(0.0, px - adj)
        return px
    ...
    for i in range(len(bars)):
        ...
        if use_emergency_sl and pos != 0 and atr_v[i] is not None:
            ...
            if pos == 1:
                ...
                if l[i] <= stop_px:
                    fill_px = _slip(stop_px, 'SELL' if pos == 1 else 'BUY')
                    pnl = entry_size_usd * pos * (fill_px - entry_price) / entry_price
                    realized += pnl
                    cum += pnl
                    trades.append(
                        Trade("LONG", entry_ts, entry_price, ts[i], fill_px, pnl, "STOP_LONG", cum)
                    )
                    pos = 0
                ...
                if h[i] >= stop_px:
                    ...
                    cum += pnl
                    trades.append(
                        Trade("SHORT", entry_ts, entry_price, ts[i], fill_px, pnl, "STOP_SHORT", cum)
                    )
                    pos = 0
                    equity[-1] = realized
                    continue
        ...
        if no_trade[i]:
            if pos != 0:
                px = c[i]
                fill_px = _slip(px, 'SELL' if pos == 1 else 'BUY')
                ...
                cum += pnl
                trades.append(
                    Trade("LONG" if pos == 1 else "SHORT", entry_ts, entry_price, ts[i], fill_px, pnl, "NO_TRADE", cum)
                )
                pos = 0
                equity[-1] = realized
            continue
        ...
        if flip_to_long and pos == -1:
            if entry_price <= 0:
                ...
                fill_px = _slip(px, 'BUY')
                pnl = entry_size_usd * pos * (fill_px - entry_price) / entry_price
                realized += pnl
                cum += pnl
                trades.append(Trade("SHORT", entry_ts, entry_price, ts[i], fill_px, pnl, "ST_FLIP", cum))
                pos = 0
                equity[-1] = realized
        ...
        if flip_to_short and pos == 1:
            if entry_price <= 0:
                ...
                fill_px = _slip(px, 'SELL')
                pnl = entry_size_usd * pos * (fill_px - entry_price) / entry_price
                realized += pnl
                cum += pnl
                trades.append(Trade("LONG", entry_ts, entry_price, ts[i], fill_px, pnl, "ST_FLIP", cum))
                pos = 0
                equity[-1] = realized
        ...
        if pos == 0:
            ...
            entry_ts = ts[i]
            entry_price = _slip(px, 'BUY' if pos == 1 else 'SELL')
            last_flip_time = ts[i]
            flips_today += 1
    ...
    if pos != 0:
        ...
        if close_at_end:
            ...
            fill_px = _slip(cur_px, 'SELL' if pos == 1 else 'BUY')
            pnl = entry_size_usd * pos * (fill_px - entry_price) / entry_price
            realized += pnl
            cum += pnl
            trades.append(
