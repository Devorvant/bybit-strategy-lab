...
def _sample_params(
    rng,
    position_usd: float,
    other_fixed: Dict[str, Any],
    other_opt: Dict[str, bool],
) -> Dict[str, Any]:
    """Sample a candidate parameter set.

    "Прочее" параметры можно либо фиксировать (other_fixed),
    либо включать в оптимизацию (other_opt[key] == True).

    Диапазоны подобраны умеренно, чтобы не взорвать поиск.
    """

    def _fx_int(key: str, default: int) -> int:
        try:
            v = other_fixed.get(key, default)
            return int(v)
        except Exception:
            return int(default)

    def _fx_bool(key: str, default: bool) -> bool:
        v = other_fixed.get(key, default)
        return bool(v)

    # Other / core indicator settings
    adx_len = rng.randint(7, 40) if other_opt.get("adx_len") else _fx_int("adx_len", 14)
    adx_smooth = rng.randint(7, 40) if other_opt.get("adx_smooth") else _fx_int("adx_smooth", 14)
    st_atr_len = rng.randint(7, 60) if other_opt.get("st_atr_len") else _fx_int("st_atr_len", 14)
    atr_len = rng.randint(7, 80) if other_opt.get("atr_len") else _fx_int("atr_len", 14)

    use_rev_cooldown = rng.choice([True, False]) if other_opt.get("use_rev_cooldown") else _fx_bool("use_rev_cooldown", True)
    use_emergency_sl = rng.choice([True, False]) if other_opt.get("use_emergency_sl") else _fx_bool("use_emergency_sl", True)
    close_at_end = rng.choice([True, False]) if other_opt.get("close_at_end") else _fx_bool("close_at_end", False)

    use_flip_limit = rng.choice([False, True])

    # cooldown hours: only meaningful if use_rev_cooldown
    if use_rev_cooldown:
...
def _run_optimizer_sync(run: RunState, bars: List[Tuple[int, float, float, float, float, float]], conn) -> None:
    ...
    other_fixed = cfg.get("other_fixed") or {}
    other_opt = cfg.get("other_opt") or {}
    ...
    _add_log(
        ...
        f" other_fixed={json.dumps(other_fixed, ensure_ascii=True)} other_opt={json.dumps(other_opt, ensure_ascii=True)} "
    ...
    try:
        if optimizer == "optuna_tpe":
            ...
            def objective(trial):
                # Other params: either fixed (from UI) or optimized if opt_* checkbox is enabled
                def _fx_int(key: str, default: int) -> int:
                    try:
                        v = other_fixed.get(key, default)
                        return int(v)
                ...
                def _fx_bool(key: str, default: bool) -> bool:
                    return bool(other_fixed.get(key, default))

                adx_len = int(trial.suggest_int('adx_len', 7, 40)) if other_opt.get('adx_len') else _fx_int('adx_len', 14)
                adx_smooth = int(trial.suggest_int('adx_smooth', 7, 40)) if other_opt.get('adx_smooth') else _fx_int('adx_smooth', 14)
                st_atr_len = int(trial.suggest_int('st_atr_len', 7, 60)) if other_opt.get('st_atr_len') else _fx_int('st_atr_len', 14)
                atr_len = int(trial.suggest_int('atr_len', 7, 80)) if other_opt.get('atr_len') else _fx_int('atr_len', 14)

                use_rev_cooldown = bool(trial.suggest_categorical('use_rev_cooldown', [True, False])) if other_opt.get('use_rev_cooldown') else _fx_bool('use_rev_cooldown', True)
                use_emergency_sl = bool(trial.suggest_categorical('use_emergency_sl', [True, False])) if other_opt.get('use_emergency_sl') else _fx_bool('use_emergency_sl', True)
                close_at_end = bool(trial.suggest_categorical('close_at_end', [True, False])) if other_opt.get('close_at_end') else _fx_bool('close_at_end', False)

                use_flip_limit = trial.suggest_categorical('use_flip_limit', [False, True])

                # cooldown hours meaningful only if enabled
                if use_rev_cooldown:
            ...
            for si, seed_i in enumerate(range(seed_from, seed_to + 1), start=1):
                ...
                for t in range(1, trials_per_seed + 1):
                    ...
                    params = _sample_params(rng_i, position_usd, other_fixed, other_opt)
                    params_eval = dict(params)
                    params_eval["confirm_on_close"] = bool(params.get("confirm_on_close", False))
                    if use_costs_in_opt:
                        params_eval.update(fixed_costs)
            ...
            for trial in range(1, trials + 1):
                ...
                params = _sample_params(rng, position_usd, other_fixed, other_opt)
...
Если выключено: используем текущую свечу (может перерисовываться на live).'>?</span></label>
        ...
        <div style='margin-top:14px; font-weight:700;'>Прочее (фикс / опционально оптимизируемое)</div>
        <div class='muted' style='margin-bottom:8px; line-height:1.35;'>
          Эти параметры можно задать руками и зафиксировать на время оптимизации. Если включить галочку «opt» под параметром —
          он будет участвовать в оптимизации как обычный коэффициент.
        </div>
        <div class='row' style='flex-wrap:wrap; gap:14px;'>
          <div>
            <label>adx_len</label>
            <input name='fx_adx_len' value='14' style='width:110px'/>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_adx_len' id='opt_adx_len'/>
              <label for='opt_adx_len' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>
          <div>
            <label>adx_smooth</label>
            <input name='fx_adx_smooth' value='14' style='width:110px'/>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_adx_smooth' id='opt_adx_smooth'/>
              <label for='opt_adx_smooth' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>
          <div>
            <label>st_atr_len</label>
            <input name='fx_st_atr_len' value='14' style='width:110px'/>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_st_atr_len' id='opt_st_atr_len'/>
              <label for='opt_st_atr_len' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>
          <div>
            <label>atr_len</label>
            <input name='fx_atr_len' value='14' style='width:110px'/>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_atr_len' id='opt_atr_len'/>
              <label for='opt_atr_len' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>

          <div>
            <label>use_rev_cooldown</label>
            <select name='fx_use_rev_cooldown' style='width:110px'>
              <option value='1' selected>True</option>
              <option value='0'>False</option>
            </select>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_use_rev_cooldown' id='opt_use_rev_cooldown'/>
              <label for='opt_use_rev_cooldown' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>

          <div>
            <label>use_emergency_sl</label>
            <select name='fx_use_emergency_sl' style='width:110px'>
              <option value='1' selected>True</option>
              <option value='0'>False</option>
            </select>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_use_emergency_sl' id='opt_use_emergency_sl'/>
              <label for='opt_use_emergency_sl' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>

          <div>
            <label>close_at_end</label>
            <select name='fx_close_at_end' style='width:110px'>
              <option value='0' selected>False</option>
              <option value='1'>True</option>
            </select>
            <div style='display:flex; gap:8px; align-items:center; margin-top:4px;'>
              <input type='checkbox' name='opt_close_at_end' id='opt_close_at_end'/>
              <label for='opt_close_at_end' style='margin:0; font-size:12px;'>opt</label>
            </div>
          </div>
        </div>
...
def _optimize_run_html(run_id: str) -> str:
    ...
    if isinstance(robust, dict) and robust.get("windows"):
        ...
        robust_table = (
            ...
            "<th>val_ret</th><th>val_dd</th><th>val_sharpe</th><th>val_trades</th>"
            "</tr></thead>"
    ...
    if cand and isinstance(cand, dict):
        ...
        if isinstance(top, list) and top:
            ...
            for i, r in enumerate(top):
                ...
                link = f"/chart?symbol={html.escape(str(cand.get('symbol','')))}&tf={html.escape(str(cand.get('tf','')))}&limit={int(cand.get('limit_bars', 5000) or 5000)}&strategy=my_strategy3.py&end_shift_hours={float((cfg or {}).get('end_shift_hours',0) or 0)}&opt_run_id={html.escape(run_id)}&opt_cand={i}"
                rows.append(
                    "<tr>"
                    f"<td>{i+1}</td>"
                    f"<td>{seed_i}</td>"
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    cfg = {
        ...
        # "Прочее" параметры: фиксируем (fx_*) или делаем оптимизируемыми (opt_*)
        "other_fixed": {
            "adx_len": _i("fx_adx_len", 14),
            "adx_smooth": _i("fx_adx_smooth", 14),
            "st_atr_len": _i("fx_st_atr_len", 14),
            "atr_len": _i("fx_atr_len", 14),
            "use_rev_cooldown": _b("fx_use_rev_cooldown"),
            "use_emergency_sl": _b("fx_use_emergency_sl"),
            "close_at_end": _b("fx_close_at_end"),
        },
        "other_opt": {
            "adx_len": _b("opt_adx_len"),
            "adx_smooth": _b("opt_adx_smooth"),
            "st_atr_len": _b("opt_st_atr_len"),
            "atr_len": _b("opt_atr_len"),
            "use_rev_cooldown": _b("opt_use_rev_cooldown"),
            "use_emergency_sl": _b("opt_use_emergency_sl"),
            "close_at_end": _b("opt_close_at_end"),
        },
        "seed": seed,
        "seed_from": seed_from,
        "seed_to": seed_to,
