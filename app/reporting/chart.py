...
def make_chart_html(
    ...
    *,
    ...
    # If 1, apply p_* query parameters as manual overrides
    use_overrides: int = 0,
) -> str:
...
def _render_value_cell(k: str, v) -> str:
    ...
    # Manual overrides use p_* query params and are only applied when use_overrides=1.
    name = f"p_{k}"
    js_mark = "document.getElementById('use_overrides').value='1'"
    ...
    return (
        "<div class='p-v'>"
        f"<input class='p-in' form='chartForm' name='{html.escape(name)}' type='number' step='{step}' "
        f"value='{html.escape(value)}' oninput=\"{js_mark}\"/>"
        "</div>"
    )

    params_items = "".join(
        f"<div class='p-k'>{html.escape(str(k))}</div>{_render_value_cell(str(k), v)}<div class='p-d'>{html.escape(PARAM_HELP.get(str(k), ''))}</div>"
