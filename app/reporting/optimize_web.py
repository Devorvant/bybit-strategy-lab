    ...
    try:
        ...
        if bool(cfg.get("robust_check")) and run.best_params:
            try:
                ...
                # For robust check we may need more history than the optimization window.
                # IMPORTANT: do NOT expand the optimization bars; load a separate buffer for robustness only.
                tf_sec = _tf_seconds(tf)
                shift_hours_eff = rshift
                if rfirst > 0.0 and rw > 1:
                    shift_hours_eff = float(rfirst) / float(rw - 1)
                shift_bars = int(round((shift_hours_eff * 3600.0) / float(tf_sec))) if tf_sec > 0 else 0
                if rw > 1:
                    shift_bars = max(1, shift_bars)
                else:
                    shift_bars = 0
                required_bars = int(rbars + shift_bars * (rw - 1))
                required_bars = max(required_bars, rbars)

                bars_robust = bars
                if required_bars > len(bars):
                    try:
                        bars_robust = load_bars(conn, symbol, tf, limit=required_bars)
                    except Exception as e:
                        _add_log(run, f"ROBUST_LOAD_ERR {type(e).__name__}: {e}")
                        bars_robust = bars
                _add_log(
                    run,
                    f"ROBUST_LOAD opt_bars={len(bars)} robust_bars={len(bars_robust)} required={required_bars} shift_bars={shift_bars} shift_hours_eff={shift_hours_eff}",
                )
