...
from .trade_key import make_strategy_trade_key
...
def _attach_strategy_match_id(rows: list[dict[str, Any]], *, symbol: str, tf: str, opt_id: Any) -> list[dict[str, Any]]:
    for row in rows or []:
        ...
        if not entry_ts_i:
            row["match_id"] = None
            continue
        row["match_id"] = make_strategy_trade_key(symbol, tf, opt_id, entry_ts_i)
    return rows
...
class TradeChartService:
    ...
    def build_trade_chart_payload(
        ...
        *,
        ...
        compare_rows = _build_compare_rows_auto_by_code(strategy_trades=strategy_trades, real_trades=real_trades)
