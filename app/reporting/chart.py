...
def make_chart_html(
    ...
    *,
    ...
    <div class="manual-trade-box" style="margin:12px 0;padding:12px;border:1px solid #2a3b5f;border-radius:10px;background:#0b1220;">
      <div style="font-weight:700;margin-bottom:8px;color:#dbeafe;">Manual Trade (Bybit Bridge)</div>

      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <input id="mt_token" type="password" placeholder="Bridge token"
...
<head>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", () => {{
      ...
      // Manual trade (Bybit bridge) buttons
      let mtSending = false;
      function mtSetButtonsDisabled(disabled) {{
        ['btnLong', 'btnShort', 'btnClose'].forEach((id) => {{
      ...
      async function mtSendBridgeAction(action) {{
        if (mtSending) return;
        const out = document.getElementById('mt_result');
        const token = (document.getElementById('mt_token')?.value || '').trim();
        const tfv = (document.getElementById('mt_tf')?.value || '2h').trim();
        const note = (document.getElementById('mt_note')?.value || 'manual from terminal').trim();
        const symbolBase = mtGetCurrentSymbolBase();
        const symbolBridge = symbolBase.endsWith('.P') ? symbolBase : (symbolBase + '.P');
        if (!token) {{ if (out) out.textContent = 'Укажите bridge token'; return; }}
        if (!['LONG', 'SHORT', 'CLOSE'].includes(action)) {{ if (out) out.textContent = 'Unsupported action: ' + action; return; }}
        if (!confirm('Отправить ' + action + ' на ' + symbolBridge + ' (tf=' + tfv + ')?')) return;
        const payload = {{ token: token, symbol: symbolBridge, tf: tfv, action: action, note: note }};
        mtSending = true;
        mtSetButtonsDisabled(true);
        if (out) out.textContent = '[' + new Date().toLocaleString() + '] Sending ' + action + '...\n' + JSON.stringify(payload, null, 2);
        try {{
          const resp = await fetch('https://tv-railway-bridge-production.up.railway.app/webhook', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify(payload)
          }});
          ...
          if (out) out.textContent = '[' + new Date().toLocaleString() + '] ' + action + ' ' + symbolBridge + '\n' + JSON.stringify({{ http_status: resp.status, response: parsed }}, null, 2);
        }} catch (e) {{
      ...
      document.getElementById('btnLong')?.addEventListener('click', () => mtSendBridgeAction('LONG'));
      document.getElementById('btnShort')?.addEventListener('click', () => mtSendBridgeAction('SHORT'));
      document.getElementById('btnClose')?.addEventListener('click', () => mtSendBridgeAction('CLOSE'));
