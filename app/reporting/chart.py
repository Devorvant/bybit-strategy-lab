...
def make_chart_html(
    ...
    *,
    ...
    symbol_options = "\n".join(
        f"<option value='{html.escape(s)}' {'selected' if s == symbol else ''}>{html.escape(s + (' [REAL DATA]' if str(s).endswith('_R') else ''))}</option>"
...
<body>
  <form id="chartForm" class="topbar" method="get" action="/chart">
    ...
    <label>Symbol</label>
    <select name="symbol">{symbol_options}</select>\n    <span style="margin-left:8px;padding:4px 8px;border-radius:999px;font-size:12px;font-weight:700;background:{'#fff4cc' if str(symbol).endswith('_R') else '#eef2ff'};color:{'#8a5a00' if str(symbol).endswith('_R') else '#334155'};">{'REAL DATA' if str(symbol).endswith('_R') else 'TEST DATA'}</span>
