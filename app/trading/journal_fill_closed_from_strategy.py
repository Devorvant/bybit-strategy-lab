...
def get_strategy_trade_by_key(conn, trade_key: str) -> dict | None:
    if not trade_key:
        return None
...
def upsert_strategy_trade_from_strategy(conn, row: dict) -> None:
    """Insert if missing. If strategy already has close fields, backfill the closing part
    when journal still has NULL/open close fields."""
    trade_key = str(row.get("trade_key") or "").strip()
    if not trade_key:
        return

    existing = get_strategy_trade_by_key(conn, trade_key)
    if not existing:
        upsert_strategy_trade_open(conn, row)
    ...
    need_close_sync = (
        row.get("exit_ts") is not None and (
            existing.get("exit_ts") is None
            or existing.get("exit_price") is None
            or existing.get("close_reason") is None
            or str(existing.get("status") or "").lower() == "open"
        )
    )
    if need_close_sync:
        update_strategy_trade_close(
