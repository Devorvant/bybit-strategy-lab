...
class TradeChartService:
    ...
    def build_trade_chart_payload(
        ...
        *,
        ...
        return {
            ...
            "debug": {
                "bt_trade_0_dict": getattr((getattr(bt, "trades", []) or [None])[0], "__dict__", {}) if (getattr(bt, "trades", []) or []) else {},
                "bt_trade_0_dir": dir((getattr(bt, "trades", []) or [None])[0]) if (getattr(bt, "trades", []) or []) else [],
                "bt_open_position_dict": getattr(getattr(bt, "open_position", None), "__dict__", {}) if getattr(bt, "open_position", None) else {},
                "bt_open_position_dir": dir(getattr(bt, "open_position", None)) if getattr(bt, "open_position", None) else [],
            },
            "discrepancies": {
                "available": has_live,
                "rows": discrepancies,
    ...
    def _build_strategy_trades(self, bt: Any) -> list[dict[str, Any]]:
            ...
            rows.append(
                {
                    ...
                    "trade_tp": _float_or_none(getattr(tr, "trade_tp", None)),
                    "trade_sl": _float_or_none(getattr(tr, "trade_sl", None)),
        ...
        op = getattr(bt, "open_position", None)
        if op is not None:
            try:
                entry_ts = int(getattr(op, "entry_ts", 0) or 0)
            except Exception:
                entry_ts = 0
            ...
            rows.insert(
                0,
                {
                    "trade_id": len(rows) + 1,
                    "status": "open",
                    "side": str(getattr(op, "side", "")).upper() or None,
                    "reason": "ОТКРЫТА",
                    "entry_ts": entry_ts,
                    "exit_ts": 0,
                    "entry_price": _float_or_none(getattr(op, "entry_price", None)),
                    "exit_price": None,
                    "trade_tp": _float_or_none(getattr(op, "trade_tp", None)),
                    "trade_sl": _float_or_none(getattr(op, "trade_sl", None)),
                    "model_size_usd": 1000.0,
                    "pnl_usd": _float_or_none(getattr(op, "pnl", None)),
                    "cum_pnl_usd": rows[0].get("cum_pnl_usd") if rows else 0.0,
                }
            )
