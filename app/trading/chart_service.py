...
from .journal import fetch_strategy_trade_journal, sync_existing_strategy_trade_from_strategy_row
...
def _attach_strategy_match_id(rows: list[dict[str, Any]], *, symbol: str, tf: str, opt_id: Any) -> list[dict[str, Any]]:
    ...
    for row in rows or []:
        ...
        try:
            entry_ts_i = int(entry_ts) if entry_ts is not None else None
        except Exception:
            entry_ts_i = None
        if not entry_ts_i:
            row["match_id"] = None
            continue
        row["match_id"] = f"{sym}:{tfv}:{optv}:{entry_ts_i}"
    return rows


def _compare_row_ts(row: dict[str, Any]) -> int:
    """Return the timestamp used to show the newest compare rows first."""
    vals = (
        row.get("actual_exit_ts"),
        row.get("strategy_exit_ts"),
        row.get("actual_entry_ts"),
        row.get("strategy_entry_ts"),
        row.get("exit_ts"),
        row.get("entry_ts"),
        row.get("ts"),
    )
...
def _sort_compare_rows_latest_first(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    for row in rows or []:
        row["comparison_ts"] = _compare_row_ts(row)
    rows.sort(key=lambda r: int(r.get("comparison_ts") or 0), reverse=True)
    return rows


def _build_compare_rows_auto_by_code(
    *,
    ...
    for r in auto_real:
        key = str(r.get("entry_code") or "").strip()
        if key:
            real_map[key] = r

    rows: list[dict[str, Any]] = []
    for tr in (strategy_trades or []):
        key = str(tr.get("match_id") or "").strip()
        real = real_map.get(key)

        status = "matched" if real else "missed_execution"
        actual_result = real.get("close_reason") if real else None
        if real and str(actual_result or "") == "exchange_external_close":
            status = "external_close"

        strategy_pnl = _float_or_none(tr.get("pnl_usd"))
        real_pnl = _float_or_none((real or {}).get("net_pnl_usd"))
        delta_pnl = (
            real_pnl - strategy_pnl
            if (real_pnl is not None and strategy_pnl is not None)
            else None
        )

        rows.append({
            "id": f"cmp_{tr.get('trade_id')}",
            "strategy_trade_id": tr.get("trade_id"),
            "strategy_side": tr.get("side"),
            "strategy_entry_ts": tr.get("entry_ts"),
            "strategy_entry_price": tr.get("entry_price"),
            "strategy_exit_ts": tr.get("exit_ts"),
            "strategy_exit_price": tr.get("exit_price"),
            "actual_entry_ts": (real or {}).get("entry_ts"),
            "actual_entry_price": (real or {}).get("entry_price"),
            "actual_exit_ts": (real or {}).get("exit_ts"),
            "actual_exit_price": (real or {}).get("exit_price"),
            "expected_result": tr.get("reason"),
            "actual_result": actual_result,
            "match_status": status,
            "comment": "matched by entry_code" if real else "no auto real trade with same code",
            "strategy_pnl_usd": strategy_pnl,
            "real_net_pnl_usd": real_pnl,
            "delta_pnl_usd": delta_pnl,
            "match_id": key,
            "real_entry_code": (real or {}).get("entry_code"),
            "real_exit_code": (real or {}).get("exit_code"),
        })
    return _sort_compare_rows_latest_first(rows)





def _build_compare_rows_from_strategy_journal(
    *,
    ...
    rows: list[dict[str, Any]] = []
    for idx, tr in enumerate(journal_rows or [], start=1):
        key = str(tr.get("trade_key") or "").strip()
        real = real_map.get(key)

        status = "matched" if real else "missed_execution"
        actual_result = (real or {}).get("close_reason")
        if real and str(actual_result or "") == "exchange_external_close":
            status = "external_close"

        strategy_pnl = None
        sp = _float_or_none(tr.get("entry_price"))
        sx = _float_or_none(tr.get("exit_price"))
        side = str(tr.get("side") or "").upper()
        if sp is not None and sx is not None:
            qty = _float_or_none((real or {}).get("qty")) or 1.0
            strategy_pnl = _calc_trade_pnl(side, sp, sx, qty)

        real_pnl = _float_or_none((real or {}).get("net_pnl_usd"))
        delta_pnl = real_pnl - strategy_pnl if (real_pnl is not None and strategy_pnl is not None) else None

        rows.append({
            "id": f"cmp_j_{idx}",
            "strategy_trade_id": key,
            "strategy_side": tr.get("side"),
            "strategy_entry_ts": tr.get("entry_ts"),
            "strategy_entry_price": tr.get("entry_price"),
            "strategy_exit_ts": tr.get("exit_ts"),
            ...
            "actual_exit_ts": (real or {}).get("exit_ts"),
            "actual_exit_price": (real or {}).get("exit_price"),
            "expected_result": tr.get("close_reason"),
            "actual_result": actual_result,
            "match_status": status,
            "comment": "matched by trade_key" if real else "no auto/mixed real trade with same trade_key",
            "strategy_pnl_usd": strategy_pnl,
            "real_net_pnl_usd": real_pnl,
            "delta_pnl_usd": delta_pnl,
            "match_id": key,
...
def _sync_strategy_trade_journal_with_strategy(
    *,
    conn: Any,
    symbol: str,
    tf: str,
    ...
    """
    Sync only already executed journal rows from current strategy state.
    Matching is by immutable trade_key/match_id.
    Identity fields are never rewritten.
    """
    if not conn or not journal_rows or not strategy_trades:
        return 0
    ...
    for tr in strategy_trades or []:
        key = str(tr.get("match_id") or "").strip()
        if key:
            strategy_by_key[key] = tr

    updated = 0
    for jr in journal_rows or []:
        key = str(jr.get("trade_key") or "").strip()
        if not key:
            continue
        st = strategy_by_key.get(key)
        if not st:
            continue
        try:
            ok = sync_existing_strategy_trade_from_strategy_row(
                conn,
                trade_key=key,
                symbol=symbol,
                tf=tf,
...
def _attach_strategy_sl_from_journal(
    rows: list[dict[str, Any]],
    journal_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    if not rows or not journal_rows:
        return rows
    ...
    for row in rows or []:
        ...
        if not key:
            continue
        jr = journal_map.get(key)
        if not jr:
            continue

        if row.get("trade_sl") in (None, ""):
            row["trade_sl"] = _float_or_none(jr.get("sl_price"))
        if row.get("trade_tp") in (None, ""):
            row["trade_tp"] = _float_or_none(jr.get("tp_price"))
        if row.get("model_size_usd") in (None, ""):
            row["model_size_usd"] = 1000.0
...
class TradeChartService:
    ...
    def build_trade_chart_payload(
        ...
        *,
        ...
        strategy_trades = _attach_strategy_match_id(strategy_trades, symbol=symbol, tf=tf, opt_id=opt_id)
        ...
        real_trades = _attach_fill_summary(real_trades, self.repo)
        real_trades = _attach_trade_tp_sl(real_trades, snapshots)
        real_trades = _attach_deposit_delta(real_trades, snapshots)
        ...
        journal_rows = []
        try:
            if self.repo.table_exists("strategy_trade_journal"):
                journal_rows = fetch_strategy_trade_journal(
                    self.repo.conn,
                    symbol=symbol,
                    tf=tf,
                ...
                strategy_trades = _attach_strategy_sl_from_journal(strategy_trades, journal_rows)
                _sync_strategy_trade_journal_with_strategy(
                    conn=self.repo.conn,
                    symbol=symbol,
                    tf=tf,
                    strategy_name='my_strategy3',
                    opt_id=opt_id,
        ...
        compare_rows = _build_compare_rows_from_strategy_journal(journal_rows=journal_rows, real_trades=real_trades) if journal_rows else _build_compare_rows_auto_by_code(strategy_trades=strategy_trades, real_trades=real_trades)
    ...
    def _build_compare_summary(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Alias for the frontend header: table rows use missed_execution,
        # but the header displays it as Missing real.
        summary["missing_real"] = summary.get("missed_execution", 0)
        return summary
...
def rebuild_strategy_trade_journal(
    *,
    symbol: str,
    tf: str,
    strategy: str,
    ...
    """
    Refresh strategy state safely:
    - do NOT delete/replace strategy_trade_journal slice
    - only update rows that already exist in journal and have the same immutable trade_key
    This preserves historical executed keys used by real/bybit comparison.
    """
    svc = TradeChartService()
    symbol = str(symbol or "").upper().strip()
    tf = str(tf or "").strip()
    strategy = str(strategy or "my_strategy3.py").strip()
    limit = max(50, min(int(limit or 2000), 50000))
    last_trades = max(1, min(int(last_trades or 30), 200))

    if not svc.repo.table_exists("strategy_trade_journal"):
        return {"ok": False, "error": "strategy_trade_journal table not found"}

    bars = svc.repo.fetch_bars(symbol=symbol, tf=tf, limit=limit)
    if not bars:
        return {"ok": False, "error": "no bars for selected symbol/tf"}

    strategy_params = svc.repo.fetch_opt_params(symbol=symbol, tf=tf, strategy=strategy, opt_id=opt_id)
    fig, bt, err_html = _build_fig_and_bt(
        bars,
        strategy=strategy,
        strategy_params=strategy_params,
    ...
    if err_html or bt is None:
        return {"ok": False, "error": "strategy backtest/chart build failed", "error_html": err_html}

    strategy_trades = svc._build_strategy_trades(bt)
    strategy_trades = _attach_strategy_match_id(strategy_trades, symbol=symbol, tf=tf, opt_id=opt_id)
    ...
    journal_rows = fetch_strategy_trade_journal(
        ...
        strategy="my_strategy3",
        opt_id=opt_id,
        limit=max(500, last_trades),
    )

    updated = _sync_strategy_trade_journal_with_strategy(
        conn=svc.repo.conn,
        symbol=symbol,
        tf=tf,
        strategy_name="my_strategy3",
        opt_id=opt_id,
        strategy_trades=list(strategy_trades or [])[:last_trades],
        journal_rows=journal_rows,
    )
    ...
    return {
        ...
        "message": "strategy journal state refreshed without rewriting executed keys",
    }
...
def _execution_price(row: dict[str, Any]) -> float | None:
    ...
    for candidate in (
        ...
        response.get("lastPrice"),
        response.get("order_price"),
        response.get("orderPrice"),
        response.get("entry_price"),
        response.get("entryPrice"),
        response.get("close_price"),
        response.get("closePrice"),
    ):
        p = _float_or_none(candidate)
        if p is not None and p > 0:
            return p
    for key in ("result", "data", "order", "execution", "fill"):
...
def _nearest_snapshot_before(ts: Any, snapshots: list[dict[str, Any]]) -> dict[str, Any] | None:
    t = _millis_from_mixed(ts)
    if t is None:
        return None
    best = None
    best_ts = None
    for s in snapshots or []:
        sts = _millis_from_mixed(s.get("ts"))
        if sts is None or sts > t:
            continue
...
def _nearest_snapshot_after(ts: Any, snapshots: list[dict[str, Any]]) -> dict[str, Any] | None:
    t = _millis_from_mixed(ts)
    if t is None:
        return None
    best = None
...
def _snapshot_wallet_balance(s: dict[str, Any] | None) -> float | None:
    if not s:
        return None
    for key in ("wallet_balance", "walletBalance"):
...
def _snapshot_equity(s: dict[str, Any] | None) -> float | None:
    if not s:
        return None
    for key in ("wallet_equity", "equity"):
        v = _float_or_none(s.get(key))
        if v is not None:
            return v
    wb = _snapshot_wallet_balance(s)
    upnl = _float_or_none(s.get("unrealised_pnl") or s.get("unrealisedPnl"))
    if wb is not None and upnl is not None:
        return wb + upnl
    return None



def _fetch_fill_summary_map(repo: TradeChartRepo, order_link_ids: list[str]) -> dict[str, dict[str, Any]]:
    ids = [str(x).strip() for x in (order_link_ids or []) if str(x or "").strip()]
    if not ids:
        return {}
    ...
    if repo.is_sqlite:
        ...
        cur = repo.conn.execute(
            f"""
            SELECT order_link_id, symbol, tf, fill_count, sum_exec_qty, avg_exec_price, sum_exec_fee,
                   fee_currency, first_exec_time, last_exec_time, updated_ts
            FROM execution_fill_summary
            WHERE order_link_id IN ({placeholders})
            """,
            ids,
        )
        rows = cur.fetchall()
    ...
    for row in rows:
        ...
        key = str(item.get("order_link_id") or "").strip()
        if key:
            out[key] = item
    return out


def _attach_fill_summary(rows: list[dict[str, Any]], repo: TradeChartRepo) -> list[dict[str, Any]]:
    if not rows or not repo.table_exists("execution_fill_summary"):
        return rows

    codes: list[str] = []
    for row in rows:
        for key in ("entry_code", "exit_code"):
            val = str(row.get(key) or "").strip()
            if val:
                codes.append(val)

    summary_map = _fetch_fill_summary_map(repo, codes)
    if not summary_map:
        return rows
    ...
    for row in ordered:
        entry_summary = summary_map.get(str(row.get("entry_code") or "").strip() or "")
        exit_summary = summary_map.get(str(row.get("exit_code") or "").strip() or "")

        if entry_summary:
            ep = _float_or_none(entry_summary.get("avg_exec_price"))
            eq = _float_or_none(entry_summary.get("sum_exec_qty"))
            ef = _float_or_none(entry_summary.get("sum_exec_fee"))
            if ep is not None:
                row["entry_price"] = ep
            row["entry_fee"] = ef
            row["entry_fee_currency"] = entry_summary.get("fee_currency")
            row["entry_fill_count"] = _int_or_none(entry_summary.get("fill_count"))
            row["entry_exec_time"] = _millis_from_mixed(entry_summary.get("last_exec_time"))
        else:
            row["entry_fee"] = row.get("entry_fee")

        if exit_summary:
            xp = _float_or_none(exit_summary.get("avg_exec_price"))
            xq = _float_or_none(exit_summary.get("sum_exec_qty"))
            xf = _float_or_none(exit_summary.get("sum_exec_fee"))
            if xp is not None:
                row["exit_price"] = xp
            row["exit_fee"] = xf
            row["exit_fee_currency"] = exit_summary.get("fee_currency")
            row["exit_fill_count"] = _int_or_none(exit_summary.get("fill_count"))
            row["exit_exec_time"] = _millis_from_mixed(exit_summary.get("last_exec_time"))
        else:
            row["exit_fee"] = row.get("exit_fee")

        # qty: prefer exit qty, then entry qty, then existing
        qty = (
            _float_or_none((exit_summary or {}).get("sum_exec_qty"))
            or _float_or_none((entry_summary or {}).get("sum_exec_qty"))
            or _float_or_none(row.get("qty"))
        )
        row["qty"] = qty
        ...
        gross = _calc_trade_pnl(side, entry_price, exit_price, qty)
        entry_fee = _float_or_none(row.get("entry_fee")) or 0.0
        exit_fee = _float_or_none(row.get("exit_fee")) or 0.0
        net = round(gross - entry_fee - exit_fee, 8) if gross is not None else None

        row["gross_pnl_usd"] = gross
        row["net_pnl_usd"] = net
        if net is not None:
            row["pnl_usd"] = net
            cumulative = round(cumulative + net, 8)
            row["cum_pnl_usd"] = cumulative
        else:
            row["cum_pnl_usd"] = cumulative if ordered.index(row) > 0 else None

        row["fee_currency"] = (
            row.get("entry_fee_currency")
            or row.get("exit_fee_currency")
...
def _attach_trade_tp_sl(rows: list[dict[str, Any]], snapshots: list[dict[str, Any]]) -> list[dict[str, Any]]:
    snaps = sorted(snapshots or [], key=lambda s: _millis_from_mixed(s.get("ts")) or 0)
    if not rows or not snaps:
        return rows

    for row in rows:
        entry_ts = _float_or_none(row.get("entry_ts"))
        exit_ts = _float_or_none(row.get("exit_ts"))

        # Prefer first snapshot after entry, because manual_after / auto_after usually lands there.
        entry_snap = _nearest_snapshot_after(entry_ts, snaps) if entry_ts is not None else None
        if entry_snap is None and entry_ts is not None:
            entry_snap = _nearest_snapshot_before(entry_ts, snaps)

        # Also keep last known tp/sl near exit as diagnostic info.
        exit_snap = _nearest_snapshot_before(exit_ts, snaps) if exit_ts is not None else None
        if exit_snap is None and exit_ts is not None:
            exit_snap = _nearest_snapshot_after(exit_ts, snaps)

        tp_entry = _float_or_none((entry_snap or {}).get("take_profit"))
        sl_entry = _float_or_none((entry_snap or {}).get("stop_loss"))
        tp_exit = _float_or_none((exit_snap or {}).get("take_profit"))
        sl_exit = _float_or_none((exit_snap or {}).get("stop_loss"))

        row["trade_tp"] = tp_entry if tp_entry is not None else tp_exit
        row["trade_sl"] = sl_entry if sl_entry is not None else sl_exit
        row["trade_tp_exit"] = tp_exit
        row["trade_sl_exit"] = sl_exit

    return rows


def _attach_deposit_delta(rows: list[dict[str, Any]], snapshots: list[dict[str, Any]]) -> list[dict[str, Any]]:
    snaps = sorted(snapshots or [], key=lambda s: _millis_from_mixed(s.get("ts")) or 0)
    for row in rows:
        entry_ts = _float_or_none(row.get("entry_ts"))
        exit_ts = _float_or_none(row.get("exit_ts"))
        before_snap = _nearest_snapshot_before(entry_ts, snaps) if entry_ts is not None else None
        after_snap = _nearest_snapshot_after(exit_ts, snaps) if exit_ts is not None else None
        if after_snap is None and exit_ts is not None:
            after_snap = _nearest_snapshot_before(exit_ts, snaps)

        equity_before = _snapshot_equity(before_snap)
        equity_after = _snapshot_equity(after_snap)
        wallet_before = _snapshot_wallet_balance(before_snap)
        wallet_after = _snapshot_wallet_balance(after_snap)

        row["equity_before"] = equity_before
        row["equity_after"] = equity_after
        row["wallet_before"] = wallet_before
        row["wallet_after"] = wallet_after

        if equity_before is not None and equity_after is not None:
            delta = equity_after - equity_before
            row["deposit_delta_usd"] = round(delta, 8)
            row["deposit_delta_pct"] = round((delta / equity_before * 100.0), 8) if equity_before else None
            row["deposit_source"] = "snapshot_equity"
        else:
            row["deposit_delta_usd"] = None
            row["deposit_delta_pct"] = None
            row["deposit_source"] = None

        if wallet_before is not None and wallet_after is not None:
            wallet_delta = wallet_after - wallet_before
            row["wallet_delta_usd"] = round(wallet_delta, 8)
            row["wallet_delta_pct"] = round((wallet_delta / wallet_before * 100.0), 8) if wallet_before else None
            row["wallet_source"] = "snapshot_wallet"
        else:
            row["wallet_delta_usd"] = None
            row["wallet_delta_pct"] = None
            row["wallet_source"] = None
    return rows
...
def _pair_real_trades(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    seq = 0
    recent_close: dict[str, Any] | None = None

    def _same_close(existing: dict[str, Any] | None, ev: dict[str, Any]) -> bool:
        if not existing or not ev:
            return False
        ev_kind = str(ev.get("kind") or "")
        prev_kind = str(existing.get("kind") or "")
        if not ev_kind.startswith("exchange_"):
            return False
        # if we already had a manual/auto close almost at the same moment for same side/qty,
        # treat exchange snapshot close as confirmation, not as a separate real trade.
        if prev_kind not in {"manual_close", "auto_close", "reversal_close"}:
            return False
        prev_ts = _float_or_none(existing.get("ts"))
        ev_ts = _float_or_none(ev.get("ts"))
        if prev_ts is None or ev_ts is None:
            return False
        if abs(ev_ts - prev_ts) > 5000:  # 5 sec
            return False

        prev_side = _trade_side_from_event(current.get("entry") if current else None, existing) if current else _trade_side_from_event(None, existing)
        ev_side = _trade_side_from_event(None, ev)
        if prev_side and ev_side and prev_side != ev_side:
            return False

        prev_qty = _float_or_none(existing.get("qty"))
        ev_qty = _float_or_none(ev.get("qty"))
        if prev_qty is not None and ev_qty is not None:
            if abs(prev_qty - ev_qty) > max(1e-8, 0.02 * max(abs(prev_qty), abs(ev_qty))):
                return False
        return True

    def close_current(exit_ev: dict[str, Any], close_reason: str | None = None) -> None:
        nonlocal current, rows, recent_close
        if not current:
            return
        row = _build_trade_row_from_pair(current, exit_ev, close_reason=close_reason or str(exit_ev.get("kind") or None), seq=seq)
        if row:
            rows.append(row)
            recent_close = dict(exit_ev)
        current = None
    ...
    for event in events:
        ...
        if _is_close_kind(kind):
            if current:
                ...
                if _same_close(recent_close, event):
                    continue
                # orphan close: keep only non-duplicate diagnostics
                seq += 1
                row = _build_trade_row_from_pair(None, event, close_reason=str(event.get("kind") or None), seq=seq)
                if row:
                    rows.append(row)
                    recent_close = dict(event)
            continue
