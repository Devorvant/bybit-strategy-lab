...
from app.config import settings, REAL_DATA_ENABLED, REAL_DATA_SYMBOLS, ui_symbols
from app.storage.db import init_db, load_bars, load_bars_before, last_signal
from app.data.bybit_ws import ws_collect, ws_collect_real
...
def _db_distinct_symbols() -> List[str]:
    ...
    try:
        ...
        return sorted(set((syms or []) + ui_symbols(base)))
    except Exception:
        base = [s.strip().upper() for s in settings.SYMBOLS if s.strip()]
        return sorted(set(ui_symbols(base)))
...
async def startup():
    ...
    if REAL_DATA_ENABLED:
        asyncio.create_task(ws_collect_real(conn))
...
@app.get("/health")
def health():
    return {"ok": True, "symbols": _db_distinct_symbols(), "tf": settings.TF, "real_data_enabled": REAL_DATA_ENABLED, "real_symbols": REAL_DATA_SYMBOLS}
