...
<div class="manual-trade-box" style="margin:12px 0;padding:12px;border:1px solid #2a3b5f;border-radius:10px;background:#0b1220;">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <input id="mt_symbol" type="text" placeholder="Symbol (e.g. BTCUSDC)"
           style="min-width:140px;padding:6px 8px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;" />
    ...
    <input id="mt_note" type="text" value="manual from terminal" placeholder="note"
           style="min-width:180px;padding:6px 8px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;" />
    <button id="btnLong" type="button" style="padding:6px 12px;border-radius:8px;border:1px solid #166534;background:#16a34a;color:white;cursor:pointer;">LONG</button>
    <button id="btnShort" type="button" style="padding:6px 12px;border-radius:8px;border:1px solid #991b1b;background:#dc2626;color:white;cursor:pointer;">SHORT</button>
    <button id="btnClose" type="button" style="padding:6px 12px;border-radius:8px;border:1px solid #9a3412;background:#f97316;color:white;cursor:pointer;">CLOSE</button>
  </div>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", () => {{
      ...
      async function mtSendTradeAction(action) {{
        ...
        if (['LONG', 'SHORT', 'CLOSE'].indexOf(action) === -1) {{
          if (out) out.textContent = 'Unsupported action: ' + action;
          return;
        }}
        ...
        if (out) out.textContent = '[' + new Date().toLocaleString() + '] Sending ' + action + ' ' + payload.symbol + '...';
        ...
        try {{
          ...
          if (out) {{
            ...
            const okFlag = resp.ok && !payloadResp.error && (r.ok !== false) && (rr.ok !== false) && (!rr.error) && (retCode === null || retCode === 0);
      ...
      // Bind after DOM is ready (we are already inside DOMContentLoaded)
      const mtBtnLong = document.getElementById('btnLong');
      const mtBtnShort = document.getElementById('btnShort');
      const mtBtnClose = document.getElementById('btnClose');
      ...
      async function mtSendTradeAction(action) {{
        ...
        const tfEl = document.getElementById('mt_tf');
        const noteEl = document.getElementById('mt_note');        const tfv = ((tfEl && tfEl.value) || '2h').trim();
        const note = ((noteEl && noteEl.value) || 'manual from terminal').trim();
        const symbolBase = mtGetCurrentSymbolBase();
        const symbolBridge = symbolBase.endsWith('.P') ? symbolBase : (symbolBase + '.P');
...
const resp = await fetch('/trade/execute', {{
          ...
          if (out) {{
            ...
            if (okFlag) {{
              out.textContent = '[' + new Date().toLocaleString() + '] ' + action + ' ' + symbolBridge + ' — OK (orderId=' + orderId + ')';
            }} else {{
              out.textContent = '[' + new Date().toLocaleString() + '] ' + action + ' ' + symbolBridge + ' — ERROR (' + (retMsg || rr.error || payloadResp.detail || 'Request failed') + ')';
            }}
          }}
        }} catch (e) {{
          if (out) out.textContent = '[' + new Date().toLocaleString() + '] ERROR\\n' + (e && e.message ? e.message : String(e));
