...
def build_plotly_payload(
    ...
    *,
    ...
    # Trades table HTML (used by Playground live updates)
    trades_html = None
    try:
        params_for_metrics = dict(strategy_params or {})
        params_for_metrics.setdefault("capital_usd", float(capital_usd))
        trades_html = _build_trades_table_html(bt, bars=bars, params=params_for_metrics)
    except Exception:
        trades_html = None
    return {
        "ok": True,
...
# Playground specs: subset of params we can tweak quickly with one slider.
...
# Build select options (with a visual separator between main params and "прочее")
opt_html = []
def _mk_opt(k: str) -> str:
    sel = 'selected' if k == play_param else ''
    return f"<option value='{k}' {sel}>{k}</option>"

for k in MAIN_PLAY_KEYS:
    if k in PLAY_SPECS:
        opt_html.append(_mk_opt(k))

# Separator line (disabled option)
opt_html.append("<option value='' disabled>──────── Прочее ────────</option>")

for k in OTHER_PLAY_KEYS:
    if k in PLAY_SPECS:
        opt_html.append(_mk_opt(k))# Put raw JSON into <script type="application/json"> so JS can JSON.parse it.
        ...
        <div class="params-card" id="playCard">
          ...
          <div class="muted" style="margin-top:6px;">Обновление происходит при отпускании ползунка. Пересчитываются график и таблица сделок.</div>
    ...
    document.addEventListener("DOMContentLoaded", () => {{
      ...
      async function doPlayUpdate() {{
        ...
        try {{
          ...
          // Update trades table if provided
          if (j.trades_html) {{
            const tw = document.getElementById('tradesWrap') || document.querySelector('.trades-wrap');
