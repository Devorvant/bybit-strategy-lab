...
def _sync_strategy_trade_journal_with_strategy(
    *,
    ...
    """
    Hard sync from strategy to strategy_trade_journal.

    Source of truth for closing is the current strategy backtest:
    - if a journal row is OPEN
    - and strategy has the same trade_key among CLOSED trades (match_id with exit_ts)
    then close the journal row strictly with strategy exit_ts/exit_price/reason.
    """
