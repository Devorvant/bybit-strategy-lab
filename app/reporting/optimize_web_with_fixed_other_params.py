...
def _robust_evaluate(
    *,
    ...
    params_eval["confirm_on_close"] = bool(confirm_on_close)
    if use_costs_in_opt:
...
def _run_optimizer_sync(run: RunState, bars: List[Tuple[int, float, float, float, float, float]], conn) -> None:
    ...
    # Fixed params from optimizer UI ("Прочее"), applied to every trial.
    fixed_params: Dict[str, Any] = dict(cfg.get("fixed_params") or {})
    # Normalize types
    for k in ("adx_len", "adx_smooth", "st_atr_len", "atr_len"):
        if k in fixed_params:
            try:
                fixed_params[k] = int(float(fixed_params[k]))
            except Exception:
                fixed_params[k] = int(14)
    for k in ("use_rev_cooldown", "use_emergency_sl", "close_at_end"):
        if k in fixed_params:
            fixed_params[k] = bool(fixed_params[k])
    ...
    def _maybe_update_best(params: Dict[str, Any], trial_no: int) -> float:
        ...
        params_eval = dict(params)
        # Apply fixed "other" params (not optimized)
        if fixed_params:
            params_eval.update(fixed_params)

        # Execution mode (same for all trials)
        params_eval["confirm_on_close"] = bool(confirm_on_close)
        if use_costs_in_opt:
            params_eval.update(fixed_costs)
        ...
        if improved:
            ...
            if fixed_params:
                run.best_params.update(fixed_params)
            run.best_params["confirm_on_close"] = bool(confirm_on_close)
            run.best_metrics = {
                ...
                for t in range(1, trials_per_seed + 1):
                    ...
                    if sel_score > best_sel_seed:
                        ...
                        best_seed_rec = {
                            ...
                            "train_score": float(s_tr),
                            "val_score": float(s_va),
                            "params": (lambda _p: (_p.update(fixed_params) or _p.update({"confirm_on_close": bool(confirm_on_close)}) or _p))(dict(params)),
...
Если выключено: используем текущую свечу (может перерисовываться на live).'>?</span></label>
        ...
        <div style='margin-top:14px; font-weight:700;'>Прочее (фикс перед оптимизацией)</div>
        <div class='muted' style='margin-bottom:8px; line-height:1.35;'>
          Эти параметры не оптимизируются: они фиксируются вручную и применяются на каждом trial (как costs).
          Удобно, чтобы тестировать влияние «прочих» настроек без раздувания пространства поиска.
        </div>
        <div class='row' style='flex-wrap:wrap; gap:14px;'>
          <div>
            <label>fx_adx_len</label>
            <input name='fx_adx_len' value='14' style='width:110px'/>
          </div>
          <div>
            <label>fx_adx_smooth</label>
            <input name='fx_adx_smooth' value='14' style='width:110px'/>
          </div>
          <div>
            <label>fx_st_atr_len</label>
            <input name='fx_st_atr_len' value='14' style='width:110px'/>
          </div>
          <div>
            <label>fx_atr_len</label>
          ...
          <div style='flex-direction:row; align-items:center; gap:10px; padding-top:20px;'>
            <input type='checkbox' name='fx_use_rev_cooldown' id='fx_use_rev_cooldown' checked/>
            <label for='fx_use_rev_cooldown' style='margin:0; font-size:13px;'>fx_use_rev_cooldown</label>
          </div>

          <div style='flex-direction:row; align-items:center; gap:10px; padding-top:20px;'>
            <input type='checkbox' name='fx_use_emergency_sl' id='fx_use_emergency_sl' checked/>
            <label for='fx_use_emergency_sl' style='margin:0; font-size:13px;'>fx_use_emergency_sl</label>
          </div>

          <div style='flex-direction:row; align-items:center; gap:10px; padding-top:20px;'>
            <input type='checkbox' name='fx_close_at_end' id='fx_close_at_end'/>
            <label for='fx_close_at_end' style='margin:0; font-size:13px;'>fx_close_at_end</label>
          </div>
        </div>
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    def _b_default(key: str, default: bool) -> bool:
        # For checkboxes: when unchecked the field is absent in payload.
    ...
    cfg = {
        ...
        # Fixed params ("Прочее") applied to every trial (not optimized)
        "fixed_params": {
            "adx_len": _i("fx_adx_len", 14),
            "adx_smooth": _i("fx_adx_smooth", 14),
            "st_atr_len": _i("fx_st_atr_len", 14),
            "atr_len": _i("fx_atr_len", 14),
            "use_rev_cooldown": _b_default("fx_use_rev_cooldown", True),
            "use_emergency_sl": _b_default("fx_use_emergency_sl", True),
            "close_at_end": _b_default("fx_close_at_end", False),
        },
        "seed": seed,
        "seed_from": seed_from,
        "seed_to": seed_to,
