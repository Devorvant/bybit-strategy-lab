...
function mtRenderLivePosition(payload) {{
      ...
      async function mtStartAutoLoop() {{
        ...
        try {{
          const resp = await fetch('/trade/auto/loop/start', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ symbol: sym.replace(/\.P$/i, ''), tf: tfv, opt_id: optId, trade_params }})
          }});
          const txt = await resp.text();
          let parsed;
          try {{ parsed = JSON.parse(txt); }} catch (_) {{ parsed = {{ ok:false, raw: txt }}; }}
      ...
      async function mtAutoCall(execute) {{
        ...
        if (tpEl && String(tpEl.value || '').trim()) trade_params.tp_percent = Number(tpEl.value);
        const optId = mtGetSelectedOptId();
        const payload = {{ symbol: sym.replace(/\.P$/i, ''), tf: tfv, execute: !!execute, trade_params, opt_id: optId }};
        ...
        try {{
          ...
          if (execute) {{
            ...
            const q = '/trade/auto/preview?symbol=' + encodeURIComponent(payload.symbol) + '&tf=' + encodeURIComponent(payload.tf) + (payload.opt_id ? ('&opt_id=' + encodeURIComponent(String(payload.opt_id))) : '');
            resp = await fetch(q);
          }}
          const txt = await resp.text();
          let parsed;
