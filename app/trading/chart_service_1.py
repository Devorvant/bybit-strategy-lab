...
class TradeChartService:
    ...
    def build_trade_chart_payload(
        ...
        *,
        ...
        live_events = self._build_live_events(symbol=symbol, tf=tf, bars=bars)
        live_summary = self._build_live_summary(live_events)

        has_live = bool(live_events) or bool(live_status.get("current_trading_strategy"))
    ...
    def build_trade_events_payload(self, *, symbol: str, tf: str, limit: int = 500) -> dict[str, Any]:
        ...
        events = self._build_live_events(symbol=symbol, tf=tf, bars=bars, limit=limit)
        return {"ok": True, "symbol": symbol, "tf": tf, "count": len(events), "events": events}
    ...
    def _build_chart_block(self, *, bars: list[tuple[int, float, float, float, float, float]], bt: Any, strategy: str) -> dict[str, Any]:
        ...
        equity = [
            {"ts": int(ts), "value": float(eq)}
            for ts, eq in zip(list(getattr(bt, "equity_ts", []) or []), list(getattr(bt, "equity", []) or []))
        ]
        return {
            "range": {"from_ts": int(bars[0][0]), "to_ts": int(bars[-1][0])},
            "bars": out_bars,
    ...
    def _build_live_events(self, *, symbol: str, tf: str, bars: list[tuple[int, float, float, float, float, float]], limit: int = 500) -> list[dict[str, Any]]:
...
def _normalize_execution_events(rows: list[dict[str, Any]], bars: list[tuple[int, float, float, float, float, float]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        req = str(row.get("requested_action") or "").upper()
        ...
        if "manual" in event_type:
            prefix = "manual"
            source = "manual"
        elif event_type.startswith("auto") or event_type.startswith("execute_"):
            prefix = "auto"
            source = "auto"
        else:
            prefix = None
        ...
        if isinstance(response, str):
            try:
                ...
                response = None
        result = ((response or {}).get("response") or {}).get("result") or {}
        price = _float_or_none(row.get("price"))
        qty = _float_or_none(row.get("qty"))
        if price is None:
            price = _float_or_none(result.get("price")) or _float_or_none(result.get("avgPrice"))
        if qty is None:
            qty = _float_or_none(result.get("qty")) or _float_or_none(result.get("cumExecQty"))

        out.append(
            {
                "id": str(row.get("id") or f"exec_{ts}_{kind}"),
                "ts": ts,
                ...
                "source": source,
                "side": _normalize_side(row.get("side")) or ("Buy" if "long" in kind else "Sell" if "short" in kind else None),
                "price": price,
                "qty": qty,
                "order_id": row.get("order_id") or result.get("orderId"),
                "exec_id": None,
...
def _derive_snapshot_events(rows: list[dict[str, Any]], bars: list[tuple[int, float, float, float, float, float]]) -> list[dict[str, Any]]:
    ...
    for row in ordered:
        ...
        if prev is not None and prev_has and not cur_has:
            had_protection = _float_or_none(prev.get("take_profit")) not in (None, 0.0) or _float_or_none(prev.get("stop_loss")) not in (None, 0.0)
            kind = "exchange_tp_sl_close" if had_protection else "exchange_external_close"
            note = "position closed via exchange TP/SL" if kind == "exchange_tp_sl_close" else "position closed externally on exchange"
            out.append(
                {
                    ...
                    "price": _float_or_none(row.get("mark_price")) or _float_or_none(prev.get("mark_price")) or _float_or_none(prev.get("avg_price")),
...
def _ret_pct(eq: list[float]) -> float:
    if len(eq) < 2:
        return 0.0
    base = eq[0] if abs(eq[0]) > 1e-9 else 10000.0
