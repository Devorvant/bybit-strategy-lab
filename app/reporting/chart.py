...
function mtRenderLivePosition(payload) {{
      ...
      async function mtSendTradeAction(action) {{
        ...
        const symbolEl = document.getElementById('mt_symbol');
        const levEl = document.getElementById('mt_leverage');
        const marginEl = document.getElementById('mt_qty_usd');
        const slEl = document.getElementById('mt_sl_pct');
        const tpEl = document.getElementById('mt_tp_pct');

        const tfVal = ((tfEl && tfEl.value) ? tfEl.value : '2h').trim() || '2h';
        const noteVal = ((noteEl && noteEl.value) ? noteEl.value : 'manual from terminal').trim() || 'manual from terminal';
        const symbolVal = ((symbolEl && symbolEl.value) ? symbolEl.value : '').trim();
        const levVal = ((levEl && levEl.value) ? levEl.value : '').trim();
        const marginVal = ((marginEl && marginEl.value) ? marginEl.value : '').trim();
        const slVal = ((slEl && slEl.value) ? slEl.value : '').trim();
        const tpVal = ((tpEl && tpEl.value) ? tpEl.value : '').trim();

        const resolved = mtResolveSymbol(symbolVal);
        if (!resolved) {{
          if (out) out.textContent = '[' + new Date().toLocaleTimeString() + '] ERROR\nSymbol is empty';
          return;
        }}
        ...
        if (levVal !== '') payload.leverage = Number(levVal);
        if (marginVal !== '') payload.qty_usd = Number(marginVal);
        if (slVal !== '') payload.sl_percent = Number(slVal);
        if (tpVal !== '') payload.tp_percent = Number(tpVal);

        const cmsg = 'Отправить ' + action + ' на ' + payload.symbol + ' (tf=' + payload.tf + ')?';
        ...
        try {{
          ...
          if (out) {{
            const payloadResp = parsed || {{}};
            const r = payloadResp.response || payloadResp || {{}};
            const rr = (r && r.result) ? r.result : {{}};
            const ex = (rr && rr.response) ? rr.response : ((r && r.response) ? r.response : {{}});
            const exResult = (ex && ex.result) ? ex.result : {{}};
            const orderId = exResult.orderId || '-';
            const retCode = (typeof ex.retCode !== 'undefined') ? ex.retCode : null;
            const retMsg = (typeof ex.retMsg !== 'undefined') ? ex.retMsg : '';
            const okFlag = resp.ok && !payloadResp.error && (payloadResp.ok !== false) && (!rr.error) && (retCode === null || retCode === 0);

            if (okFlag) {{
              out.textContent = '[' + new Date().toLocaleTimeString() + '] ' + action + ' ' + payload.symbol + ' — OK (orderId=' + orderId + ')';
              try {{ mtRefreshLivePosition(); }} catch (_) {{}}
            }} else {{
              out.textContent = '[' + new Date().toLocaleTimeString() + '] ' + action + ' ' + payload.symbol + ' — ERROR (' + (retMsg || rr.error || payloadResp.detail || 'Request failed') + ')';
            }}
          }}
        }} catch (e) {{
      ...
      if (mtBtnLong && !mtBtnLong.dataset.bound) {{ mtBtnLong.dataset.bound='1'; mtBtnLong.addEventListener('click', function() {{ mtSendTradeAction('LONG'); }}); }}
      if (mtBtnShort && !mtBtnShort.dataset.bound) {{ mtBtnShort.dataset.bound='1'; mtBtnShort.addEventListener('click', function() {{ mtSendTradeAction('SHORT'); }}); }}
      if (mtBtnClose && !mtBtnClose.dataset.bound) {{ mtBtnClose.dataset.bound='1'; mtBtnClose.addEventListener('click', function() {{ mtSendTradeAction('CLOSE'); }}); }}

      // Live panel controls init
      var mtLiveRefreshBtn = document.getElementById('mt_live_refresh');
      if (mtLiveRefreshBtn && !mtLiveRefreshBtn.dataset.bound) {{
        mtLiveRefreshBtn.dataset.bound = '1';
        mtLiveRefreshBtn.addEventListener('click', function() {{ mtRefreshLivePosition(); }});
      }}
      var mtLiveCloseBtn = document.getElementById('mt_live_close');
      if (mtLiveCloseBtn && !mtLiveCloseBtn.dataset.bound) {{
        mtLiveCloseBtn.dataset.bound = '1';
        mtLiveCloseBtn.addEventListener('click', function() {{ mtLiveClosePosition(); }});
      }}
      try {{ mtLiveStartAutoRefresh(); }} catch (_) {{}}
      try {{ mtRefreshLivePosition(); }} catch (_) {{}}
