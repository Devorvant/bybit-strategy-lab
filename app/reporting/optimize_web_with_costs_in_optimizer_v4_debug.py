...
def _run_optimizer_sync(run: RunState, bars: List[Tuple[int, float, float, float, float, float]], conn) -> None:
    ...
    _add_log(run, "WEIGHTS: " + json.dumps(weights, ensure_ascii=False) + f" min_trades={min_trades}")
    if use_costs_in_opt:
        _add_log(run, "COSTS enabled: " + json.dumps(costs_fixed, ensure_ascii=False))
    ...
    def _maybe_update_best(params: Dict[str, Any], trial_no: int) -> float:
        ...
        params_eval = {**params, **costs_fixed} if costs_fixed else params
        ...
        if s_tr > best_train_score:
            ...
            # Debug / transparency: when COSTS are enabled, also compute the same metrics WITHOUT costs
            m_va_no_costs = None
            m_tr_no_costs = None
            if costs_fixed:
                try:
                    bt_va0 = backtest_strategy3(va_bars, **params)
                    m_va_no_costs = _equity_metrics(bt_va0.equity, base=position_usd)
                except Exception:
                    m_va_no_costs = None
                try:
                    bt_tr0 = backtest_strategy3(train_bars, **params)
                    m_tr_no_costs = _equity_metrics(bt_tr0.equity, base=position_usd)
                except Exception:
                    m_tr_no_costs = None

            if costs_fixed and m_va_no_costs:
                try:
                    d_ret = (m_va.get("ret", 0.0) - m_va_no_costs.get("ret", 0.0))
                    d_dd = (m_va.get("dd", 0.0) - m_va_no_costs.get("dd", 0.0))
                    _add_log(run, f"COSTS_DELTA val: d_ret={d_ret:+.4f} d_dd={d_dd:+.4f}")
                except Exception:
                    pass

            run.best_score = s_va
            run.best_params = params_eval
            run.best_trial = int(trial_no)
            run.best_metrics = {
                "costs_in_opt": bool(use_costs_in_opt),
                "costs": costs_fixed if costs_fixed else {},
                "val_metrics_no_costs": m_va_no_costs if m_va_no_costs else {},
                "train_metrics_no_costs": m_tr_no_costs if m_tr_no_costs else {},
                **m_va,
                "trades": trades_va,
                "val_score": s_va,
                "train_score": s_tr,
            ...
            if costs_fixed and m_va_no_costs:
                try:
                    d_ret = m_va.get("ret", 0.0) - float(m_va_no_costs.get("ret", 0.0))
                    _add_log(run, f"    COST_IMPACT val_ret_delta={d_ret:+.4f} (with_costs - no_costs)")
                except Exception:
                    pass
        else:
            no_improve += 1
...
<div>
        ...
        <div style='margin-top:4px; font-weight:700;'>Комиссии в оптимизации (опционально)</div>
        <div class='muted' style='margin:6px 0 10px 0; line-height:1.35;'>
          По умолчанию оптимизация идёт <b>без</b> комиссий/спрэда/проскальзывания (как раньше).<br/>
          Включи галочку, чтобы оптимизатор сразу учитывал «трение» и выбирал параметры, устойчивые к реальным издержкам.
        </div>

        <div class='row' style='flex-wrap:wrap; gap:14px;'>
          <div>
            <label><span class='lbl'>use_costs_in_opt<span class='tip' data-tip='Если включено — оптимизация будет считать комиссии/спрэд/проскальзывание/фандинг прямо в каждом trial.&#10;Если выключено — полностью классический режим (как раньше).'>?</span></span></label>
            <div style='display:flex; align-items:center; gap:8px; height:34px;'>
              ...
              <span class='muted' style='font-size:12px;'>учитывать издержки</span>
            </div>
          </div>

          <div>
            <label><span class='lbl'>fee_percent<span class='tip' data-tip='Комиссия биржи на сторону (%).&#10;Напр.: taker 0.06% → 0.06.&#10;Считается на вход и на выход.'>?</span></span></label>
            <input name='fee_percent' value='0.06' style='width:110px'/>
          </div>

          <div>
            <label><span class='lbl'>spread_ticks<span class='tip' data-tip='Bid-ask спрэд в тиках (полный спрэд).&#10;В модели берём половину на входе и половину на выходе (хуже для трейдера).&#10;1.0 = один тик.'>?</span></span></label>
            <input name='spread_ticks' value='1.0' style='width:110px'/>
          </div>

          <div>
            <label><span class='lbl'>slippage_ticks<span class='tip' data-tip='Проскальзывание в тиках на сторону.&#10;2 = дополнительно 2 тика «хуже» к цене на входе и на выходе.'>?</span></span></label>
            <input name='slippage_ticks' value='2' style='width:110px'/>
          </div>

          <div>
            <label><span class='lbl'>tick_size<span class='tip' data-tip='Размер тика (шаг цены).&#10;Для APTUSDT Perp обычно 0.0001.&#10;Если tick_size=0, модель спрэда/проскальзывания отключится.'>?</span></span></label>
            <input name='tick_size' value='0.0001' style='width:110px'/>
          </div>

          <div>
            <label><span class='lbl'>funding_8h_percent<span class='tip' data-tip='Фандинг per 8 часов (%), для perpetual.&#10;0.01 = 0.01% каждые 8 часов.&#10;Пока моделируем как фиксированную ставку (по желанию).'>?</span></span></label>
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    def _b(key: str, default: bool = False) -> bool:
        ...
        if v is None or v == "":
            return bool(default)
        s = str(v).strip().lower()
        return s in ("1", "true", "on", "yes", "y", "t", "да")
    ...
    cfg = {
        ...
        "seed": 42,
                ...
                # costs (optional; classic optimizer ignores unless enabled)
