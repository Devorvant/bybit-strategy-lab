...
from app.trading.trade_chart_routes import router as trade_chart_router
from app.trading.strategy_registry import ensure_opt_auto_columns, get_opt_strategy_entry
app.include_router(trading_router)
app.include_router(trade_chart_router)
...
@app.get("/health")
def health():
    return {"ok": True, "symbols": settings.SYMBOLS, "tf": settings.TF, "bybit_mode": settings.BYBIT_MODE}
...
def api_save_chart_snapshot(body: dict = Body(...)):
    ...
    end_shift_hours = float(body.get("end_shift_hours") or 0.0)
