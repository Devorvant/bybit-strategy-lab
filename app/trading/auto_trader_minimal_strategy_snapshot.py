...
def _sync_strategy_trade_journal_from_bt(
    *,
    ...
    """Persist recent strategy trades into strategy_trade_journal without rewriting history."""
    try:
        conn = _get_conn()
        now_ms = int(time.time() * 1000)
        trades = list(getattr(bt, 'trades', []) or [])[-max(1, int(limit)):]
        sl_pct = float(trade_params.get('sl_percent') or 0.0)
        tp_pct = float(trade_params.get('tp_percent') or 0.0)
        ...
        for tr in trades:
            ...
            entry_price = float(getattr(tr, 'entry_price', 0.0) or 0.0)
            if not entry_ts:
                continue
            trade_key = make_strategy_trade_key(symbol, tf, opt_id, entry_ts)
            tp_price = None
            sl_price = None
            if side == 'LONG':
                if tp_pct > 0: tp_price = entry_price * (1.0 + tp_pct / 100.0)
                if sl_pct > 0: sl_price = entry_price * (1.0 - sl_pct / 100.0)
            elif side == 'SHORT':
                if tp_pct > 0: tp_price = entry_price * (1.0 - tp_pct / 100.0)
                if sl_pct > 0: sl_price = entry_price * (1.0 + sl_pct / 100.0)

            upsert_strategy_trade_open(conn, {
                'trade_key': trade_key, 'symbol': symbol, 'tf': tf, 'strategy': 'my_strategy3', 'opt_id': opt_id,
                'side': side, 'entry_ts': entry_ts, 'entry_price': entry_price, 'tp_price': tp_price, 'sl_price': sl_price,
                'status': 'open', 'bybit_order_link_id': trade_key, 'created_ts': now_ms, 'updated_ts': now_ms,
            })
            update_strategy_trade_close(
                conn, trade_key=trade_key, exit_ts=int(getattr(tr, 'exit_ts', 0) or 0) or None,
                exit_price=float(getattr(tr, 'exit_price', 0.0) or 0.0) or None,
                close_reason=getattr(tr, 'exit_reason', None), updated_ts=now_ms,
            )

        pos = getattr(bt, 'open_position', None)
        if pos:
            side = str((pos or {}).get('side') or '').upper()
            entry_ts = int((pos or {}).get('entry_ts') or 0)
