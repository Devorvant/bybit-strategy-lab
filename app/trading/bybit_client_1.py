...
# Trade environment:
# - TRADE_ENV=testnet|mainnet (preferred)
# - fallback to legacy BYBIT_TESTNET (default was testnet)
TRADE_ENV = (os.getenv('TRADE_ENV') or '').strip().lower() or ('testnet' if os.getenv('BYBIT_TESTNET', '1') != '0' else 'mainnet')
BYBIT_TESTNET = TRADE_ENV == 'testnet'

# Separate keys per environment (recommended). Legacy BYBIT_KEY/BYBIT_SECRET still supported.
if BYBIT_TESTNET:
    BYBIT_KEY = os.getenv('BYBIT_KEY_TESTNET', '') or os.getenv('BYBIT_KEY', '')
    BYBIT_SECRET = os.getenv('BYBIT_SECRET_TESTNET', '') or os.getenv('BYBIT_SECRET', '')
else:
    BYBIT_KEY = os.getenv('BYBIT_KEY_MAINNET', '') or os.getenv('BYBIT_KEY', '')
    BYBIT_SECRET = os.getenv('BYBIT_SECRET_MAINNET', '') or os.getenv('BYBIT_SECRET', '')
        ...
        try:
            ...
            def _f(v):
                try:
                    ...
                    return None
            # Prefer account-level fields when present, fallback to USDT coin fields
