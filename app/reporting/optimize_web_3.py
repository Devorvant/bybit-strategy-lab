...
def _robust_eval_candidate(
    *,
    bars_all: List[Tuple[int, float, float, float, float, float]],
    tf_minutes: int,
    params: Dict[str, Any],
    ...
    """Walk-forward style robustness evaluation.

    We create `windows` rolling windows ending at the latest bar and moving backwards by `shift_hours`.
    Inside each window we do the same train/val split and compute val_score.

    Returns a dict with per-window metrics and summary statistics.
    """
    ...
    if windows <= 0 or window_bars <= 50 or not bars_all:
        return out

    bars_per_hour = 60.0 / float(max(1, tf_minutes))
    shift_bars = int(round(float(shift_hours) * bars_per_hour))
    shift_bars = max(1, shift_bars) if windows > 1 else 0

    need = window_bars + shift_bars * (windows - 1)
    if len(bars_all) < need:
        # reduce windows to what we can support
        max_windows = 1 + max(0, (len(bars_all) - window_bars) // max(1, shift_bars))
        windows = max(1, int(max_windows))
        out["windows"] = int(windows)
        need = window_bars + shift_bars * (windows - 1)
        if len(bars_all) < window_bars:
            return out

    # Use same params, optionally with fixed costs
    p = dict(params)
    if use_costs:
        p.update(dict(fixed_costs))

    base = float(p.get("position_usd", 1000.0) or 1000.0)
    ...
    for wi in range(windows):
        end_i = len(bars_all) - wi * shift_bars
        start_i = max(0, end_i - window_bars)
        wbars = bars_all[start_i:end_i]
        if len(wbars) < 300:
            continue
        ...
        # backtest train
        bt_tr = backtest_strategy3(tr_bars, **p)
        m_tr = _equity_metrics(bt_tr.equity, base=base)
        m_tr.update(_trade_metrics(bt_tr.trades, tr_bars[0][0], tr_bars[-1][0], base=base))
        trades_tr = int(len(bt_tr.trades))
        tr_score = _score(m_tr, trades_tr, weights, min_trades)

        # backtest val
        bt_va = backtest_strategy3(va_bars, **p)
        m_va = _equity_metrics(bt_va.equity, base=base)
        m_va.update(_trade_metrics(bt_va.trades, va_bars[0][0], va_bars[-1][0], base=base))
        trades_va = int(len(bt_va.trades))
        va_score = _score(m_va, trades_va, weights, min_trades)

        val_scores.append(float(va_score))
        val_rets.append(float(m_va.get("ret", 0.0)))
        val_dds.append(float(m_va.get("dd", 0.0)))
        val_sharpes.append(float(m_va.get("sharpe", 0.0)))

        out["per_window"].append(
            {
                "i": int(wi),
                "start_ts": int(wbars[0][0]),
                "end_ts": int(wbars[-1][0]),
                "train_score": float(tr_score),
                "val_score": float(va_score),
                "val_metrics": {k: float(v) for k, v in m_va.items() if isinstance(v, (int, float))},
                "train_metrics": {k: float(v) for k, v in m_tr.items() if isinstance(v, (int, float))},
            }
        )
    ...
    out["ok"] = True
    out["summary"] = {
        "val_score_mean": float(mean(val_scores)),
        "val_score_median": float(median(val_scores)),
        "val_score_min": float(min(val_scores)),
        "val_ret_mean": float(mean(val_rets)) if val_rets else 0.0,
        "val_dd_mean": float(mean(val_dds)) if val_dds else 0.0,
        "val_sharpe_mean": float(mean(val_sharpes)) if val_sharpes else 0.0,
        "windows_used": int(len(val_scores)),
    }
    return out
...
def _run_optimizer_sync(run: RunState, bars: List[Tuple[int, float, float, float, float, float]], conn) -> None:
    ...
    # Defaults match /chart (Strategy3) so that optimizer and chart live in the same world.
    fixed_costs: Dict[str, Any] = {
    ...
    # Robustness check (walk-forward style) after we have a candidate.
    # End is always the latest bar in DB; windows are shifted backwards.
    robust_check = bool(cfg.get("robust_check", False))
    robust_windows = int(cfg.get("robust_windows", 10) or 10)
    robust_bars = int(cfg.get("robust_bars", cfg.get("limit_bars", 5000)) or cfg.get("limit_bars", 5000))
    robust_shift_hours = float(cfg.get("robust_shift_hours", 10.0) or 10.0)
    robust_start_hours_ago = float(cfg.get("robust_start_hours_ago", 0.0) or 0.0)
    robust_top_k = int(cfg.get("robust_top_k", 5) or 5)

    robust_windows = max(1, min(200, robust_windows))
    robust_bars = max(300, min(200000, robust_bars))
    robust_top_k = max(1, min(200, robust_top_k))

    # If user provided a desired span (start hours ago), compute shift automatically.
    if robust_start_hours_ago and robust_windows > 1:
        robust_shift_hours = max(0.5, float(robust_start_hours_ago) / float(robust_windows - 1))
    ...
    _add_log(
        ...
        f"robust_check={robust_check} robust_windows={robust_windows} robust_bars={robust_bars} "
        f"robust_shift_hours={robust_shift_hours:.3g} robust_top_k={robust_top_k}",
    )
    ...
    try:
        if optimizer == "optuna_tpe":
            ...
            for si, seed_i in enumerate(range(seed_from, seed_to + 1), start=1):
                ...
                for t in range(1, trials_per_seed + 1):
                    ...
                    m_tr = _equity_metrics(bt_tr.equity, base=position_usd)
                    trades_tr = len(bt_tr.trades)
                    s_tr = _bt_score(m_tr, weights=weights, trades=trades_tr, min_trades=min_trades)
                    bd_tr = _score_breakdown(m_tr, weights=weights, trades=trades_tr, min_trades=min_trades)

                    bt_va = backtest_strategy3(val_bars, **params_eval)
                    m_va = _equity_metrics(bt_va.equity, base=position_usd)
                    trades_va = len(bt_va.trades)
                    s_va = _bt_score(m_va, weights=weights, trades=trades_va, min_trades=min_trades)
                    bd_va = _score_breakdown(m_va, weights=weights, trades=trades_va, min_trades=min_trades)
        ...
        # Robustness evaluation (optional) on several shifted windows.
        # This helps assess sensitivity to time shifts.
        if robust_check and run.best_params:
            try:
                tf_minutes = int(float(tf)) if str(tf).strip() else 60
            except Exception:
                tf_minutes = 60

            try:
                # Estimate how many bars we need for the requested windows.
                bars_per_hour = 60.0 / float(max(1, tf_minutes))
                shift_bars = int(round(float(robust_shift_hours) * bars_per_hour))
                shift_bars = max(1, shift_bars) if robust_windows > 1 else 0
                need_bars = robust_bars + shift_bars * (robust_windows - 1)

                # Load enough bars (may be larger than the training window).
                bars_robust = load_bars(conn, symbol, str(tf), limit=int(need_bars))
                if len(bars_robust) < 300:
                    _add_log(run, f"ROBUST_SKIP not enough bars: {len(bars_robust)}")
                else:
                    rb_best = _robust_eval_candidate(
                        bars_all=bars_robust,
                        tf_minutes=tf_minutes,
                        params=dict(run.best_params),
                        weights=weights,
                        min_trades=min_trades,
                        train_frac=train_frac,
                        windows=robust_windows,
                        window_bars=robust_bars,
                        shift_hours=float(robust_shift_hours),
                        fixed_costs=fixed_costs,
                        use_costs=bool(use_costs_in_opt),
                    )
                    if run.best_metrics is None:
                        run.best_metrics = {}
                    run.best_metrics["robust_eval"] = rb_best
                    if rb_best.get("ok"):
                        s = rb_best.get("summary", {}) or {}
                        _add_log(
                            run,
                            "ROBUST_DONE "
                            f"windows_used={s.get('windows_used')} "
                            f"val_score_mean={s.get('val_score_mean'):.6g} "
                            f"val_score_median={s.get('val_score_median'):.6g} "
                            f"val_score_min={s.get('val_score_min'):.6g}",
                        )

                    # If multi-seed candidates exist, evaluate top-K candidates too.
                    try:
                        cand_payload = load_candidates(run.run_id)
                    except Exception:
                        cand_payload = None
                    if cand_payload and isinstance(cand_payload.get("top_candidates"), list):
                        top_list = list(cand_payload.get("top_candidates") or [])
                        k = min(len(top_list), int(robust_top_k))
                        for i in range(k):
                            c = top_list[i]
                            p = c.get("params") if isinstance(c, dict) else None
                            if not isinstance(p, dict):
                                continue
                            rb = _robust_eval_candidate(
                                bars_all=bars_robust,
                                tf_minutes=tf_minutes,
                                params=dict(p),
                                weights=weights,
                            ...
                            if isinstance(c, dict):
                                c["robust_eval"] = rb
                                if rb.get("ok"):
                                    c["robust_summary"] = rb.get("summary", {})
                        cand_payload["top_candidates"] = top_list
                        save_candidates(run.run_id, cand_payload)
                        _add_log(run, f"ROBUST_CANDIDATES_DONE top_k={k}")
            except Exception as e:
...
def _optimize_index_html(conn) -> str:
    ...
    <div class='card'>
      <form id='startForm'>
        ...
        <div class='muted' style='margin-bottom:8px; line-height:1.35;'>
          После подбора параметров делаем проверку на нескольких сдвинутых окнах времени.
          Конец последнего окна всегда «текущее» (последний бар), остальные окна сдвигаем назад.
          Считаем val_score на каждом окне и даём сводку (mean/median/min).
        </div>
        <div class='row' style='flex-wrap:wrap; gap:14px;'>
          <div style='flex-direction:row; align-items:center; gap:10px; padding-top:20px;'>
          ...
          <div>
            <label><span class='lbl'>robust_windows<span class='tip' data-tip='Сколько окон проверять.\nПример: 10.'>?</span></span></label>
            <input name='robust_windows' value='10' style='width:110px'/>
          </div>
          <div>
            <label><span class='lbl'>robust_bars<span class='tip' data-tip='Длина каждого окна в барах.\nМожно поставить 20000 для проверки на длинной истории.'>?</span></span></label>
            <input name='robust_bars' value='5000' style='width:110px'/>
          </div>
          <div>
            <label><span class='lbl'>robust_shift_hours<span class='tip' data-tip='Сдвиг конца окна назад (в часах).\nОкна будут: now, now-shift, now-2*shift, ...'>?</span></span></label>
            <input name='robust_shift_hours' value='10' style='width:110px'/>
          </div>
          <div>
            <label><span class='lbl'>robust_start_hours_ago<span class='tip' data-tip='Опционально: если задано >0, то shift будет вычислен автоматически так, чтобы самое старое окно начиналось примерно N часов назад.\nПриоритет над robust_shift_hours.'>?</span></span></label>
            <input name='robust_start_hours_ago' value='0' style='width:130px'/>
          </div>
          <div>
            <label><span class='lbl'>robust_top_k<span class='tip' data-tip='Сколько кандидатов проверять.\nДля обычного run берём best, для multi-seed берём top-K из candidates.'>?</span></span></label>
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    cfg = {
        ...
        # Defaults match /chart (Strategy3)
        ...
        # robustness (walk-forward) check (optional)
        ...
        "robust_start_hours_ago": _f("robust_start_hours_ago", 0.0),
        "robust_top_k": _i("robust_top_k", 5),
