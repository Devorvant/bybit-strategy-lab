...
def _json_sanitize(obj: Any) -> Any:
    """Recursively sanitize data for strict JSON storage (Postgres json doesn't allow NaN/Infinity)."""
    try:
        import math
...
def _atomic_write_json(path: Path, data: Dict[str, Any]) -> None:
    ...
    tmp.write_text(json.dumps(_json_sanitize(data), ensure_ascii=False, allow_nan=False), encoding="utf-8")
    tmp.replace(path)
    ...
    if cand and isinstance(cand, dict):
        ...
        if isinstance(top, list) and top:
            ...
            for i, r in enumerate(top):
                ...
                # For multi-window mode we want the chart to open on the SAME shifted window.
end_shift_h = cand.get("shift_hours", None)
if end_shift_h is None:
    end_shift_h = float((cfg or {}).get("end_shift_hours", 0) or 0)
link = (
    f"/chart?symbol={html.escape(str(cand.get('symbol','')))}"
    f"&tf={html.escape(str(cand.get('tf','')))}"
    f"&limit={int(cand.get('limit_bars', 5000) or 5000)}"
    f"&strategy=my_strategy3.py"
    f"&end_shift_hours={float(end_shift_h)}"
    f"&opt_run_id={html.escape(run_id)}&opt_cand={i}"
)
