...
def _robust_evaluate(
    *,
    ...
    Create N windows ending at: now, now-shift, now-2*shift, ...
    Each window has length `window_bars` (bars). For each window we run a
    train/val split and compute scores.

    score_on:
      - val: summarize by val_score
      - train: summarize by train_score
      - mix: summarize by alpha*val + (1-alpha)*train
    """

    so = str(score_on or "val").lower().strip()
    if so not in ("train", "val", "mix"):
        so = "val"
    try:
    ...
    for i in range(windows):
        ...
        if so == "train":
            ...
            s_sel = alpha * float(s_va) + (1.0 - alpha) * float(s_tr)
        else:
            s_sel = float(s_va)
        ...
        # For summary, require minimum trades on val (always), because
        # val part determines real-world tradability.
        if trades_va >= min_trades:
            ok_windows += 1
            scores_sel.append(float(s_sel))

    if scores_sel:
        scores_sorted = sorted(scores_sel)
        mean = sum(scores_sel) / float(len(scores_sel))
        median = scores_sorted[len(scores_sorted) // 2]
        out["summary"] = {
            "windows_done": int(len(out["windows"])),
            "ok_windows": int(ok_windows),
            "mean_sel_score": float(mean),
            "median_sel_score": float(median),
            "min_sel_score": float(min(scores_sel)),
            "max_sel_score": float(max(scores_sel)),
        }
    else:
...
def _sample_params(
    ...
    min_hold_bars = int(rng.choice([0, 1, 2, 3, 4, 6, 8, 12])) if _opt("min_hold_bars") else int(_fx("min_hold_bars", 0))
    ...
    try:
        if optimizer == "optuna_tpe":
            ...
            def _params_from_trial(trial, position_usd: float, fixed_params: Dict[str, Any], opt_flags: Dict[str, bool]) -> Dict[str, Any]:
                ...
                min_hold_bars = int(trial.suggest_categorical("min_hold_bars", [0, 1, 2, 3, 4, 6, 8, 12])) if _opt("min_hold_bars") else int(_fx("min_hold_bars", 0))
        ...
        if bool(cfg.get("robust_check")) and run.best_params:
            try:
                ...
                for c in cand_list:
                    ...
                    _add_log(run, f"ROBUST_CAND name={c.get('name')} mean={summ.get('mean_sel_score')} min={summ.get('min_sel_score')} ok={summ.get('ok_windows')}")
...
Если выключено: используем текущую свечу (может перерисовываться на live).'>?</span></label>
          ...
          <div class='card' style='grid-column:1/-1; margin-top:6px; margin-bottom:16px;'>
            ...
            <div style='display:grid; grid-template-columns: 200px 120px 70px 1fr; gap:8px 10px; align-items:center;'>
              ...
              <div>min_hold_bars</div>
              <div><input name='fx_min_hold_bars' value='0' style='width:110px'/></div>
              <div><label class='lbl'><input type='checkbox' name='opt_min_hold_bars' /> opt</label></div>
              <div class='muted'>Мин. число баров удержания перед переворотом</div>
...
def _optimize_run_html(run_id: str) -> str:
    ...
    if isinstance(robust, dict) and robust.get("windows"):
        ...
        sum_line = (
            ...
            f"mean={html.escape(str(summ.get('mean_sel_score')))} "
            f"median={html.escape(str(summ.get('median_sel_score')))} "
            f"min={html.escape(str(summ.get('min_sel_score')))} "
            f"max={html.escape(str(summ.get('max_sel_score')))}"
            f"</div>"
        )
        ...
        for w in robust.get("windows") or []:
            ...
            try:
                ...
                # Build a chart link that reproduces the exact window and parameters.
                _qp: Dict[str, Any] = {}
                try:
                    for _k, _v in (best_params or {}).items():
                        # Chart expects overrides as p_<param>=...
                        if isinstance(_v, bool):
                            _qp[f"p_{_k}"] = "1" if _v else "0"
                        else:
                            _qp[f"p_{_k}"] = str(_v)
                except Exception:
                    _qp = {}
                _qstr = urllib.parse.urlencode(_qp, doseq=True)
                win_link = (
                    f"/chart?symbol={urllib.parse.quote(str(_sym))}"
                    f"&tf={urllib.parse.quote(str(_tf))}"
                    f"&limit={_limit}"
                    f"&strategy={urllib.parse.quote(str(_strategy))}"
                    f"&end_shift_hours={_shift_h}"
                    + (f"&{_qstr}" if _qstr else "")
                )
            except Exception:
                win_link = ""
            link_html = ("<a class='btn ghost' target='_blank' href='%s'>📈</a>" % win_link) if win_link else ""
            rows.append(
                "<tr>"
                f"<td>{int(w.get('i', 0))}</td>"
                f"<td>{html.escape(str(w.get('end_shift_hours')))}</td>"
                f"<td>{html.escape(str(w.get('bars')))}</td>"
                f"<td>{html.escape(str(w.get('sel_score', w.get('val_score'))))}</td>"
        ...
        robust_table = (
            ...
            f"score_on={html.escape(str(cfg_r.get('score_on','val')))} alpha={html.escape(str(cfg_r.get('mix_alpha','')))}"
            f"</div>"
            + sum_line +
            "<div style='overflow:auto'>"
            "<table class='table' style='min-width:1280px'>"
            "<thead><tr>"
            "<th>i</th><th>shift_h</th><th>bars</th><th>sel_score</th><th>train_score</th><th>val_score</th>"
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    fixed_params = {
        ...
        "min_hold_bars": _i("fx_min_hold_bars", 0),
    ...
    opt_flags = {
        ...
        "adx_no_trade_below": _bdef("opt_adx_no_trade_below", False),
        "st_factor": _bdef("opt_st_factor", False),
        ...
        "atr_mult": _bdef("opt_atr_mult", False),
        "min_hold_bars": _bdef("opt_min_hold_bars", False),
