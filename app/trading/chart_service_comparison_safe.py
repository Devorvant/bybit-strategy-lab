...
def _attach_strategy_match_id(rows: list[dict[str, Any]], *, symbol: str, tf: str) -> list[dict[str, Any]]:
    ...
    for row in rows or []:
        ...
        try:
            ...
            entry_ts_i = None
        if not entry_ts_i or side not in {"LONG", "SHORT"}:
            row["match_id"] = None
            continue
        action = "long" if side == "LONG" else "short"
        row["match_id"] = f"auto-{sym}-{tfv}-{action}-{entry_ts_i}"
    return rows
...
class TradeChartService:
    ...
    def build_trade_chart_payload(
        ...
        *,
        ...
        strategy_trades = _attach_strategy_match_id(strategy_trades, symbol=symbol, tf=tf)
