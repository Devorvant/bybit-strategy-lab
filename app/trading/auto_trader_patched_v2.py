...
def start_auto_loop(symbol: str, tf: str, *, opt_id: int, poll_sec: float | None = None, trade_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Start behavior:
    # - if flat + OPEN_LONG/OPEN_SHORT => execute immediately
    # - otherwise just start loop in RUNNING mode
    start_result = probe
    start_state = 'RUNNING'
    armed_bar_ts = probe.get('bar_ts')

    probe_action = str(probe.get('action') or '').upper()
    probe_side = str(probe.get('exchange_side') or '').upper()

    should_execute_now = (
        probe_side == 'FLAT'
        and probe_action in {'OPEN_LONG', 'OPEN_SHORT'}
    )

    if should_execute_now:
        _reset_db_connection()
        executed = process_symbol(
            symbol=symbol,
            tf=tf,
            execute=True,
            opt_id=int(opt_id),
        ...
        start_result = executed
        if not executed.get('ok'):
            return {
                'ok': False,
                'error': executed.get('error') or 'initial execute failed',
                'probe': probe,
                'result': executed,
            }
    ...
    with _LOOP_LOCK:
        ...
        _LOOP_STATE.update({
            ...
            'last_error': start_result.get('error') if isinstance(start_result, dict) else None,
            'trade_params': safe_trade_params,
            'state': start_state,
            'armed_bar_ts': armed_bar_ts,
            'blocked_reason': None,
        })
    ...
    _log_event({
        ...
        'mode': 'loop_start',
        'loop': payload,
        'reason': 'started immediately' if should_execute_now else 'started in running mode',
    })
    return {
        'ok': True,
        'loop': payload,
