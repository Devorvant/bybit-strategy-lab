...
def _persist_fills_for_order(conn, client: BybitClient, *, symbol: str, tf: str | None, order_link_id: str | None) -> int:
    if not order_link_id:
        logger.info("fills persist skipped: empty order_link_id symbol=%s tf=%s", symbol, tf)
        return 0
    try:
        logger.info("fills persist start: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        fills = client.find_fills_by_order_link_id(symbol=symbol, order_link_id=order_link_id, limit=100)
        logger.info("fills persist fetched: symbol=%s tf=%s order_link_id=%s fills_count=%s", symbol, tf, order_link_id, len(fills or []))
        if not fills:
            return 0
        ...
        try:
            logger.info("fills persist first row: order_link_id=%s row=%s", order_link_id, rows[0] if rows else None)
        except Exception:
            pass

        log_execution_fills(conn, rows)
        logger.info("fills persist inserted: symbol=%s tf=%s order_link_id=%s rows_count=%s", symbol, tf, order_link_id, len(rows))
        upsert_execution_fill_summary(conn, symbol=symbol, tf=tf, order_link_id=order_link_id)
        logger.info("fills persist summary updated: symbol=%s tf=%s order_link_id=%s", symbol, tf, order_link_id)
        return len(rows)
    except Exception:
