...
<head>
  ...
  <style>
    ...
    .btnPrimary { background:#1e8e5a; border:1px solid rgba(255,255,255,0.12); color:#fff; padding:8px 12px; border-radius:10px; cursor:pointer; font-weight:700; }
    .btnPrimary:hover { filter: brightness(1.08); }
    .btnSecondary { background:#0b1220; border:1px solid #2b3d5f; color:#e6e6e6; padding:8px 12px; border-radius:10px; cursor:pointer; font-weight:700; }
...
def _run_optimizer_sync(run: RunState, bars: List[Tuple[int, float, float, float, float, float]], conn) -> None:
    def _mix_sel_score(s_tr: float, s_va: float, alpha: float) -> float:
        try:
            s_tr_f = float(s_tr)
            s_va_f = float(s_va)
        except Exception:
            return float("-inf")
        if not math.isfinite(s_tr_f) or not math.isfinite(s_va_f):
            return float("-inf")
        return alpha * s_va_f + (1.0 - alpha) * s_tr_f

    def _select_score(select_on: str, s_tr: float, s_va: float, alpha: float) -> float:
        if select_on == "train":
            return float(s_tr)
        if select_on == "val":
            return float(s_va)
        return _mix_sel_score(s_tr, s_va, alpha)
    ...
    def _maybe_update_best(params: Dict[str, Any], trial_no: int) -> float:
        ...
        if select_on not in ("train", "val"):
            select_on = "train"
        sel_score = _select_score(select_on, s_tr, s_va, cfg.get("mix_alpha", 0.7))
        ...
        "mix_alpha": mix_alpha,
                ...
                "sel_score_breakdown": (bd_tr if select_on == "train" else bd_va) if select_on != "mix" else bd_va,
            ...
            sel_bd = (bd_tr if select_on == "train" else bd_va) if select_on != "mix" else bd_va
            try:
            ...
            if mode == "windows":
                ...
                for wi in range(windows_count):
                    ...
                    for t in range(1, trials_per_seed + 1):
                        ...
                        sel_score = float(_select_score(select_on, s_tr, s_va, cfg.get("mix_alpha", 0.7)))
                        sel_bd = (bd_tr if select_on == "train" else bd_va) if select_on != "mix" else bd_va
...
Это влияет и на оптимизацию, и на robust-check (нулевое окно).'>?</span></span></label>
          ...
          <div>
            <label>mix_alpha</label>
            <input name='mix_alpha' value='0.7' style='width:90px' title='Для select_on=mix: score = alpha*val + (1-alpha)*train'/>
          </div>
          <div>
...
Если выключено: используем текущую свечу (может перерисовываться на live).'>?</span></label>
          ...
          <div class='card' class='card' style='grid-column:1/-1; margin-top:10px; margin-bottom:18px;'>
          ...
          <div>
            <button type='submit' class='startbtn btnPrimary'>Start optimize</button>
          </div>
          <div>
            <button type='button' id='btnMultiSeed' style='background:#1f5c3b;' class='startbtn btnPrimary'>Start multi-seed (Random)</button>
          </div>
          <div>
        ...
        <div class='card'>
  <div class='card-title'>Формула score и веса (classic)</div>
        <div class='muted' style='margin-bottom:10px; line-height:1.35;'>
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    try:
        ...
        mix_alpha = 0.7
    mix_alpha = 0.0 if mix_alpha < 0.0 else (1.0 if mix_alpha > 1.0 else mix_alpha)
