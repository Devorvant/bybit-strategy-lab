...
from .journal import fetch_strategy_trade_journal
...
def _sync_strategy_trade_journal_with_strategy(
    *,
    ...
    """
    Hard sync of strategy_trade_journal with current strategy state.

    Requested rule:
    - if keys match,
    - and journal row is still open,
    - and it is NOT the latest currently open strategy trade,
      then rewrite/close the journal row.
    - exit fields are taken from strategy when available.
    """
    if not conn or not journal_rows:
        return 0

    strategy_by_key: dict[str, dict[str, Any]] = {}
    open_strategy_rows: list[dict[str, Any]] = []
    ...
    for tr in strategy_trades or []:
        ...
        try:
            exit_ts_i = int(exit_ts) if exit_ts is not None else None
        except Exception:
            exit_ts_i = None

        if not exit_ts_i:
            open_strategy_rows.append(tr)

    current_open_key = None
    if open_strategy_rows:
        def _entry_ts_safe(row: dict[str, Any]) -> int:
            try:
                return int(row.get("entry_ts") or 0)
            except Exception:
                return 0
        current_open = max(open_strategy_rows, key=_entry_ts_safe)
        current_open_key = str(current_open.get("match_id") or "").strip() or None

    updated = 0
    for jr in journal_rows or []:
        if str(jr.get("status") or "").lower() != "open":
            continue
        ...
        if current_open_key and key == current_open_key:
            continue

        exit_ts = st.get("exit_ts")
        exit_price = st.get("exit_price")
        close_reason = st.get("reason") or st.get("close_reason") or "strategy_sync_close"
        ...
        # If strategy does not expose explicit exit yet, still close stale journal row
        # using the best available strategy fields.
        if not exit_ts_i:
            try:
                exit_ts_i = int(st.get("entry_ts") or jr.get("entry_ts") or 0)
            except Exception:
                exit_ts_i = None

        if exit_price is None:
            exit_price = st.get("exit_price") or st.get("entry_price") or jr.get("entry_price")
