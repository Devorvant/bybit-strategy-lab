...
def _robust_evaluate(
    *,
    ...
    params_eval["confirm_on_close"] = bool(cfg.get("confirm_on_close", False))
    if use_costs_in_opt:
...
def _sample_params(
    ...
    adx_no_trade_below = float(rng.uniform(10.0, 35.0)) if _opt("adx_no_trade_below") else float(_fx("adx_no_trade_below", 24.0))
    st_factor = float(rng.uniform(2.0, 8.0)) if _opt("st_factor") else float(_fx("st_factor", 4.0))
    atr_mult = float(rng.uniform(1.5, 10.0)) if _opt("atr_mult") else float(_fx("atr_mult", 4.0))
    ...
    try:
        if optimizer == "optuna_tpe":
            ...
            def _params_from_trial(trial, position_usd: float, fixed_params: Dict[str, Any], opt_flags: Dict[str, bool]) -> Dict[str, Any]:
                ...
                adx_no_trade_below = float(trial.suggest_float("adx_no_trade_below", 10.0, 35.0)) if _opt("adx_no_trade_below") else float(_fx("adx_no_trade_below", 24.0))
                st_factor = float(trial.suggest_float("st_factor", 2.0, 8.0)) if _opt("st_factor") else float(_fx("st_factor", 4.0))
                atr_mult = float(trial.suggest_float("atr_mult", 1.5, 10.0)) if _opt("atr_mult") else float(_fx("atr_mult", 4.0))
...
Если выключено: используем текущую свечу (может перерисовываться на live).'>?</span></label>
          ...
          <div style='grid-column:1/-1; border:1px solid rgba(255,255,255,0.10); padding:10px 12px; border-radius:14px; margin-top:6px;'>
            ...
            <div style='display:grid; grid-template-columns: 200px 120px 70px 1fr; gap:8px 10px; align-items:center;'>
              ...
              <div><label class='lbl'><input type='checkbox' name='opt_use_no_trade' checked /> opt</label></div>
              ...
              <div><label class='lbl'><input type='checkbox' name='opt_use_rev_cooldown' checked /> opt</label></div>
              ...
              <div><label class='lbl'><input type='checkbox' name='opt_rev_cooldown_hrs' checked /> opt</label></div>
              ...
              <div><label class='lbl'><input type='checkbox' name='opt_use_flip_limit' checked /> opt</label></div>
              ...
              <div><label class='lbl'><input type='checkbox' name='opt_max_flips_per_day' checked /> opt</label></div>
...
async def optimize_start(payload: Dict[str, Any]):
    ...
    opt_flags = {
        # main defaults: True
        "use_no_trade": _bdef("opt_use_no_trade", True),
        "adx_no_trade_below": _bdef("opt_adx_no_trade_below", True),
        "st_factor": _bdef("opt_st_factor", True),
        "use_rev_cooldown": _bdef("opt_use_rev_cooldown", True),
        "rev_cooldown_hrs": _bdef("opt_rev_cooldown_hrs", True),
        "use_flip_limit": _bdef("opt_use_flip_limit", True),
        "max_flips_per_day": _bdef("opt_max_flips_per_day", True),
