    ...
    *,
    ...
    if strategy == "my_strategy3.py":
        try:
            ...
            signal_compare_html = f"""
    <div class=\"params-card\" style=\"margin-top:12px\">
      <div class=\"params-head\"><b>Signal compare</b><span class=\"muted\">(selected: {html.escape(signal_mode)})</span></div>
      <div style=\"display:grid;grid-template-columns:1fr 1fr;gap:10px\">
        <div style=\"padding:10px;border:1px solid #334155;border-radius:10px;background:#0b1220\">
          <div class=\"muted\" style=\"font-size:12px;margin-bottom:6px\">Legacy signal</div>
          <div style=\"font-size:12px;color:#94a3b8;margin-bottom:6px\">{html.escape(legacy_note or 'signal')}</div>
          <div style=\"display:inline-block;padding:4px 10px;border-radius:999px;background:{'#16a34a' if legacy_signal == 'LONG' else ('#dc2626' if legacy_signal == 'SHORT' else '#475569')};color:#fff;font-weight:700\">{html.escape(legacy_signal)}</div>
        </div>
        <div style=\"padding:10px;border:1px solid #334155;border-radius:10px;background:#0b1220\">
          <div class=\"muted\" style=\"font-size:12px;margin-bottom:6px\">Backtest core signal</div>
          <div style=\"font-size:12px;color:#94a3b8;margin-bottom:6px\">{html.escape(bt_note or 'signal')}</div>
          <div style=\"display:inline-block;padding:4px 10px;border-radius:999px;background:{'#16a34a' if bt_signal == 'LONG' else ('#dc2626' if bt_signal == 'SHORT' else '#475569')};color:#fff;font-weight:700\">{html.escape(bt_signal)}</div>
        </div>
      </div>
    </div>
    """
    ...
    if signal_mode not in ("legacy", "backtest"):
        signal_mode = "backtest"
    signal_mode_options = "\n".join(
        f"<option value='{m}' {'selected' if m == signal_mode else ''}>{m}</option>"
        for m in ("backtest", "legacy")
    )
...
<body>
  <form id="chartForm" class="topbar" method="get" action="/chart">
    ...
    <select name="signal_mode" style="width:120px">{signal_mode_options}</select>
  ...
  <div class="wrap">{params_html}{slider_html}{manual_trade_html}{live_trade_html}{plot_html}{trades_table_html}</div>
</body>
</html>
"""
