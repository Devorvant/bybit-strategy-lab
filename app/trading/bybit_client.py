...
def _make_client() -> Optional['HTTP']:
    ...
    if mode == "mainnet":
        if not (BYBIT_KEY and BYBIT_SECRET):
            return None
        return HTTP(testnet=False, api_key=BYBIT_KEY, api_secret=BYBIT_SECRET)

    # default: testnet
    if not (BYBIT_KEY and BYBIT_SECRET):
        return None
    return HTTP(testnet=True, api_key=BYBIT_KEY, api_secret=BYBIT_SECRET)
...
class BybitClient:
    ...
    def clean_symbol(self, sym: str) -> str:
        ...
        for suf in ['.P', '.PERP', 'PERP']:
            if s.endswith(suf):
                s = s[:-len(suf)]
        # backward safety: old _R symbols should not hit the exchange
        if s.endswith('_R'):
            s = s[:-2]
        return s
