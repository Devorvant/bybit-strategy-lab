...
def _persist_fills_for_order(conn, client: BybitClient, *, symbol: str, tf: str | None, order_link_id: str | None) -> int:
    ...
    try:
        ...
        if fills:
            try:
                logger.info("fills persist first fill: order_link_id=%s fill=%s", order_link_id, fills[0])
            except Exception:
                pass
        if not fills:
            return 0
        log_execution_fills(conn, symbol=symbol, tf=tf, fills=fills)
        logger.info("fills persist inserted: symbol=%s tf=%s order_link_id=%s fills_count=%s", symbol, tf, order_link_id, len(fills or []))
