...
def make_chart_html(
    ...
    *,
    ...
    # --- Simple in-page "playground" slider (strategy3 only) ---
    slider_html = ""
    if strategy == "my_strategy3.py":
        try:
            st_val = float(cur_params.get("st_factor", 4.0))
        except Exception:
            st_val = 4.0
        slider_html = f"""
        <div class="params-card" id="playCard">
          <div class="params-head">
            <b>Playground</b>
            <span class="muted">двигаем один параметр и обновляем график через fetch (без перезагрузки страницы)</span>
          </div>
          <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-top:8px;">
            <div style="min-width:140px;opacity:0.9;">st_factor</div>
            <input id="play_st_factor" type="range" min="1" max="8" step="0.05" value="{st_val:.4f}" style="flex:1 1 260px;"/>
            <div style="min-width:110px;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace;">
              <span id="play_st_factor_val">{st_val:.4f}</span>
              <span id="play_status" class="muted" style="margin-left:8px;"></span>
            </div>
          </div>
          <div class="muted" style="margin-top:6px;">
            Обновление происходит при отпускании ползунка. Таблица сделок ниже сейчас не пересчитывается — только сам график.
          </div>
        </div>
        """
...
<head>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", () => {{
      ...
      // Playground slider: update only the plot via /api/chart_update
      const slider = document.getElementById('play_st_factor');
      const valEl = document.getElementById('play_st_factor_val');
      const stEl = document.getElementById('play_status');
      if (slider) {{
        let ctrl = null;
        const doUpdate = async () => {{
          const v = slider.value;
          if (valEl) valEl.textContent = Number(v).toFixed(4);
          ...
          const inp = document.getElementById('p_st_factor');
          if (inp) inp.value = v;

          // Build query from the current form fields.
          const form = document.getElementById('chartForm');
          const fd = new FormData(form);
          fd.set('use_overrides', '1');
          fd.set('p_st_factor', v);
        ...
        slider.addEventListener('change', doUpdate);
        // Live value preview while dragging (no fetch).
        slider.addEventListener('input', () => {{
          if (valEl) valEl.textContent = Number(slider.value).toFixed(4);
        }});
      }}
    }});
  </script>
...
<body>
  ...
  <div class="wrap">{params_html}{slider_html}{plot_html}{trades_table_html}</div>
</body>
</html>
"""
