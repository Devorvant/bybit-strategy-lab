...
def upsert_strategy_trade_from_strategy_row(
    conn,
    *,
    trade_key: str,
    symbol: str,
    ...
    bybit_order_link_id: str | None = None,
) -> None:
    ...
    upsert_strategy_trade_open(conn, {
        ...
        "tp_price": row.get("trade_tp") if row.get("trade_tp") is not None else row.get("tp_price"),
        "sl_price": row.get("trade_sl") if row.get("trade_sl") is not None else row.get("sl_price"),
        "exit_ts": exit_ts_i,
        "exit_price": row.get("exit_price"),
        "close_reason": row.get("reason") or row.get("close_reason"),
        "status": "closed" if exit_ts_i else "open",
        "bybit_order_link_id": bybit_order_link_id or trade_key,
        "created_ts": row.get("created_ts") or now_ms,
        "updated_ts": now_ms,
    })
...
def upsert_strategy_trade_open(conn, row: dict) -> None:
    ...
    SET
        -- immutable identity fields: keep original executed identity
        symbol = strategy_trade_journal.symbol,
        tf = strategy_trade_journal.tf,
        strategy = strategy_trade_journal.strategy,
        opt_id = strategy_trade_journal.opt_id,
        side = strategy_trade_journal.side,
        entry_ts = strategy_trade_journal.entry_ts,
        bybit_order_link_id = COALESCE(strategy_trade_journal.bybit_order_link_id, EXCLUDED.bybit_order_link_id),

        -- mutable state fields: may be refined from strategy state later
        entry_price = COALESCE(EXCLUDED.entry_price, strategy_trade_journal.entry_price),
        tp_price = COALESCE(EXCLUDED.tp_price, strategy_trade_journal.tp_price),
        sl_price = COALESCE(EXCLUDED.sl_price, strategy_trade_journal.sl_price),
        exit_ts = COALESCE(EXCLUDED.exit_ts, strategy_trade_journal.exit_ts),
        exit_price = COALESCE(EXCLUDED.exit_price, strategy_trade_journal.exit_price),
        close_reason = COALESCE(EXCLUDED.close_reason, strategy_trade_journal.close_reason),
        status = COALESCE(EXCLUDED.status, strategy_trade_journal.status),
        updated_ts = EXCLUDED.updated_ts
    """
    sqlite_sql = """
    ...
    *,
    ...
    if _is_sqlite_conn(conn):
        cur = conn.execute(
            ...
            WHERE symbol=? AND tf=? AND strategy=? AND (? IS NULL OR opt_id=?) AND status='open'
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT trade_key, symbol, tf, strategy, opt_id, side,
                   ...
                   bybit_order_link_id, created_ts, updated_ts
            FROM strategy_trade_journal
            WHERE symbol=%s AND tf=%s AND strategy=%s AND (%s IS NULL OR opt_id=%s) AND status='open'
            ORDER BY updated_ts DESC
            LIMIT 1
            """,
            (symbol, tf, strategy, opt_id, opt_id),
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE strategy_trade_journal
            SET exit_ts = COALESCE(%(exit_ts)s, exit_ts),
                exit_price = COALESCE(%(exit_price)s, exit_price),
                close_reason = COALESCE(%(close_reason)s, close_reason),
                status = 'closed',
...
def _fetch_existing_strategy_trade_slice(
    conn,
    *,
    symbol: str,
    tf: str,
    ...
    rows = fetch_strategy_trade_journal(
        conn,
        symbol=symbol,
        tf=tf,
        strategy=strategy,
...
def replace_strategy_trade_journal_slice(
    conn,
    *,
    symbol: str,
    tf: str,
    ...
    # Preserve previously captured strategy state that current recalculation may not carry,
    # especially SL/TP and immutable execution identity fields.
    existing_map = _fetch_existing_strategy_trade_slice(
        conn,
        symbol=symbol,
        tf=tf,
        strategy=strategy,
    ...
    for row in rows:
        ...
        prev = existing_map.get(key) if key else None
        if prev:
            # Preserve identity if incoming row is sparse or shifted.
            item["trade_key"] = prev.get("trade_key") or item.get("trade_key")
            item["bybit_order_link_id"] = prev.get("bybit_order_link_id") or item.get("bybit_order_link_id") or item.get("trade_key")
            item["entry_ts"] = prev.get("entry_ts") or item.get("entry_ts")
            item["side"] = prev.get("side") or item.get("side")

            # Preserve SL/TP if recalculated row does not provide them.
            if item.get("sl_price") in (None, "") and item.get("trade_sl") in (None, ""):
                item["sl_price"] = prev.get("sl_price")
            if item.get("tp_price") in (None, "") and item.get("trade_tp") in (None, ""):
                item["tp_price"] = prev.get("tp_price")

            # Preserve entry price if not present.
            if item.get("entry_price") in (None, ""):
                item["entry_price"] = prev.get("entry_price")

            # Preserve created_ts for historical continuity.
            if item.get("created_ts") in (None, ""):
                item["created_ts"] = prev.get("created_ts")
        prepared_rows.append(item)

    if _is_sqlite_conn(conn):
        conn.execute(
            """
            DELETE FROM strategy_trade_journal
            WHERE symbol=? AND tf=? AND strategy=? AND (? IS NULL OR opt_id=?)
            """,
            (symbol, tf, strategy, opt_id, opt_id),
        )
        try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                DELETE FROM strategy_trade_journal
                WHERE symbol=%s AND tf=%s AND strategy=%s AND (%s IS NULL OR opt_id=%s)
                """,
                (symbol, tf, strategy, opt_id, opt_id),
            )
        try:
    ...
    for row in prepared_rows:
        upsert_strategy_trade_from_strategy_row(
            ...
            opt_id=opt_id,
            strategy_row=row,
            bybit_order_link_id=str(row.get("bybit_order_link_id") or row.get("trade_key") or ""),
        )
    return len(prepared_rows)


def sync_existing_strategy_trade_from_strategy_row(
    conn,
    *,
    trade_key: str,
    symbol: str,
    ...
    """
    Update only mutable state for an already executed strategy_trade_journal row.
    Do NOT change immutable identity fields such as trade_key / entry_ts / side.
    Returns True when a matching journal row existed and was updated.
    """
    row = dict(strategy_row or {})
    if not trade_key:
        return False
    ...
    if _is_sqlite_conn(conn):
        cur = conn.execute(
            ...
            SELECT 1
            FROM strategy_trade_journal
            WHERE trade_key=? AND symbol=? AND tf=? AND strategy=? AND (? IS NULL OR opt_id=?)
            LIMIT 1
            """,
            (trade_key, symbol, tf, strategy, opt_id, opt_id),
        )
        ...
        conn.execute(
            ...
            (
                row.get("entry_price"),
                row.get("trade_tp") if row.get("trade_tp") is not None else row.get("tp_price"),
                row.get("trade_sl") if row.get("trade_sl") is not None else row.get("sl_price"),
                exit_ts_i,
                row.get("exit_price"),
                row.get("reason") or row.get("close_reason"),
                "closed" if exit_ts_i else "open",
                now_ms,
                trade_key, symbol, tf, strategy, opt_id, opt_id
            ),
        )
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT 1
            FROM strategy_trade_journal
            WHERE trade_key=%s AND symbol=%s AND tf=%s AND strategy=%s AND (%s IS NULL OR opt_id=%s)
            LIMIT 1
            """,
            (trade_key, symbol, tf, strategy, opt_id, opt_id),
        )
        ...
        cur.execute(
            """
            UPDATE strategy_trade_journal
            SET entry_price = COALESCE(%(entry_price)s, entry_price),
                tp_price = COALESCE(%(tp_price)s, tp_price),
                sl_price = COALESCE(%(sl_price)s, sl_price),
                exit_ts = COALESCE(%(exit_ts)s, exit_ts),
                exit_price = COALESCE(%(exit_price)s, exit_price),
                close_reason = COALESCE(%(close_reason)s, close_reason),
                status = COALESCE(%(status)s, status),
                updated_ts = %(updated_ts)s
            WHERE trade_key=%(trade_key)s AND symbol=%(symbol)s AND tf=%(tf)s AND strategy=%(strategy)s
              AND (%(opt_id)s IS NULL OR opt_id=%(opt_id)s)
            """,
            {
                "trade_key": trade_key,
                "symbol": symbol,
